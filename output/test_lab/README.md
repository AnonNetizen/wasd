...
- `polygon_book_test.tscn`：翻开书本 Polygon 素材管线隔离原型。`assets/polygon_art/open_book_source.png` 是由内置 `imagegen` 生成、再用 Godot 归一为纯 `#ff00ff` 键色背景的制作输入；`data/polygon_imports/open_book.json` 逐素材声明源图、Style Profile、输出、锚点、语义区域、重点面和碰撞策略，`data/polygon_art_style.json` 固定 256×256 分析分辨率、3px RDP、24px 内部采样、确定性 seed、70–140 目标面数 / 160 硬上限、4px 最小可见面高度及页面 / 封皮 / 强调色板。`polygon_asset_compiler.gd` 只使用 Godot `BitMap`、`Geometry2D` 与 `Image`：色键分离后用 marching squares + RDP 保留最大轮廓，以轮廓约束单元承载内部 Delaunay 和确定性细分，过滤退化面并保留最大顶点连通面集，再做色板吸附和语义标记；高度不足 4px 的瘦长三角仍负责无缝填充，但强制继承相邻主体面的颜色，不再成为独立细线块。当前书本产出 71 面、57 个共享逻辑顶点的 schema v1 JSON，并通过 manifest 将左下最大页面面设为明确暗阶。右侧 `PolygonAsset2D` 完全不读取源 PNG，而是把 71 个原始平色面各展开一次，统一生成一个单层 `ArrayMesh` surface；不复制底页或翻页覆盖层。清理顺序和逐面运动掩码保存在 UV，外轮廓由固定 `Line2D` 单独绘制，调试网格由 draw API 绘制，`Area2D` 使用不随视觉动画变形的简化凸包。翻页动画只使用两类变化：右页内部顶点由一条从书脊扫向外缘的有界折痕波改变位置，13 个右页面片按各自相位在页面亮 / 暗 / 暖色之间改变颜色；待机不再移动顶点，避免页面与固定边界形成亚像素细缝。进入场景默认循环演示静止展示、1.2 秒折痕扫页与色变、清理和复原，`A` 关闭 / 重启自动演示；`Space` 手动翻页，`C` 手动清理 / 复原，`M` 切网格、`O` 切源图、`R` 重置、`Esc` 返回索引。`polygon_asset_smoke.gd` 覆盖编译确定性、源哈希、拓扑 / 面数 / 色板 / 区域 / 锚点 / 碰撞、瘦长面视觉合并、重点面、无重复面和单 MeshInstance2D / 单 surface / 无纹理运行时约束，`capture_polygon_book_test.gd` 自动生成静态源图对比、翻页中段截图及五阶段运动条带。最终风格、轮廓可读性和动画观感仍为待用户人工验收。
