...
- `polygon_book_test.tscn`：翻开书本 Polygon 素材管线隔离原型。`assets/polygon_art/open_book_source.png` 是由内置 `imagegen` 生成、再用 Godot 归一为纯 `#ff00ff` 键色背景的制作输入；运行时不读取它。`data/polygon_imports/open_book.json` 逐素材声明源图、内部形状引导、输出、锚点、重点面和碰撞策略；当前把书脊声明为可检测的 `vertical_band`，不是人工描好的最终面。`data/polygon_art_style.json` 使用 `shape_guided` 构建模式，固定 256×256 分析分辨率、3px RDP、48–80 目标面数 / 160 硬上限、8px 最短轮廓边、80px² 最小面面积、6px 最小面高度及页面 / 封皮 / 强调色板。编译器只从源图提取最大外轮廓、内部特征的大致位置 / 宽度 / 颜色；实际共享节点和块由工具重建。书脊会在三条采样线上检测中心暗带，记录原始宽度和平均颜色，再扩展到 Style Profile 规定的 20–28px 可读宽度，按最近色吸附到统一色板。工具随后在书脊两侧插入受保护共边，把外轮廓切成左页 / 书脊 / 右页三个分区并分别构网，因此没有任何面跨过书脊，也没有额外书脊覆盖层。轮廓清理与后续细分都必须满足面积和高度门槛，坏拓扑会直接令编译失败，不通过共色或隐藏小面掩盖。当前书本产出 48 面、32 个共享逻辑顶点、14 个轮廓节点的 schema v1 JSON；书脊由 3 个固定面表现，原图检测宽度约 3px，工具构建宽度为 20px，颜色取自源图后吸附为 `accent_warm`。manifest 另将左下大页面面设为明确暗阶。每个面保存独立 `surface_kind`，页面运动不依赖视觉颜色。右侧 `PolygonAsset2D` 把每个原始平色面各展开一次，统一生成一个单层 `ArrayMesh` surface；不复制底页、书脊或翻页覆盖层。清理顺序与共享逻辑顶点运动权重保存在 UV，逐面色变标记保存在顶点色 alpha；同一逻辑点的全部渲染副本始终使用相同运动权重，书脊与固定书体不参与翻页。外轮廓由固定 `Line2D` 单独绘制，调试网格由 draw API 绘制，`Area2D` 使用不随视觉动画变形的简化凸包。翻页动画只改变右页节点位置和原始面的颜色：有界折痕从书脊扫向外缘，可翻页面片按各自相位在页面亮 / 暗 / 暖色之间变化。进入场景默认循环演示静止展示、1.2 秒折痕扫页与色变、清理和复原，`A` 关闭 / 重启自动演示；`Space` 手动翻页，`C` 手动清理 / 复原，`M` 切网格、`O` 切源图、`R` 重置、`Esc` 返回索引。`polygon_asset_smoke.gd` 覆盖编译确定性、源哈希、共边连通 / 水密拓扑、内部形状提取、源色吸附、书脊受保护共边、禁止跨书脊面、共享点变形一致性、轮廓短边、最小面面积 / 高度、面数 / 色板 / 区域 / 锚点 / 碰撞、重点面、无重复面和单 MeshInstance2D / 单 surface / 无纹理运行时约束；`capture_polygon_book_test.gd` 自动生成静态源图对比、翻页中段截图及五阶段运动条带。最终风格、书脊辨识度、轮廓可读性和动画观感仍为待用户人工验收。
...
运行时素材的权威输入是 `data/polygon_assets/open_book.polygon.json`、`polygon_asset_2d.gd` 与外部 shader；源 PNG 只作为制作证据和 Lab 左侧对照。Style Profile 与 schema v1 都是 Test Lab 私有实验约定，不自动成为正式项目契约。
