...
- `slime_room_shooter_3d.tscn`：3D 史莱姆房间射击实验。WASD / 方向键控制史莱姆在房间内移动，鼠标射线投射到 XZ 地面决定朝向，按住鼠标左键使用 24 发轻量对象池连续射击。史莱姆软体参考 `output/steamworks_lab/scripts/slime_body.gd`：`SlimeMembrane/EdgeRig` 用 24 个 `Marker3D` 控制点保存边缘状态，每段经 4 次 centripetal Catmull-Rom 采样形成 96 段连续轮廓，再沿 6 层纬线生成运行时 `ArrayMesh`；每个控制点带弹簧、阻尼、邻点平滑、面积压力、移动惯性和射击方向局部鼓包，玩法移动边界仍由稳定的 `Slime3D` 根节点控制。视觉改为原创“余烬地窖”弹幕房：近俯视正交镜头、48 块深色石砖、模块墙、封闭铁门、旧地毯、地牢射击靶、边缘木箱与两个暖色火盆构成封闭战斗房；`dungeon_room_fx_3d.gd` 用确定性多频正弦驱动四层火焰和两盏火光轻微闪烁，屏幕叠低强度颗粒暗角与旧铜边框，HUD 压成四角房间牌 / 操作条 / 无限弹药计数。连续膜继续使用深色胶体和浅绿亮边，弹丸改为带亮芯的拉长枪火并加入短促枪口闪光。`slime_room_shooter_3d_smoke.gd` 覆盖地牢结构、火焰 / 灯光控制器、控制点 / 动态网格、移动与射击形变、瞄准方向和池化发射；`capture_slime_room_shooter_3d.gd` 在移动连射与枪口闪光状态生成预览截图，`create_slime_room_shooter_3d_scene.gd` 用于重新生成场景并登记入口。
