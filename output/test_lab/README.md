...
三张源 PNG 保持原样；`universal_tile_grid.gd` 在运行时为 24 个逻辑 cell 叠加统一代码皮肤：连续地板色底衬、每格 2px 视觉出血和地板源图 3.5px 内采样共同消除相邻格黑缝；全部 tile 保留轻微圆角，非地板 tile 使用从自身图像主色自动推导、保留色相的深色不规则内边，并叠加缓慢环边游光与亚像素轮廓起伏；地板使用完整颜色覆盖和低强度周期亮边，四格交点不会因圆角透明度露底。碰撞继续保持完整 128×128 矩形，不随视觉圆角收缩。截图工具会冻结动画相位并关闭 collision / metadata 调试覆盖层，保证预览可重复且不让直线调试框干扰美术判断。
