...
- `glow_orb_bullet_focus_test.tscn`：独立的 1280×760 Shader 渐变圆球子弹专场，不替换既有胶质与材质实验。敌方红球和玩家白球严格共享一个纹理无关的 `canvas_item` Shader：无纹理 `Polygon2D` 由 `VERTEX` 局部坐标生成渐变坐标，以圆形 SDF 和导数抗锯齿确定纯圆边缘，再用 `smoothstep` 距离场从左上白热高光连续过渡到圆周阵营主体色。每个样本只保留一个固定 `Polygon2D + ShaderMaterial` 渲染面，飞行与命中不重建节点；保持无黑色 / 深色描边、无外发光和无纹理。左右展示超大红白弹体，底部继续演示玩家 `r=8 / 520 px/s` 与敌人 `r=5 / 280 px/s` 的实战飞行、无光晕短拖尾和命中。`H / D / T / B / Space / R / Esc` 分别切换超大弹体 / 命中、判定圆、拖尾、三档背景、暂停、重置和返回索引。`glow_orb_bullet_focus_smoke.gd` 检查 Shader 资源、四个稳定渲染面、圆形几何、`VERTEX` 坐标路径、SDF / `smoothstep` / `fwidth`、禁用旧同心色阶与三角扇代码、无描边 / 外发光、真实半径 / 速度、有界拖尾、无动态特效子节点和重置清理；`capture_glow_orb_bullet_focus.gd` 固定生成总览图，并探测红白弹体中心像素，透明空渲染会直接失败。最终渐变、高光、高速可读性和判定误导仍待用户人工验收。
