...
- `polygon_book_test.tscn`：翻开书本 Polygon 素材管线隔离原型。`assets/polygon_art/open_book_source.png` 是由内置 `imagegen` 生成、再用 Godot 归一为纯 `#ff00ff` 键色背景的制作输入；`data/polygon_imports/open_book.json` 逐素材声明源图、Style Profile、输出、锚点、语义区域、重点面和碰撞策略，`data/polygon_art_style.json` 固定 256×256 分析分辨率、3px RDP、24px 内部采样、确定性 seed、48–80 目标面数 / 160 硬上限、8px 最短轮廓边、80px² 最小面面积、6px 最小面高度及页面 / 封皮 / 强调色板。`polygon_asset_compiler.gd` 只使用 Godot `BitMap`、`Geometry2D` 与 `Image`：色键分离后用 marching squares + RDP 保留最大轮廓，先删除会形成短边或尖薄耳面的轮廓节点，再以清理后的轮廓约束确定性三角细分；只有三个子面都满足面积与高度门槛时才允许新增中心节点。编译器直接拒绝退化面、小面和细面，不再通过共色、并色或动画分组掩盖坏拓扑；随后验证每条内部边恰有两个所属面、每条轮廓边恰有一个所属面，再做色板吸附和语义标记。当前书本产出 48 面、31 个共享逻辑顶点、12 个轮廓节点的 schema v1 JSON，原始 RDP 轮廓有 16 个问题节点在编译时被移除；manifest 将左下大页面面设为明确暗阶。每个面另存独立 `surface_kind`，页面运动不依赖视觉颜色。右侧 `PolygonAsset2D` 完全不读取源 PNG，而是把 48 个原始平色面各展开一次，统一生成一个单层 `ArrayMesh` surface；不复制底页或翻页覆盖层。清理顺序与共享逻辑顶点运动权重保存在 UV，逐面色变标记保存在顶点色 alpha；同一逻辑点的全部渲染副本始终使用相同运动权重，页面与固定书体交界点保持固定，避免变形时露出背景。外轮廓由固定 `Line2D` 单独绘制，调试网格由 draw API 绘制，`Area2D` 使用不随视觉动画变形的简化凸包。翻页动画只使用两类变化：右页内部顶点由一条从书脊扫向外缘的有界折痕波改变位置，可翻页面片按各自相位在页面亮 / 暗 / 暖色之间改变颜色；待机不再移动顶点。进入场景默认循环演示静止展示、1.2 秒折痕扫页与色变、清理和复原，`A` 关闭 / 重启自动演示；`Space` 手动翻页，`C` 手动清理 / 复原，`M` 切网格、`O` 切源图、`R` 重置、`Esc` 返回索引。`polygon_asset_smoke.gd` 覆盖编译确定性、源哈希、共边连通 / 水密拓扑、共享点变形一致性、轮廓短边、最小面面积 / 高度、页面 / 封皮语义隔离、面数 / 色板 / 区域 / 锚点 / 碰撞、重点面、无重复面和单 MeshInstance2D / 单 surface / 无纹理运行时约束，`capture_polygon_book_test.gd` 自动生成静态源图对比、翻页中段截图及五阶段运动条带。最终风格、轮廓可读性和动画观感仍为待用户人工验收。
