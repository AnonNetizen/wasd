...
三张源 PNG 保持原样；`universal_tile_grid.gd` 在运行时为 24 个逻辑 cell 叠加统一代码皮肤：连续地板色底衬、每格 2px 中心对称视觉出血和地板源图 3.5px 内采样共同消除相邻格黑缝。地板与障碍分别使用固定种子的确定性错落绘制顺序，水平 / 垂直相邻格都存在正反覆盖，障碍始终位于地板上方，因此突出不再固定偏向左上。非地板 tile 的边色从较亮主体样本提取主色相，基础边保持约 24% 以上明度差，呼吸峰值也始终暗于主体；7px 边宽以 ±12% 幅度、约 4.8 秒周期呼吸，并保留较弱环边游光和亚像素轮廓起伏。地板使用完整颜色覆盖，5px 亮边以 ±8% 幅度、约 5.5 秒周期错相呼吸，四格交点不会露底。碰撞继续保持完整 128×128 矩形，不随视觉圆角、出血或动画变化。截图工具支持 `--capture-time=<秒>` 冻结任意动画时间，默认相位保持确定性，并关闭 collision / metadata 调试覆盖层。
