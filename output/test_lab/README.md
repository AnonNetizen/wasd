...
- `underground_spawn_beacon_test.tscn`：独立的“地底涌出信标”敌人出生预警实验，不修改正式 `client/`、`enemy_spawn_telegraph` cue、生成计划、Replay 或 VFX catalog。首版写实电能圈、火焰烟柱和晶体碎片因不符合项目史莱姆风被用户否决；当前版本改用深红胶体反边、危险粉红 `#ED2F72` 软胶主体、窄幅白色湿润高光，按 `CHARGE 0–0.45s / ERUPTION 0.45–1.25s / BREAKOUT 1.25–1.50s / REST 1.50–1.75s` 播放：两层宽瓣胶膜与暗色胶体凹井先受压收束，64 px 钝头拉丝胶柱从固定地面锚点向上生长，最多五枚大圆胶滴向上或外上运动，最后外胶膜与软胶瓣扩散并短促爆亮；不再使用电裂纹、火焰、烟雾、晶体尖片或微粒火星。1280×760 场景同时放置 3× 细节、1× 实战和三个相邻出生点；`A / Space / R` 控制自动、暂停和复位，`C` 固定三阶段，`B` 切换暗底 / 低对比意识层 / 拥挤战斗背景。三张 1254×1254 黑底灰度源 PNG 由内置 ImageGen 参照 `player_slime_fusion_test.png` 重生成，运行时通过 `Image.load()` 外部读取并做亮度转透明、史莱姆危险色换色；地面与液滴素材采用 4×4 逻辑图集，`style_pack.json` 保存精确提示词、尺寸、SHA-256 和两轮修订记录。效果主体不使用正式 `RingGeometry`、`_draw()`、`Polygon2D` 或 `Line2D`，场景也不嵌入图片数据。史莱姆风统一度、第一眼地底生成语义、与传送 / 攻击范围 / 奖励的区分、1× 与拥挤样本可读性及爆亮时刻均保持“待用户人工验收”。
...
- `svg_curve_pear_test.tscn`：按用户提供的第二版梨 SVG 原位替换旧双轮廓来源；净化后的 `assets/svg_curve/pear_source.svg` 保持原 `d` 路径与 `translate(189.226660,627.553395) scale(0.054699,-0.054699)` 坐标变换，只有一个闭合 `<path>`。轻量导入器将它直接转换为一个权威 `Curve2D + Path2D`，固定等距细分同时供内部三角化和边缘生成：内部 `ArrayMesh` 共用 `anchored_star_window.gdshader`，边缘由唯一一个无 Shader 的闭合 `Line2D` 绘制，基础边宽可在 2–30px 内独立调整而不改变内部拓扑。专用控制层默认显示 SVG 的 30 个唯一锚点和 58 个三次 Bézier 控制柄：黄色为锚点，青色为入柄，粉色为出柄，并以同色辅助线连接所属锚点；导入器为闭合复制的第 31 个末点不会重复显示。待机循环让整条矢量同步轻浮动、微倾与呼吸式挤压，轮缘在基础边宽之上做不超过 0.9px 的薄荷色脉冲，背景层附带一个低对比落地影和三颗缓慢环绕微光；所有动画都由同一个可固定的预览时间驱动，不修改 SVG 控制点、曲线拓扑或内部三角形。没有内外双轮廓、弹簧、面积压力、质点或局部曲线形变。`Space` 暂停待机动画，`Q / E` 调细 / 调粗基础边缘，`D` 隐藏 / 显示控制层，`R` 复原，`Esc` 返回索引。`svg_curve_pear_smoke.gd` 检查单闭合路径、唯一 Path2D / Line2D / 控制层、锚点 / 控制柄数量、显示切换隔离、填充面积、材质分层、边宽范围、确定性位移 / 微倾 / 呼吸 / 轮缘脉冲、三颗环境微光和零软体依赖；`capture_svg_curve_pear.gd` 先隐藏控制层探测内部与动态边，再显示控制层分别探测三类点色并生成确定性预览。动画节奏、控制点布局可读性、边宽 / 边色、梨形辨识度和固定空间透视读法仍待用户人工验收。
...
## Underground Spawn Beacon 验证

以下命令均从仓库根目录运行；`$godot` 指向 Godot 4.7.1 stable 可执行文件。截图命令需要正常渲染窗口，其他门禁均可 headless 执行。

```powershell
# 生成轻量场景、正式扫描 Test Lab、显式加载实验场景和索引入口
& $godot --headless --path output/test_lab --script res://scripts/create_underground_spawn_beacon_scene.gd
py -3 tools/godot_bridge.py --project output/test_lab headless-boot
& $godot --headless --path output/test_lab --quit-after 8 res://scenes/underground_spawn_beacon_test.tscn
& $godot --headless --path output/test_lab --quit-after 8 res://scenes/test_lab_index.tscn

# 外部素材 / manifest、时间轴、绝对定位、剩余时长缩放、固定地面锚点、向上运动、节点稳定与源码门禁
& $godot --headless --path output/test_lab --script res://tools/underground_spawn_beacon_smoke.gd

# 生成总览、CHARGE、ERUPTION、BREAKOUT 四张 1280×760 预览
& $godot --resolution 1280x760 --path output/test_lab --script res://tools/capture_underground_spawn_beacon.gd
```

截图只作为机器侧实现证据；“是否读成敌人从地底出现”等观感结论必须由用户人工验收。
