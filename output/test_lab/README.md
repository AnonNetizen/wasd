...
- `polygon_book_test.tscn`：翻开书本 Polygon 素材管线隔离原型。`assets/polygon_art/open_book_source.png` 是由内置 `imagegen` 生成、再用 Godot 归一为纯 `#ff00ff` 键色背景的制作输入；`data/polygon_imports/open_book.json` 逐素材声明源图、Style Profile、输出、锚点、语义区域、重点面和碰撞策略，`data/polygon_art_style.json` 固定 256×256 分析分辨率、3px RDP、24px 内部采样、确定性 seed、70–140 目标面数 / 160 硬上限、4px 最小可见面高度、200px² 最小同色连通区域面积及页面 / 封皮 / 强调色板。`polygon_asset_compiler.gd` 只使用 Godot `BitMap`、`Geometry2D` 与 `Image`：色键分离后用 marching squares + RDP 保留最大轮廓，以轮廓约束三角承载内部采样和确定性扇形细分，过滤退化面并保留最大共边连通面集，再验证每条内部边恰有两个所属面、每条轮廓边恰有一个所属面，然后做色板吸附和语义标记；高度不足 4px 的瘦长三角仍负责无缝填充但继承相邻主体面的颜色，同色共边区域小于 200px² 时再按共享边长度整体并入相邻主色域，本素材共有 28 个面参与小色域合并。当前书本产出 76 面、53 个共享逻辑顶点的 schema v1 JSON，并通过 manifest 将左下最大页面面设为明确暗阶。每个面另存不随并色改变的 `surface_kind`，因此封皮小色岛即使为视觉连续而吸附为页面色，也仍保持固定，不会参与翻页。右侧 `PolygonAsset2D` 完全不读取源 PNG，而是把 76 个原始平色面各展开一次，统一生成一个单层 `ArrayMesh` surface；不复制底页或翻页覆盖层。清理顺序与共享逻辑顶点运动权重保存在 UV，逐面色变标记保存在顶点色 alpha；同一逻辑点的全部渲染副本始终使用相同运动权重，页面与固定书体交界点保持固定，避免变形时露出背景。外轮廓由固定 `Line2D` 单独绘制，调试网格由 draw API 绘制，`Area2D` 使用不随视觉动画变形的简化凸包。翻页动画只使用两类变化：右页内部顶点由一条从书脊扫向外缘的有界折痕波改变位置，可翻页面片按各自相位在页面亮 / 暗 / 暖色之间改变颜色；待机不再移动顶点，避免页面与固定边界形成亚像素细缝。进入场景默认循环演示静止展示、1.2 秒折痕扫页与色变、清理和复原，`A` 关闭 / 重启自动演示；`Space` 手动翻页，`C` 手动清理 / 复原，`M` 切网格、`O` 切源图、`R` 重置、`Esc` 返回索引。`polygon_asset_smoke.gd` 覆盖编译确定性、源哈希、共边连通 / 水密拓扑、共享点变形一致性、最小色域、页面 / 封皮语义隔离、面数 / 色板 / 区域 / 锚点 / 碰撞、瘦长面视觉合并、重点面、无重复面和单 MeshInstance2D / 单 surface / 无纹理运行时约束，`capture_polygon_book_test.gd` 自动生成静态源图对比、翻页中段截图及五阶段运动条带。最终风格、轮廓可读性和动画观感仍为待用户人工验收。
