...
- `slime_room_shooter_3d.tscn`：3D 史莱姆房间射击实验。WASD / 方向键控制史莱姆在房间内移动，鼠标射线投射到 XZ 地面决定朝向，按住鼠标左键使用 24 发轻量对象池连续射击。史莱姆软体参考 `output/steamworks_lab/scripts/slime_body.gd`：`SlimeMembrane/EdgeRig` 用 24 个 `Marker3D` 控制点保存边缘状态，每段经 4 次 centripetal Catmull-Rom 采样形成 96 段连续轮廓，再沿 6 层纬线生成运行时 `ArrayMesh`；每个控制点带弹簧、阻尼、邻点平滑、面积压力、移动惯性和射击方向局部鼓包，玩法移动边界仍由稳定的 `Slime3D` 根节点控制。连续膜使用深色胶体材质和反向膨胀亮边壳，保留脸、瞄准鳍、整体挤压回弹与射击后坐力。`slime_room_shooter_3d_smoke.gd` 覆盖控制点 / 动态网格、移动形变、瞄准方向与池化发射，`capture_slime_room_shooter_3d.gd` 在移动连射状态生成预览截图；`create_slime_room_shooter_3d_scene.gd` 用于重新生成场景并登记入口。
