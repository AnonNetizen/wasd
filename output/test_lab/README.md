...
- `ai_universal_tile_test.tscn`：AI 驱动通用 Tile 场景工作流的独立 PoC，采用“正俯视、手绘轻暗黑童话、废弃大理石温室” Style Pack。`assets/ai_tiles/abandoned_marble_conservatory/` 只保留最终的 128×128 `marble_floor_01.png`、`tree_01.png`、`wood_cabinet_01.png` 与 `style_pack.json`；场景配置在 `data/ai_universal_tile_test.json`，以固定 seed 在 6×4 网格铺满 24 个 Base，并在外围无重叠放置 3 棵树和 3 个木柜。运行时按 Base / Shadow / Object / Detail / Metadata 分层组合，Detail 在 v1 留空；鼠标悬停显示格坐标、tags、footprint、碰撞和交互标志，图层可独立显隐，`R` 使用下一确定性 seed 重新生成，`Esc` 返回索引。PNG 通过 `Image.load()` 创建运行时纹理，不把 `.godot/imported` 当首次预览依赖，也不把 `ImageTexture` / `PackedByteArray` 写入 `.tscn`。三张素材分别使用内置 `imagegen` 生成；树与木柜先置于纯 `#ff00ff` 背景，再做 chroma-key 去底、soft matte / despill、112px 安全框适配和 Lanczos 缩放至 128×128。`style_pack.json` 保存每张素材的精确提示词、工具、修订号与后处理记录。该实验只验证 Style Pack → 原子素材 → manifest → Godot 组合的最小闭环，不代表正式 `client/` 已集成，也不代表批量变体生产、完整编辑器或运行时 AI 已完成。
