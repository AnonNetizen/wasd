...
- `glow_orb_bullet_focus_test.tscn`：独立的 1280×760 渐变圆球子弹专场，不替换既有胶质与材质实验。敌方红球和玩家白球严格共享圆形轮廓，只用阵营主体色、同色十级径向渐变和左上白热高光建立层次；按用户反馈删除黑色 / 深色描边与全部外发光，不使用纹理、Shader 或材质切换。左右展示超大红白弹体，底部继续演示玩家 `r=8 / 520 px/s` 与敌人 `r=5 / 280 px/s` 的实战飞行、无光晕短拖尾和命中。`H / D / T / B / Space / R / Esc` 分别切换超大弹体 / 命中、判定圆、拖尾、三档背景、暂停、重置和返回索引。`glow_orb_bullet_focus_smoke.gd` 检查圆形几何、十级渐变、无描边 / 外发光、红色主体、真实半径 / 速度、主体边界、有界拖尾、无子节点命中和重置清理；`capture_glow_orb_bullet_focus.gd` 固定生成总览图。最终渐变、高光、高速可读性和判定误导仍待用户人工验收。
