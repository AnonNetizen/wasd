...
const SCRIPT_PATH: String = "res://scripts/underground_spawn_beacon_test.gd"


func _initialize() -> void:
	var script: Script = load(SCRIPT_PATH) as Script
	if script == null:
		push_error("Failed to load underground spawn beacon test script: %s" % SCRIPT_PATH)
		quit(1)
		return
	var root_node := Node2D.new()
	root_node.name = "UndergroundSpawnBeaconTest"
	root_node.set_script(script)
	var packed_scene := PackedScene.new()
	var pack_error: Error = packed_scene.pack(root_node)
	if pack_error != OK:
		push_error("Failed to pack underground spawn beacon scene: %s" % pack_error)
		root_node.free()
		quit(pack_error)
		return
	var save_error: Error = ResourceSaver.save(packed_scene, SCENE_PATH, ResourceSaver.FLAG_OMIT_EDITOR_PROPERTIES)
	if save_error != OK:
		push_error("Failed to save underground spawn beacon scene: %s" % save_error)
		root_node.free()
		quit(save_error)
		return
	var normalize_error: Error = _strip_generated_unique_ids()
	if normalize_error != OK:
		push_error("Failed to normalize underground spawn beacon scene: %s" % normalize_error)
		root_node.free()
		quit(normalize_error)
		return
	print("Saved underground spawn beacon scene: %s" % SCENE_PATH)
	root_node.free()
	quit(0)
