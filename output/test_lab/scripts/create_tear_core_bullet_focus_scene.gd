...
const OUTPUT_SCENE_PATH: String = "res://scenes/tear_core_bullet_focus_test.tscn"
const SCENE_SCRIPT_PATH: String = "res://scripts/tear_core_bullet_focus_test.gd"


func _initialize() -> void:
	var scene_script := load(SCENE_SCRIPT_PATH) as Script
	if scene_script == null or not scene_script.can_instantiate():
		push_error("Tear-core bullet focus scene script is not instantiable.")
		quit(1)
		return

	var scene_root := Node2D.new()
	scene_root.name = "TearCoreBulletFocusTest"
	scene_root.set_script(scene_script)

	var packed_scene := PackedScene.new()
	var pack_error: Error = packed_scene.pack(scene_root)
	if pack_error != OK:
		push_error("Failed to pack tear-core bullet focus scene: %s" % pack_error)
		scene_root.free()
		quit(pack_error)
		return
	...
	if save_error != OK:
		push_error("Failed to save tear-core bullet focus scene: %s" % save_error)
		scene_root.free()
		quit(save_error)
		return
	var normalize_error: Error = _strip_generated_unique_ids()
	if normalize_error != OK:
		push_error("Failed to normalize tear-core bullet focus scene: %s" % normalize_error)
		scene_root.free()
