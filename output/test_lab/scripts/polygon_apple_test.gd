...
const AUTO_CYCLE_DURATION: float = 6.0
const AUTO_DISSOLVE_END: float = 3.7
const AUTO_DISSOLVE_START: float = 2.5
const AUTO_GENERATE_END: float = 0.9
const AUTO_HOLD_END: float = 4.3
const AUTO_RESTORE_END: float = 5.2
...
func _ready() -> void:
	...
	if not custom_animation.is_empty():
		...
		_auto_demo_enabled = false
		return
	_polygon_asset.set_motion_strength(0.45)
...
func _build_interface() -> void:
	...
	var subtitle := _make_label(
		"Subtitle",
		"No landmark band · per-asset palette · no semantic animation adapter",
		Vector2(36.0, 58.0),
		Vector2(850.0, 28.0),
		15,
		Color("#918ca8")
	...
	_polygon_asset.position = Vector2(348.0, 260.0)
