...
const OUTPUT_SCENE_PATH: String = "res://scenes/polygon_book_test.tscn"
const SCENE_SCRIPT := preload("res://scripts/polygon_book_test.gd")


func _initialize() -> void:
	var scene_root := Control.new()
	scene_root.name = "PolygonBookTest"
	...
	if pack_error != OK:
		push_error("Failed to pack Polygon book test scene: %s" % error_string(pack_error))
		scene_root.free()
		quit(pack_error)
		return
	...
	scene_root.free()
	if save_error != OK:
		push_error("Failed to save Polygon book test scene: %s" % error_string(save_error))
		quit(save_error)
		return
