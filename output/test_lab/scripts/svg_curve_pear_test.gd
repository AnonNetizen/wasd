...
func _add_header(overlay: CanvasLayer) -> void:
	...
	subtitle.text = "单一 SVG 闭合曲线 · 内部透视填充 · Line2D 自定义边宽"
...
func _add_controls(overlay: CanvasLayer) -> void:
	...
	instruction.text = "整条 SVG 曲线只做平移 · Q / E 直接调节 Line2D 边宽"
	instruction.position = Vector2(170.0, 655.0)
	instruction.size = Vector2(640.0, 28.0)
	...
	controls.position = Vector2(204.0, 698.0)
	controls.size = Vector2(568.0, 48.0)
