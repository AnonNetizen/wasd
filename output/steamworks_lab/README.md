...
- **每 5 分钟一个 boss**：瞄准扇 + 环形弹两种弹幕，血量低于 30% 狂暴加速；每个 boss 都比上一个更强。boss 期间普通刷怪减半，顶部显示 boss 血条。
...
多人沿用 host 权威：进入联机 session 后先停在准备房间，host 至少等到 2 名玩家后手动开始战斗；敌人 / boss / 障碍物 / 伤害 / buff / 时停都在 host 结算，位置与 HP 走 60Hz 快照，开始战斗 / 敌弹齐射 / 开火 / phase / buff / 重开走 reliable RPC。Host 退出 = 全场结束（不支持 host 迁移）。中途加入会收到开始战斗信号并立即从快照对齐当前战况。
...
headless 战斗回归（刷怪 / 受击 / 无敌帧 / GameOver / 重开 / buff 时停 / boss / 障碍物）：
...
2. 两端确认：A / B 都停在准备房间，没有敌人和计时；A 看到玩家数达到 2 后点 `Start Battle`，两端才进入战斗。
3. 两端确认：敌人从顶部同步出现；B 开火能打死敌人（血量在 A 端结算，B 端子弹碰到敌人会视觉消隐）；敌弹两端轨迹一致；障碍物会挡住玩家和敌人；双方血心随受击同步扣减并闪烁。
4. 撑到 30 秒：两端同时冻结（空中子弹悬停、背景停滚），各自弹三选一；A 先选后显示"等待其他玩家选择… (n)"；B 挂机 20 秒验证超时自动选择并恢复，恢复后敌人明显变强。
5. 让 B 死亡：B 变半透明观战、不能开火、不再被打；A 继续打；A 也死 → 双端出结算面板，只有 A 有"再来一局"，按下后两端同步复位再来一轮。
6. B 中途 `Leave Game` 再重新 Join：验证 B 收到开战信号后立即看到当前 tier / 计时 / 在场敌人与障碍物；若加入时正值选 buff，显示等待且不阻塞 A。
7. （可选）把 `battle_director.gd` 的 `BOSS_INTERVAL` 临时调小手测 boss 两端同步与血条，测完改回 `300.0`。
...
- `scripts/steamworks_lab.gd`：主场景、三页 UI、玩家生成、host 权威同步、射击链路、滚动背景与战斗接线。
- `scripts/battle_director.gd`：战斗核心。权威端：刷怪波次 / tier 缩放 / boss / 障碍物调度、圆-圆判伤、buff 状态机与时停；client 端：快照镜像重建、敌弹 volley 视觉、玩家弹视觉消隐。
- `scripts/enemy.gd` / `scripts/enemy_bullet.gd`：三种敌人（直冲 / 炮手 / 掠射）与暖色敌弹。
- `scripts/boss.gd`：每 5 分钟的 boss（瞄准扇 + 环形弹、enrage）。
- `scripts/obstacle.gd`：可打碎的下落岩块（seed 形状 + 按损伤显示裂纹）。
- `scripts/battle_hud.gd`：血心 / 存活时间 / Tier / boss 血条 / 观战角标 / 结算面板。
- `scripts/buff_panel.gd`：三选一强化面板（含等待与超时倒计时）。
...
- `scripts/expression_wheel.gd`：按住 `T` 的表情轮盘。
- `scripts/network_session.gd`：统一 host / join / leave / RPC 同步入口（输入 / 快照 / 射击 / 表情 / phase / buff / volley / 重开）。
