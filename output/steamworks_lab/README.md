...
- 单人模式下，P1 子弹命中敌人 / Boss 获得 1 点大招能量，普通敌人击杀额外获得 5 点、Boss 击杀额外获得 20 点；充满 100 点后按 `E` 召唤 AI 队友。AI 在 10 秒有效战斗时间内按确定性规则自主游击、预判敌弹轨迹，并避让敌弹、普通敌人、Boss 与障碍物，同时继续瞄准敌人和自动开火；常规游击距 P1 使用 210 px 硬限，距离超过 220 px 时触发复位。召唤后必须先松开再按住 `E` 才会发起合体；AI 会高速归队，进入 P1 的 92 px 范围后持续停靠 0.8 秒再自动同意合体。每次召唤最多合体一次，暂停与强化选择会冻结持续时间；大招不进入存档，新局、重开、返回菜单和 Game Over 会清零。AI 仍不可受伤、不吸引火力，网络、存档与正式 `client/` 不变。
...
> ADR #140 自动权威验收已通过：目标 battle 1/1、local-couch 与 `smoke --suite all` 全绿；以下保留为发版前人工复核清单。
...
4. 召唤后保持 `E` 不应误合体；松开再按住 `E`，确认 AI 退出游击并高速归队，进入 P1 的 92 px 范围后仍需连续停靠 0.8 秒才自动同意合体；归队途中不得瞬间跨越或提前合体。合体后 P1 控制移动与主炮、AI 控制副炮；护盾提前破裂后 AI 继续助战但不能再次合体，接近 10 秒时合体则到期立即拆分且不延长持续时间。
...
- `scripts/ai_teammate.gd`：单人满能量召唤的临时 AI 队友，负责确定性自主游击、敌弹预判、敌人 / Boss / 障碍避让、210 px 常规硬限 / 220 px 复位，以及按 `E` 后高速归队至 92 px、停靠 0.8 秒再自动同意合体；不加入真人玩家集合或网络快照。
