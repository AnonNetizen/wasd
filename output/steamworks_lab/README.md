...
- **障碍物**：不定期从上方飘下的岩块，会挡住玩家子弹，可打碎，撞到玩家扣 1 血。
- **死亡观战**：多人下玩家死亡变半透明观战（不能开火、不再被打），全灭才 Game Over；单机自己死了即结束。结算面板显示存活时间 / tier / boss 击破数，只有 host（或单机）有"再来一局"按钮，重开后全场同步复位。

多人沿用 host 权威：敌人 / boss / 障碍物 / 伤害 / buff / 时停都在 host 结算，位置与 HP 走 60Hz 快照，敌弹齐射 / 开火 / phase / buff / 重开走 reliable RPC。Host 退出 = 全场结束（不支持 host 迁移）。中途加入会立即从快照对齐当前战况。
...
2. 两端确认：敌人从顶部同步出现；B 开火能打死敌人（血量在 A 端结算，B 端子弹碰到敌人会视觉消隐）；敌弹两端轨迹一致；双方血心随受击同步扣减并闪烁。
3. 撑到 30 秒：两端同时冻结（空中子弹悬停、背景停滚），各自弹三选一；A 先选后显示"等待其他玩家选择… (n)"；B 挂机 20 秒验证超时自动选择并恢复，恢复后敌人明显变强。
4. 让 B 死亡：B 变半透明观战、不能开火、不再被打；A 继续打；A 也死 → 双端出结算面板，只有 A 有"再来一局"，按下后两端同步复位再来一轮。
5. B 中途 `Leave Game` 再重新 Join：验证 B 立即看到当前 tier / 计时 / 在场敌人与障碍物；若加入时正值选 buff，显示等待且不阻塞 A。
6. （可选）把 `battle_director.gd` 的 `BOSS_INTERVAL` 临时调小手测 boss 两端同步与血条，测完改回 `300.0`。
