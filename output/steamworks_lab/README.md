...
4. 手测主动道具：打怪掉落后任一玩家踩到收进 HUD 的 `Q` 槽；已有道具时新道具替换；按 `Q` 使用后槽位清空。优先验证修复波 / 清场脉冲 / 凝滞场 / 团队过载在两端都影响全队或全场，而不是只改使用者本地状态。
...
- `scripts/battle_hud.gd`：血心 / 主动槽 / 存活时间 / Tier / boss 血条 / 观战角标 / 结算面板。
