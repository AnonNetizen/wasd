...
- 每个玩家是一只软体史莱姆。静止轮廓保持近圆并只做轻微整体呼吸；移动时平滑变为圆润椭圆，不产生尖头或肩点，碰撞和开火继续触发局部软体形变；P1 固定使用键盘与鼠标：WASD / 方向键移动、鼠标瞄准、左键开火、`Q` 主动道具、`E` 单人大招或多人合体、`T` 表情轮、`Esc` 暂停；本地同屏检测到的独立手柄按顺序自动成为 P2–P4，最多 4 人。
- 手柄统一为左摇杆 / D-pad 移动、右摇杆瞄准、RT 按住开火、X 主动道具、A 合体、Y 表情轮、Start 暂停；右摇杆使用 0.25 deadzone，回中后保留最后有效方向。
- 单人模式下，P1 子弹命中敌人 / Boss 获得 1 点大招能量，普通敌人击杀额外获得 5 点、Boss 击杀额外获得 20 点；充满 100 点后按 `E` 召唤 AI 队友。AI 在 10 秒有效战斗时间内按确定性规则自主游击、预判敌弹轨迹，并避让敌弹、普通敌人、Boss 与障碍物，同时继续瞄准敌人和自动开火；常规游击距 P1 使用 210 px 硬限，距离超过 220 px 时触发复位。召唤后必须先松开再按住 `E` 才会发起合体；AI 会以真实 `1.75×` 软体速度追赶移动中的 P1，进入 92 px 范围后贴近 64 px 编队位置并累计 0.8 秒确认，再自动同意合体。HUD 会依次提示松开按键、正在归队和当前合体同步进度。每次召唤最多合体一次，暂停与强化选择会冻结持续时间；大招不进入存档，新局、重开、返回菜单和 Game Over 会清零。AI 仍不可受伤、不吸引火力，网络、存档与正式 `client/` 不变。
...
> ADR #140 及后续合体归队闭环修复已通过自动权威验收：目标 battle 1/1、local-couch 与 `smoke --suite all` 全绿；以下保留为发版前人工复核清单。
...
3. 未满时按 `E` 不召唤；满能量后按 `E`，确认能量清零、AI 进入确定性自主游击并继续瞄准 / 开火。分别制造迎面敌弹、普通敌人、Boss 和下落障碍物威胁，确认 AI 会按预判轨迹侧移 / 脱离，不穿入危险源；相同战场状态下决策应可复现。常规游击与 P1 的距离不得超过 210 px；用测试入口把距离推到 220 px 以上时必须触发复位。
4. 召唤后保持 `E` 不应误合体；HUD 应先提示松开，松开再按住 `E` 后切换为“正在归队”，进入 P1 的 92 px 范围后显示 `0.0 / 0.8` 秒合体同步进度。归队期间继续移动 P1，确认 AI 依靠真实软体移动追上、稳定贴近约 64 px 并完成合体，途中不得瞬移、提前累计或因为 P1 移动而长期追不上；松开 `E` 必须立刻取消归队并清空进度。合体后 P1 控制移动与主炮、AI 控制副炮；护盾提前破裂后 AI 继续助战但不能再次合体，接近 10 秒时合体则到期立即拆分且不延长持续时间。
5. 确认自主游击没有改变既有排除边界：AI 仍不可受伤、不拾取道具、不吸引敌火，也不加入强化选择、Game Over、玩家卡、纪录或网络快照；同屏 / Steam、wire、`user://save.cfg` 与正式 `client/` 保持不变。暂停或进入 30 秒强化选择时倒计时冻结，恢复后继续计时并在累计 10 秒时离场；重开、新局、Game Over 与返回主菜单均清零大招。
...
- `scripts/ai_teammate.gd`：单人满能量召唤的临时 AI 队友，负责确定性自主游击、敌弹预判、敌人 / Boss / 障碍避让、210 px 常规硬限 / 220 px 复位，以及按 `E` 后以真实软体速度归队、近距离跟随移动中的 P1 并累计 0.8 秒确认；不加入真人玩家集合或网络快照。
- `scripts/active_pickup.gd`：可拾取主动道具，host 生成并通过快照镜像到 client。
- `scripts/battle_hud.gd`：单人 / Steam 展开 HUD、仅单人可见的 AI 大招充能 / 召唤 / 倒计时 / 释放门槛 / 归队 / 合体同步提示，以及本地同屏最多四张紧凑玩家卡。
...
- `scripts/slime_player.gd`：玩家实体包装，含外观色板、3 血 / 无敌帧 / 观战 / 快照扩展，以及供 AI 模式倍率真正驱动软体目标距离的 `set_input_drive_scale()`。
