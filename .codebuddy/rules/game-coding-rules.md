...
## 14. 项目索引与导航（AI 友好）
- 维护一份项目导航文件 `AI导航.md`（项目根），告诉 agent：**每类改动该改哪些文件、复用哪些场景、扩展点在哪**。
- 开始任何任务前**优先阅读** `AI导航.md` 与本规则文件，按其指引定位，避免盲目全仓搜索。
- 新增系统/模块/数据类型时，必须同步更新 `AI导航.md` 的对应入口与扩展点说明。

## 15. 词表与契约（消灭裸字符串）
- 所有"约定字符串"必须来自集中白名单 `词表与契约.md`，并在代码中以**常量/枚举**引用，禁止散落裸字符串：
...
- **只能使用白名单中已存在的 id**；需要新 id 时，先在 `词表与契约.md` 登记，再在逻辑层实现对应原语，最后才在数据中使用。
...
- 数据字段含义、单位、取值范围记录在 `/data` 字段说明文档中，与 `词表与契约.md` 配合。
...
## 18. 决策记录、日志与运行说明
- 重要设计/技术决策记入 `决策记录.md`（轻量 ADR）：一句话写清**做了什么 + 为什么**，让后续 agent 不误改既定约束。
...
## 20. 文档维护（元规则）
- 项目文档（`游戏设计文档.md`、`修改建议.md`、`AI导航.md`、`词表与契约.md`、`决策记录.md`、`/data` 字段说明等）必须与实际设计/代码**保持同步**，是与规则文件同等重要的权威来源。
...
- [ ] 约定字符串（stat/effect/event/设置/locale key）是否都来自 `词表与契约.md` 且以常量引用（无裸字符串）？
- [ ] 新增数据是否照「黄金样例」结构填写，并能通过 `DataLoader` 校验？
- [ ] 新代码是否使用类型化 GDScript？是否复用了模板？
- [ ] 是否已更新 `AI导航.md`、`决策记录.md` 等相关文档？
