...
7. 再跑 `python tools/docs_health_check.py`，收集：知识库索引、Markdown 链接、`related_code`、AI 修改说明位置、ADR 矩阵、孤儿文档与同步风险报告。
8. 再跑 `tools/health_metrics.py`（若不存在 → 当前阶段继续做**手动版**）。
9. 手动版当前阶段执行：
   - `修改建议.md` 编号唯一
   - `README.md` ↔ `docs/游戏设计文档.md` 版本号一致
   - 汇总 `tools/docs_health_check.py` 输出的 error 与 report
10. 输出到 `docs/reports/<YYYY-MM-DD>-health.md`，含：
   - 时间 / git commit
   - 各指标当前值 + 目标 + 趋势（如有上次报告）
   - **未达标项的具体位置**（文件 + 行号）
11. 如发现严重不健康（红色项）→ 提示用户立即修，不要堆积
