...
OpenCode 项目级 skills 统一放在 `.opencode/skills/<name>/SKILL.md`，由 `.opencode/opencode.json` 只注册 `.opencode/skills` 路径，并按需通过 skill tool 直接加载。当前已安装：`godot-gdscript`、`godot-scene-validation`、`godot-test-diagnostics`、`playtest-review`、`project-doc-sync`、`safe-git-commit`、`code-review-factual`、`ai-resource-curator`、`mcp-tool-evaluation`。
