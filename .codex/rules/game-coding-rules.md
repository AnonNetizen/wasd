---
description: 俯视角射击刷宝生存游戏项目编码规则，所有代码的编写/修改/生成都必须遵循
alwaysApply: true
---
...
## 12-B. 平衡测试接口预留（框架级）
- 输入解耦：`Player` 与所有可被 AI 替换的角色，输入必须走 `InputService` 的归一化 intent（不直接读 GUIDE、`Input`、`InputMap`、`Input.is_key_pressed` / `Input.is_joy_button_pressed` / 原始 joy axis）。
...
## 18. 决策记录、日志与运行说明
- ADR 只记录重大、长期且跨边界或高代价难回滚的设计 / 技术决策，例如核心玩法方向、跨系统架构与所有权、存档 / 回放 / 数据协议断代、项目级红线或工具链策略。普通功能、局部行为 / UI、bugfix、数值、内容、本地化、测试和实现细节不得为了留痕新增 ADR；按影响同步对应权威文档即可，拿不准时默认不建。
...
- 涉及规则、设计、词表、测试、AI 协作工具、CI、README/CONTRIBUTING 入口的修改，必须检查是否需要同步 `docs/AI导航.md`、`docs/AI记忆/项目记忆.md`、`docs/AI记忆/current_state.json` 与当日会话日志；只有达到重大决策门槛时才同步 `docs/决策记录.md`。
...
- 若代码改变玩家可见行为、架构边界、约定字符串或测试义务，不能只改 `docs/代码/`，还必须按影响同步 GDD / 词表 / 测试策略等对应权威来源；只有达到重大决策门槛时才同步 ADR。
...
- 当需求、术语、验收标准、授权边界或上下文含义不清，且无法从项目权威文档或当前对话中可靠确认时，必须先问一个简短澄清问题；禁止为了推进任务而自行脑补、替用户做高风险假设。只有低风险、可撤销且已明说假设的细节，才可边做边标注假设。
...
## 30. 人工检查专属边界（强制）
- 任务说明、计划、ADR、GDD、工作包、测试策略、checklist 或验收项凡明确标为“人工检查 / 人工验收 / 手动检查 / 手动验收 / L5 / 真实设备验证 / 视觉、听觉、手感、可读性或色弱验收”，或明确写明由用户 / 人工执行，一律视为人工专属项。
- AI 禁止执行人工专属项，禁止用 GUI / 浏览器 / Computer Use、模拟输入、截图自审、录屏分析或自动化脚本替代人工，也禁止把自身观察写成该项“通过 / 失败 / 已完成”。
- AI 必须完成同一任务中所有未标为人工的自动化验证；人工项不会成为跳过 L0~L4、schema、lint、smoke、headless、Replay 或其他机器检查的理由。
- 对人工专属项，AI 只能整理前置条件、可复现步骤、预期现象和已知风险，并明确标记“待人工验收”。人工提供结果或证据后，AI 可以记录、分析和修复问题，但验收结论必须注明来自人工；修复后仍需人工复验的项目继续交回人工。
- 普通的“手动运行命令”不自动等同于人工验收；本规则只约束明确要求人类体验、观察、真实设备或人工签字的检查 / 验收项。
...
- [ ] 玩家描述中可能调整的数值是否来自配置与统一格式化器，而不是在 `*_desc` 译文中重复硬编码？
...
- [ ] 是否识别了所有人工专属检查 / 验收项，只完成自动化验证并交付“待人工验收” checklist，且未用 GUI 自动化、截图自审或自身观察冒充人工结论？
...
- [ ] 是否已按影响更新对应权威文档，并且只在达到重大决策门槛时新增 / 更新 ADR？
