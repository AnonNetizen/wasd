...
> 一款偏《星际战甲》与《暗黑》内核、视角表现参考《哈迪斯》的固定斜俯视 2.5D 刷宝动作生存游戏，玩法仍按 2D 平面移动 / 碰撞 / 刷怪；`Camera2D` 保持屏幕水平和等比缩放，斜俯视观感由量化菱形地图格、舞台地面、障碍物、遮挡层级和低模角色承担，避免横纵屏幕移动速度失真；保留局内随机奖励、构筑选择和生存压力，但不再沿用旧类型定位。
...
## 一句话定位
固定斜俯视 2.5D 表现，WASD / 手柄左摇杆仍在 2D 平面移动，鼠标 / 方向键 / 手柄右摇杆或 D-pad 控制射击方向，**全自动开火**；在开放有限大地图中反复刷装备、词条、遗物、技能资源和局外成长，用掉落追求与构筑迭代驱动长期复刷，在敌群与机关中推进破巢行动。
