...
2. [`AI导航.md`](AI导航.md) —— 项目地图与扩展点速查
3. [`词表与契约.md`](词表与契约.md) —— 约定字符串白名单
4. [`游戏设计文档.md`](游戏设计文档.md) —— 完整设计
5. [`决策记录.md`](决策记录.md) —— 既定决策
6. [`修改建议.md`](修改建议.md) —— 待决策项

> AI agent 工作前请按 `AI导航.md` 定位，避免盲目全仓搜索。
...
### 1. 选定任务类型，按对应扩展点修改
参见 `AI导航.md` 第 4 节「扩展点速查」。常见入口：

| 任务 | 入口 |
|------|------|
| 加敌人 / 遗物 / 道具 | 改 `res://data/*.json`，**不改逻辑** |
| 改数值 | 只改 `res://data/`，**绝不改代码常量** |
| 加面向玩家文本 | 加 `res://locale/strings.csv` 的 key + 译文 |
| 加设置项 | `Settings` 加配置 + UI 控件 |
| 加埋点 | 调用 `Analytics.track_event(name, params)` |
| 加效果原语 | **先在 `词表与契约.md` 登记**，再实现，最后在数据中使用 |
...
- [ ] 新遗物/道具是加数据而非加逻辑分支？
- [ ] 高频实体用了对象池？
- [ ] 约定字符串都来自 `词表与契约.md` 且以常量引用？
- [ ] 新代码使用类型化 GDScript？
- [ ] 已更新 `AI导航.md` / `决策记录.md` 等相关文档？
- [ ] 本次新规则 / 设计变更已同步到对应文档？
...
- **设计建议**：先在 `修改建议.md` 起草（按现有 A~D / J~R 风格编号），由维护者评审后转为决策（写入 `决策记录.md`）或采纳实施。
