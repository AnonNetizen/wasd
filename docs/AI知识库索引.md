...
| 维护 Steamworks Slime Lab / App ID | `output/steamworks_lab/README.md`、ADR #129、`docs/测试策略.md`、`docs/AI导航.md` | `output/steamworks_lab/project.godot`、`steam_appid.txt`、`scripts/transport_adapter.gd`、`scripts/network_session.gd`、`tests/steam_config_smoke.gd`；Lab 与正式 `client/PlatformServices` 边界、必要时 AI 记忆 | `python tools/godot_bridge.py --project output/steamworks_lab headless-boot`；隔离用户目录运行 `steam_config_smoke.gd -- --disable-steam`；真实发布追加 runtime App ID、双账号 overlay / Lobby / invite / cold-start / offline 手动 smoke 与 Depot 内容检查 |
