...
| 维护旧局外成长 / 元进度迁移 | `docs/代码/meta_progression_system.md`、`docs/AI协作/工作包/F6-MetaProgression.md`、`docs/代码/gear_mod_system.md`、GDD §7.2 / §9.16 | `MetaProgressionSystem`、`meta_progression.json`、旧 profile 迁移 / 补偿、SaveManager meta 兼容、`gear_mods.legacy_migration` | 旧系统只作 legacy 迁移；旧 `purchased_upgrades` 已由 GearModSystem 按历史成本补偿为 `gear_mod_dust`；若改迁移逻辑跑 `gear-mod-smoke` / `meta-smoke` / `save-smoke` 和文档健康检查 |
...
| 写/改 F6 旧局外成长闭环 | `docs/AI协作/工作包/F6-MetaProgression.md`、`docs/代码/meta_progression_system.md`、`docs/代码/gear_mod_system.md`、`docs/游戏设计文档.md` §7.2 / §9.16、`docs/代码/save_manager.md`、`client/data/README.md` 的 `meta_progression.json` legacy 段 | `MetaProgressionSystem`、`MetaProgressionPanel`、旧结算流程、`client/data/meta_progression.json`、locale、SaveManager / gameplay runtime 模块文档、AI导航 | Legacy 维护 / 迁移才改；跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/godot_bridge.py --project client meta-smoke`；改 run / SaveManager envelope 时追加 `save-smoke`，改死亡结算接入时追加 `runtime-smoke` |
...
| 写/改 F11 装备 Mod / 局外装配 | `docs/AI协作/工作包/F11-GearModLoadout.md`、`docs/代码/gear_mod_system.md`、GDD §7.2 / §9.16、`client/data/README.md`、`docs/测试策略.md` | `GearModSystem`、`gear_mod_smoke.gd`、Gear Mod UI、Gear Mod HUD 获得提示、旧 purchased_upgrades 成本补偿、`gear_mods.json`、`gear_mod_drop_tables.csv`、`gear_mod_fusion_costs.csv`、SaveManager meta payload、旧 MetaProgression 迁移、locale、词表、AI导航 | 数据 / 契约变化跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/test_data_loader_schema.py`；运行时变化跑 `python tools/godot_bridge.py --project client gear-mod-smoke`；改击杀归因或开局应用追加 `runtime-smoke`；改存档迁移追加 `save-smoke` / `meta-smoke`，影响稳定行为时评估 golden |
...
| #115 | 跨局成长改为参考《星际战甲》的装备 Mod 系统，玩家配置英雄 / 武器两套 Mod；旧 `MetaProgressionSystem` / `meta_progression.json` 进入 legacy 迁移范围 | GDD §7.2 / §9.16、F11 工作包、`docs/代码/gear_mod_system.md`、`docs/代码/meta_progression_system.md`、`client/data/README.md`、测试策略、AI导航、AI记忆 |
| #116 | 旧 `purchased_upgrades` 按历史花费补偿为 `gear_mod_dust`，补偿进度记录在 `meta.gear_mods.legacy_migration.purchased_upgrades_compensation`，旧字段保留但不再作为下一局属性来源 | `docs/代码/gear_mod_system.md`、`docs/代码/meta_progression_system.md`、`docs/代码/save_manager.md`、F11 工作包、GDD §7.2 / §9.16、测试策略、AI导航、AI记忆 |
