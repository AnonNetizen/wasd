...
| 加 / 改 / 绑定视觉效果与 UI 动效 | ADR #158、GDD §9.24、`docs/代码/visual_effects.md`、`docs/代码/ui_effects.md`、词表 §16、数据手册 | 用 `VFX Library` 向导创建组合 `PackedScene` 并登记 `visual_effects.json`；内容通过 `presentation_profiles.json` / `presentation_profile_id` 绑定 cue，Actor 保留标准挂点；UI 只动画 `MotionRoot`。运行时不得引用 editor-only / `output/test_lab`，裸程序几何不得作为成品 | contracts + data/schema + 三档 lint + `vfx-smoke` + `ui-manager-smoke` + actor/settings/loading/runtime/save/L1 + export-tree + headless editor/boot + 四条黄金回放；手动检查 CHARGE/CONTACT/AFTERMATH、25% 与 reduced motion；不得自动运行性能 probe |
...
| 改玩家相机 Rig 归属或绑定 | ADR #148 / #156、GDD §5.2、`docs/代码/phantom_camera.md`、`docs/代码/gameplay_runtime.md`、测试策略 | `client/scenes/gameplay/gameplay_run_loop.tscn` 预置唯一 Rig，`gameplay_run_loop.gd` 在角色创建 / 恢复后绑定；`player_base.tscn` 与角色专属场景不得携带相机。插件内部、震屏数据和 follow 行为未明确变更时不要联动修改 | contracts + data/schema + 三档 lint + `actor-scene-smoke` + settings/runtime + export-tree + headless editor/boot + module-world full/technical + save/L1 + 四条黄金回放；不跑性能 probe |
...
| 加敌人 | `docs/AI协作/任务模板/加敌人.md`、ADR #155、`client/data/README.md`、`client/locale/README.md`、`docs/代码/enemy_ai.md`、`docs/词表与契约.md` §8/9/12、GDD 敌人章节 | `client/data/enemies.csv`、`client/data/enemy_ai_profiles.json`、专属继承场景、`client/locale/strings.csv`、`game_modes` 敌人池、必要时 AI action / content tag 词表；不同敌人可复用场景但必须独立池 | `validate_data` + DataLoader schema + `actor-scene-smoke` + runtime；场景 / 池契约变化按测试策略追加完整回归 |
...
| #156 | 玩家摄像机 Rig 从 `PlayerBase` 解耦为 `GameplayRunLoop/ActiveWorld` 的唯一对局级场景节点；角色基础 / 专属场景不再携带相机，RunLoop 在默认角色创建和 run v4 续局角色重建后把 GLUED `PlayerCamera.follow_target` 绑定到当前 Player。缺失 / 错误 Rig fail closed；严格居中、震屏、priority/layer、插件和存档契约不变，仅取代 #148 的场景归属部分 | GDD §5.2、Phantom Camera / Gameplay Runtime 文档、测试策略、AI导航、知识索引、三层项目记忆、`actor-scene-smoke` 与 `runtime-smoke` |
