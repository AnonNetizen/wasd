...
| 改地图 / 菱形格 / PCG / 人工摆点 | `docs/代码/map_manager.md`、`client/data/README.md` 的 `map_layouts.json` 段、GDD §5、ADR #93 / #105 / #106 / #107 | `client/data/map_layouts.json`、`client/scripts/gameplay/map_manager.gd`、`client/tools/runtime_smoke.gd`、必要时 `docs/代码/hazard_system.md`；bounds 需为 `grid.cell_width/cell_height` 的奇数倍，出生安全区视觉必须贴格，机关摆点按 `radius_tiles` 奇偶吸附到合法锚点 | `python tools/validate_data.py`、`python tools/test_data_loader_schema.py`、`python tools/godot_bridge.py --project client runtime-smoke`；机关相关追加 `f9-demo-smoke` / `save-smoke` / `perf-probe` |
| 加 / 改机关 | `docs/代码/hazard_system.md`、`client/data/README.md`、`client/locale/README.md`、`docs/词表与契约.md` §8/9/12、`docs/AI导航.md` | `client/data/hazards.csv`、`client/data/map_layouts.json`、`client/locale/strings.csv`、`game_modes` 机关池、必要时地图 / 机关 primitive；当前通用机关是菱形危险地块，`radius_tiles` 为占用地图菱形格的整数倍 | `python tools/sync_contracts.py --check`；`python tools/validate_data.py`；DataLoader schema 变化跑 `python tools/test_data_loader_schema.py`；运行时机关变化跑 `runtime-smoke` / `f9-demo-smoke`；文档变化跑 `python tools/docs_health_check.py` |
...
| 维护旧局外成长 / 元进度迁移 | `docs/代码/gear_mod_system.md`、`docs/代码/save_manager.md`、`docs/AI协作/工作包/F6-MetaProgression.md`、GDD §7.2 / §9.16 | `meta_progression.json` legacy 成本表、旧 profile 迁移 / 补偿、SaveManager meta 兼容、`gear_mods.legacy_migration` | 旧运行时和 UI 已按 ADR #117 删除；旧 `purchased_upgrades` 已由 GearModSystem 按历史成本补偿为 `gear_mod_dust`；若改迁移逻辑跑 `gear-mod-smoke` / `save-smoke` 和文档健康检查 |
...
| 查 F6 旧局外成长历史 / 改迁移补偿 | `docs/AI协作/工作包/F6-MetaProgression.md`、`docs/代码/gear_mod_system.md`、`docs/游戏设计文档.md` §7.2 / §9.16、`docs/代码/save_manager.md`、`client/data/README.md` 的 `meta_progression.json` legacy 段 | `client/data/meta_progression.json`、旧 `purchased_upgrades` 补偿、SaveManager / gameplay runtime 模块文档、AI导航 | 旧运行时 / UI / smoke 已删除；迁移补偿变化跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/godot_bridge.py --project client gear-mod-smoke`；改 run / SaveManager envelope 时追加 `save-smoke` |
...
| 写/改 F11 装备 Mod / 局外装配 | `docs/AI协作/工作包/F11-GearModLoadout.md`、`docs/代码/gear_mod_system.md`、GDD §7.2 / §9.16、`client/data/README.md`、`docs/测试策略.md` | `GearModSystem`、`gear_mod_smoke.gd`、Gear Mod UI、Gear Mod HUD 获得提示、旧 purchased_upgrades 成本补偿、`gear_mods.json`、`gear_mod_drop_tables.csv`、`gear_mod_fusion_costs.csv`、SaveManager meta payload、legacy `meta_progression.json` 成本表、locale、词表、AI导航 | 数据 / 契约变化跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/test_data_loader_schema.py`；运行时变化跑 `python tools/godot_bridge.py --project client gear-mod-smoke`；改击杀归因或开局应用追加 `runtime-smoke`；改存档迁移追加 `save-smoke`，影响稳定行为时评估 golden |
...
| #98 | 类型定位改为刷宝动作生存；参考星际战甲 / 暗黑的装备、词条、长期养成与刷战利品循环（当前视角、机关、地图格和资产口径已由 #100~#107 更新为固定斜俯视 2.5D） | README、GDD、IP设定、术语表、AI导航、CICD规划、功能建议池、AI记忆、平台规则 / skills |
...
| #101 | 修正固定斜俯视相机实现：`CenteredCamera` 不滚转屏幕，保持玩家居中和屏幕水平；后续非等比缩放边界由 #102 修正 | README、GDD、术语表、AI导航、Gameplay runtime 文档、`client/scripts/gameplay/player.gd`、`client/scripts/gameplay/player_3d_visual.gd`、`client/tools/runtime_smoke.gd`、AI记忆 |
| #102 | 固定斜俯视显示改为哈迪斯式等比屏幕尺度：`Camera2D` 不旋转、不非等比缩放，世界横纵单位映射到屏幕保持同尺度；斜俯视感由舞台地面、障碍物、Y-sort / 遮挡和 2.5D 视觉层承担 | README、GDD、规则入口、术语表、AI导航、Gameplay runtime 文档、`client/scripts/gameplay/player.gd`、`client/scripts/gameplay/world_background.gd`、`client/tools/runtime_smoke.gd`、AI记忆 |
| #103 | 通用机关改为哈迪斯式菱形危险地块；`Hazard` 占位表现和触发判定都使用菱形范围。字段口径已由 #105 从 `hazards.csv.radius` 更新为 `radius_tiles` | GDD、`client/data/README.md`、AI导航、Gameplay runtime 文档、HazardSystem 文档、`client/scripts/gameplay/hazard.gd`、AI记忆 |
| #104 | 固定斜俯视资产处理规则：地面范围读菱形，非地面资产保留自由轮廓但用落地点 / 阴影 / 遮挡 / 排序统一，AI 资产 brief 必须说明 footprint、anchor、shadow、sort layer 和真实判定形状 | GDD §8.2-A、AI导航、F9 工作包、Gameplay runtime 文档、术语表、AI记忆 |
| #105 | 地图采用量化菱形格度量；`map_layouts.json.grid.cell_width/cell_height` 定义格尺寸，bounds 是格尺寸奇数倍，出生点 / 人工机关 / PCG 机关中心吸附到格点，机关尺寸改用 `hazards.csv.radius_tiles` 正整数；机关锚点奇偶口径由 #106 修正 | GDD §5 / §8.2-A、`client/data/README.md`、MapManager / HazardSystem / Gameplay runtime 文档、`client/scripts/gameplay/map_manager.gd`、`client/scripts/gameplay/hazard.gd`、`client/scripts/gameplay/world_background.gd`、AI记忆 |
| #106 | 机关锚点按 `radius_tiles` 奇偶吸附；奇数尺寸中心落在菱形格心，偶数尺寸中心落在网格顶点，人工摆点和 PCG 共用同一规则 | GDD §5、`client/data/README.md`、MapManager / HazardSystem 文档、`client/scripts/gameplay/map_manager.gd`、`client/scripts/autoload/data_loader.gd`、`tools/validate_data.py`、`client/tools/runtime_smoke.gd`、AI记忆 |
...
| #115 | 跨局成长改为参考《星际战甲》的装备 Mod 系统，玩家配置英雄 / 武器两套 Mod；旧 `MetaProgressionSystem` / `meta_progression.json` 进入 legacy 迁移范围 | GDD §7.2 / §9.16、F11 工作包、`docs/代码/gear_mod_system.md`、`client/data/README.md`、测试策略、AI导航、AI记忆 |
| #116 | 旧 `purchased_upgrades` 按历史花费补偿为 `gear_mod_dust`，补偿进度记录在 `meta.gear_mods.legacy_migration.purchased_upgrades_compensation`，旧字段保留但不再作为下一局属性来源 | `docs/代码/gear_mod_system.md`、`docs/代码/save_manager.md`、F11 工作包、GDD §7.2 / §9.16、测试策略、AI导航、AI记忆 |
| #117 | 旧局外成长运行时退役；删除旧 autoload / UI / 死亡结算 / `meta-smoke`，保留 `meta_progression.json` 作为旧 purchased_upgrades 成本补偿表 | `docs/代码/gear_mod_system.md`、`docs/代码/save_manager.md`、F11 工作包、GDD §7.2 / §9.16、测试策略、AI导航、AI记忆 |
