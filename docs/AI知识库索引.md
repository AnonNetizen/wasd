...
| 加 / 改状态效果 | `docs/代码/status_effect_component.md`、`docs/代码/skill_system.md`、GDD §9.15.2、`docs/词表与契约.md` §9-A~§9-B / §12-F~§12-G、`docs/测试策略.md` | `client/scripts/combat/status_effect.gd`、`client/scripts/combat/status_effect_component.gd`、`client/scripts/gameplay/skill_system.gd`、`client/data/skills.json`、DataLoader schema、必要时 run 快照 | `python tools/sync_contracts.py --check`；`python tools/validate_data.py`；`python tools/test_data_loader_schema.py`；`python tools/godot_bridge.py --project client l1-smoke`；影响续局时追加 `save-smoke`；影响整局时评估 golden |
