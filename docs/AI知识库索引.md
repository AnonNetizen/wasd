...
| 启动 / 推进正式项目 | 当前阶段工作包；F13 模块大地图读 `docs/AI协作/工作包/F13-ModularGridWorld.md`、`docs/代码/module_world_manager.md`、GDD §5、`client/data/README.md` 与 `docs/测试策略.md`；旧 `F13-HandcraftedRooms.md` 只作 ADR #127/#128 历史；F12 开放战区读 `docs/AI协作/工作包/F12-ShortLootRuns.md` 与 `docs/代码/gameplay_runtime.md`，现仅作非默认回归；其余阶段按对应 F11/F10/F9/F8/F7/F6/F4/F3 工作包进入 | `client/`、模块文档、必要时 TODO / GDD / ADR / 词表 / 测试策略 | 按工作包验收命令运行；模块世界至少跑 `sync_contracts --check`、`validate_data`、schema test、`module-world-smoke`、`save-smoke`；项目规则变化跑 project lint；文档变化跑 docs health |
...
| 改 EnemyAI 对玩家行为 | `docs/代码/enemy_ai.md`、GDD §5.3、`client/data/README.md` 的 `enemy_ai_profiles.json` 段、`docs/词表与契约.md` §12-B、ADR #144 | `client/scripts/gameplay/enemy.gd`、`client/data/enemy_ai_profiles.json`、`client/data/enemies.csv`、`client/tools/runtime_smoke.gd`、必要时 golden replay | `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/test_data_loader_schema.py`、`python tools/godot_bridge.py --project client runtime-smoke`；必须保持玩家唯一战斗目标、敌方友伤拒绝和无伤害中心分离；改变稳定行为时重录 / 重跑四条 golden replay；性能测试仅在用户明确要求时运行 |
...
| Gameplay Runtime 模块文档 | `docs/代码/gameplay_runtime.md` / `docs/代码/combat.md` / `docs/代码/skill_system.md` / `docs/代码/status_effect_component.md` / `docs/代码/map_manager.md` / `docs/代码/module_world_manager.md` / `docs/代码/hazard_system.md` / `docs/代码/warzone_director.md` | 展示最小可玩闭环、统一伤害入口、可复用主动技能、状态效果生命周期、F13 模块大地图与流式邻域、开放战区回归、对象池实体、HUD 与验证方式 |
| EnemyAI 模块文档 | `docs/代码/enemy_ai.md` | 展示对玩家行为 profile、Utility/FSM/Steering 分工、敌方友伤护栏、中心分离与旧快照兼容方式 |
| ADR #144 黄金回放报告 | `docs/reports/2026-07-21-replay-regression.md` | 记录四条 golden 的重录数据指纹、逐条真实运行时复跑结果和未运行性能测试的边界 |
