...
| #154 | 模块 JSON schema v2 恢复为布局与表现唯一制作主源，Godot Dock 只可视化编辑 JSON；baker 严格单向生成含三层 TileMap、合并碰撞和四边封锁的旋转 TSCN，check 重建规范指纹且不信任 TSCN metadata；运行时预加载 assignment 唯一生成场景并复用九 chunk。gameplay approval hash 与完整 bake hash 分离，schema v1 等价 map hash、run v4、导航、存档和回放不变；取代 #153 的模块制作源部分 | Module Authoring Pipeline / ModuleWorldManager 文档、F13 工作包、GDD、数据手册、测试策略、AI导航、current_state、AI记忆、2026-07-23 回放报告 |
