...
| 维护 Steamworks Slime Lab / 本地同屏 / App ID | `output/steamworks_lab/README.md`、ADR #129 / #135、`docs/测试策略.md`、`docs/AI导航.md` | `scripts/local_input_router.gd`、`scripts/steamworks_lab.gd`、`project.godot`、`steam_appid.txt`、`scripts/transport_adapter.gd`、`scripts/network_session.gd`、`tests/local_couch_smoke.gd`、`tests/steam_config_smoke.gd`；Lab 与正式 `client/PlatformServices` 边界、必要时 AI 记忆 | `local_couch_smoke.gd -- --disable-steam`、`battle_smoke.gd -- --disable-steam`、Lab headless boot；transport 变更追加内部 ENet host/client；真实发布追加 1–3 手柄同屏、runtime App ID、双账号 overlay / Lobby / invite / cold-start / offline 与 Depot 内容检查 |
