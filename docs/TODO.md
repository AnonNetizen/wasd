...
- [ ] 继续 F8 回放 / 测试 / 平衡基线：已完成 L1 smoke、Replay runner、gameplay 输入录制首片、runner 输入播放首片、首条运行时摘要 golden 和 perf probe；下一步推进帧级 diff 与更多黄金回放。
...
- [ ] 扩展黄金回放：`golden_basic_run` 已有运行时摘要版，runner 已有输入播放首片；后续补帧级版 `golden_basic_run`、`golden_pause_resume`、`golden_full_death`、`golden_relic_synergy`。
