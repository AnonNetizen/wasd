...
- [ ] 继续 F8 回放 / 测试 / 平衡基线：已完成 L1 smoke、Replay runner、gameplay 输入录制首片、runner 输入播放首片、runtime event 播放首片、basic / pause-resume / full-death 三条运行时摘要 + 稳定帧样本 golden 和 perf probe；下一步推进更完整的逐帧字段。
...
- [ ] 扩展黄金回放：`golden_basic_run`、`golden_pause_resume` 和 `golden_full_death` 已有运行时摘要 + 稳定帧样本版，runner 已有输入播放与 runtime event 播放首片；后续补更完整的 `golden_basic_run` 帧字段与 `golden_relic_synergy`。
