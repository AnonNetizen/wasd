...
- [ ] 继续正式项目 F3 数据 / 契约闭环：在已补 DataLoader schema 回归测试、weapons、relics 和 enemies 边界后，继续扩展 hazards / spawn_waves / active_items 等后续内容数据 schema。

## 2. 下一批任务（P1）

- [ ] 完成正式项目 F3 数据 / 契约闭环：把已落地 schema 继续扩展到机关、刷怪波次和道具等后续内容数据。
