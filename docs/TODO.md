...
- [ ] 扩展黄金回放：`golden_basic_run`、`golden_pause_resume` 和 `golden_full_death` 已有运行时摘要 + 扩展稳定帧样本版，runner 已有输入播放与 runtime event 播放首片；后续在遗物运行时 / 协同原语存在后补 `golden_relic_synergy` 等更多场景。
