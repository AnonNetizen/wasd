...
- 本地与 CI 统一调用 `python tools/godot_bridge.py --project client gut`。Bridge 默认运行 unit + integration，在临时项目快照和临时 `APPDATA` / XDG / `HOME` 下隔离 `.godot`、`user://` 与 JUnit，并在同一快照内先阻塞完成 Godot import，再启动 GUT；import 或测试出现 GUT / Godot fatal 与 GDScript 解析 / 加载错误时拒绝，并要求 JUnit 文件存在、可解析且 `tests > 0 / failures = 0 / errors = 0`，不能把 Godot / GUT 退出码 0 单独当成通过。CI 仍可按 workflow 顺序串行，本地多个只读 Bridge 命令则可安全并行
