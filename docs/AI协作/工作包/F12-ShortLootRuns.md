...
- **当前已落地首片**：默认局已改为偏外侧投放、0-1 / 1-4 / 4-7 / 7-9 / 9+ 分钟导演阶段、四个 director 兴趣点、可暂存 dust / Gear Mod 奖励、小巢核完成面板，以及可被子弹摧毁的 Mod 缓存 / 精英巢点 / 小巢核目标；可伤害目标已改为立即受伤、贴合地图菱形格，并与关联机关分离摆放；击破小巢核会开启贴格撤离区，站进撤离区完成短读条才结算暂存战利品，死亡 / 放弃会丢失；结果面板首片已列出成功带回或失败丢失的 dust / Gear Mod、击杀数和用时；真实宝箱交互、正式核心实体行为 / 美术、多出口撤离和更正式的 Result UI 待做。
