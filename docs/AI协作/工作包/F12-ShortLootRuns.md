...
→ 手动拾取无等级 Mod 实例；在 7×7 棋盘确认合法位置后生效
→ 清理意识核
→ 立即显示本局结果与最终构筑
→ 下一局重新空 Mod 开始
```
...
缓存仍通过可见箱体和 `interact` action 打开；所有 Gear Mod 奖励都先生成带 `instance_id` 的池化 CPU 拾取实体。再次交互只预占最近一件并打开非暂停棋盘，确认合法四邻坐标后才原子放置 / 应用 / 回收；取消留地。
