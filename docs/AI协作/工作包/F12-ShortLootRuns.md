...
- **当前已落地首片**：默认局已改为偏外侧投放、0-1 / 1-4 / 4-7 / 7-9 / 9+ 分钟导演阶段、四个 director 兴趣点和 7 分钟小巢核压力；兴趣点目前仍是初始机关占位，奖励 / 核心实体 / 结算 UI 尚未接线。
...
| Gear Mod 掉落 | `client/data/gear_mod_drop_tables.csv`、`GearModSystem` | 精英 / 小巢核后续应接更高价值掉落 |
| 结算 / 撤离 | `GameplayRunLoop`、`GameOverPanel` 或后续 Result UI | 首片可先简化，后续补正式结算 |
...
3. 做 3 个首片兴趣点：精英巢点、Mod 缓存、资源缓存。（已做 director 机关占位；奖励交互待做）
4. 做小巢核终点：击破后给高价值结算或返回局外。（已做 7 分钟压力窗口和小巢核占位；核心实体 / 结算待做）
5. 调整掉落：普通怪少量，精英 / 兴趣点 / 小巢核显著更高。
6. 加最小结算反馈：展示本局获得的 Mod / dust / 击杀 / 用时。
