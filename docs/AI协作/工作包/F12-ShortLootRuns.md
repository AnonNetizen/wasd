...
- **战利品需要成功带回**：ADR #122 后默认标准局不再“拿到即永久入账”；Gear Mod / dust 等奖励先进入 `run.pending_loot`，击破小巢核或未来撤离成功才写入 `meta.gear_mods`，死亡 / 放弃 / 重开 / 回标题会丢失未结算战利品，保存退出只保存当前 run 继续。
- **当前已落地首片**：默认局已改为偏外侧投放、0-1 / 1-4 / 4-7 / 7-9 / 9+ 分钟导演阶段、四个 director 兴趣点、可暂存 dust / Gear Mod 奖励、小巢核完成面板，以及可被子弹摧毁的 Mod 缓存 / 精英巢点 / 小巢核目标；可伤害目标已改为立即受伤、贴合地图菱形格，并与关联机关分离摆放；击破小巢核会结算暂存战利品，死亡 / 放弃会丢失；真实宝箱交互、正式核心实体行为 / 美术、撤离和完整结算清单待做。
...
| 9:00-12:00 | 撤离 / 加压 | 结算、撤离，或继续贪一个高危点 | 高压尾声，不鼓励无限拖局；未撤离 / 未完成则不带回暂存战利品 |

首版可以先不做撤离交互；击破小巢核后直接进入结算 / 回标题也可接受。关键是每 2-3 分钟至少有一次明确奖励节点，但奖励在成功带回前只算本局暂存。
...
- 地面范围、机关和巢点 footprint 继续使用量化菱形格；可伤害兴趣点目标必须有独立格心 anchor，不能和机关 / 陷阱重合；非地面资产用落地点 / 阴影 / 遮挡表达。
...
| 结算 / 撤离 | `GameplayRunLoop`、`GameOverPanel` 或后续 Result UI | 小巢核已能结束本局、提交暂存战利品并显示完成面板；后续补撤离和更完整结算清单 |
...
3. 做 3 个首片兴趣点：精英巢点、Mod 缓存、资源缓存。（已做 director 机关占位；资源缓存可进圈领取并进入暂存，精英巢点 / Mod 缓存已有可伤害目标和首版暂存奖励，目标立即可被子弹打且贴合地图格，真实守卫 / 宝箱交互待做）
4. 做小巢核终点：击破后给高价值结算或返回局外。（已做 7 分钟压力窗口、小巢核可伤害目标、保底 dust / Mod 暂存奖励、完成面板和成功结算入账，目标与陷阱分离摆放；正式核心行为 / 撤离 / 完整结算待做）
