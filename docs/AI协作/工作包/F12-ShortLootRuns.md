...
| Mod 缓存 | 打开守卫箱 / 击破缓存 | Gear Mod 或高权重掉落机会 | 当前首片已生成可见缓存箱，进半径后按 `interact` 打开并暂存 Gear Mod；正式箱体美术、守卫清空条件和掉落爆出表现后续补 |
| 资源缓存 | 低风险补给点 | `gear_mod_dust`、治疗或局内补给 | 当前首片已生成可见缓存箱，进半径后按 `interact` 打开并暂存 dust；治疗、商店或武器缓存后续补 |
...
3. 做 3 个首片兴趣点：精英巢点、Mod 缓存、资源缓存。（已做 director 机关占位；资源缓存 / Mod 缓存已有可见缓存箱和 `interact` 打开流程并进入暂存，精英巢点已有可伤害目标和首版暂存奖励，目标立即可被子弹打且贴合地图格；真实守卫、正式箱体美术和爆出表现待做）
