...
- **当前已落地首片**：默认局已改为偏外侧投放、0-1 / 1-4 / 4-7 / 7-9 / 9+ 分钟导演阶段、四个 director 兴趣点、可暂存 dust / Gear Mod 奖励、小巢核完成面板，以及可被子弹摧毁的精英巢点 / 小巢核目标；资源缓存 / Mod 缓存已改为可见缓存箱，玩家进入领取半径后按 `interact` 打开，不再被子弹摧毁或进圈自动领取；缓存箱已改为独立格心 anchor、菱形 glyph 视觉，并与关联机关 / 陷阱分离摆放；可伤害目标已改为立即受伤、贴合地图菱形格，并与关联机关分离摆放；击破小巢核会开启贴格撤离区，站进撤离区完成短读条才结算暂存战利品，死亡 / 放弃会丢失；结果面板首片已列出成功带回或失败丢失的 dust / Gear Mod、击杀数和用时；正式核心实体行为 / 美术、多出口撤离、更正式的 Result UI、缓存箱正式美术 / 守卫 / 爆出表现待做。
...
当前首版已接最小撤离交互：击破小巢核后开启一个贴合菱形地图格的撤离区，玩家站入并保持短读条后才结算。关键是每 2-3 分钟至少有一次明确奖励节点，但奖励在撤离成功前只算本局暂存。
...
- 地面范围、机关和巢点 footprint 继续使用量化菱形格；可伤害兴趣点目标与交互缓存箱必须有独立格心 anchor，不能和机关 / 陷阱重合；非地面资产用落地点 / 阴影 / 遮挡表达。
...
| Mod 缓存 | 打开守卫箱 / 击破缓存 | Gear Mod 或高权重掉落机会 | 当前首片已生成独立格心可见缓存箱，菱形 glyph 视觉贴合地图格，进半径后按 `interact` 打开并暂存 Gear Mod，关联机关不会压到箱体 footprint；正式箱体美术、守卫清空条件和掉落爆出表现后续补 |
| 资源缓存 | 低风险补给点 | `gear_mod_dust`、治疗或局内补给 | 当前首片已生成独立格心可见缓存箱，菱形 glyph 视觉贴合地图格，进半径后按 `interact` 打开并暂存 dust，关联机关不会压到箱体 footprint；治疗、商店或武器缓存后续补 |
...
3. 做 3 个首片兴趣点：精英巢点、Mod 缓存、资源缓存。（已做 director 机关占位；资源缓存 / Mod 缓存已有独立格心可见缓存箱、菱形 glyph 视觉、避开陷阱 footprint 和 `interact` 打开流程并进入暂存，精英巢点已有可伤害目标和首版暂存奖励，目标立即可被子弹打且贴合地图格；真实守卫、正式箱体美术和爆出表现待做）
...
- 复杂迷宫 / 房间制地图。
- 默认模式局内 3 选 1。
- 多出口撤离 UI、地图大地图标记系统、缓存箱正式美术 / 守卫 / 爆出表现。
- 隐藏 DDA 或读取玩家状态动态调难。
