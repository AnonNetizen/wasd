...
`FormalClientBoot` 支持 `--room-carrier` 用户参数：开窗直接进入一个**可玩**的房间 carrier 局（不挂自动 smoke），重开 / 新局也保持房间 carrier（`_start_new_gameplay_run` 透传 `_room_carrier_launch`）。这是调试 / 试玩入口，**opt-in、不改默认 open-warzone 流程**。运行：

```powershell
& "<godot.exe>" --path client -- --room-carrier
```

操作：WASD 移动、鼠标瞄准、按住左键开火；清掉房间 1 敌人后东侧出口门解锁，走进门切到房间 2（1 敌 + 1 陷阱），再清房走到东侧完成门即结算；`Esc` 暂停，可 Save & Quit 回标题再 Continue 验证续局。把房间设为默认可玩模式仍是后续切片。
