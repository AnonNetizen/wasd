...
- **节奏导演**：用固定阶段组织暖场、加压、路线压力和中段尖峰。
- **巢变异导演**：每局绑定一个可诊断的巢主题 / mutation，用于组合阶段、兴趣点和后续奖励语义。
- **地图兴趣点 / 机关导演**：在数据中声明兴趣点和机关组合，由 `WarzoneDirector` 过滤当前 layout 后交给 `MapManager` 生成初始 `source="director"` 机关 placement。
- **wave gating**：阶段只声明当前允许的 `spawn_waves.csv` wave；敌人组合归波次或 F13 模块 JSON 管理，不再维护独立的导演敌人组合元数据。
...
1. `client/data/warzone_directors.json` schema v2 声明标准模式导演、固定 mutation、阶段时间窗、阶段启用 wave 和兴趣点。
...
5. `DataLoader` 和 `tools/validate_data.py` fail-fast 校验导演 schema、模式引用、wave 引用、hazard 引用和 map layout 引用，并拒绝 schema v1 的已删除导演敌人组合字段。
...
| 字段 | 规则 |
|------|------|
| `schema_version` | 必须为 `2`；旧导演敌人组合字段必须 fail-fast |
...
- 新增兴趣点先做通用 schema 和校验，再接生成逻辑；不得恢复已删除的导演敌人组合元数据。
