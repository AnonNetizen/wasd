...
- **地图兴趣点 / 机关导演**：先在数据中声明兴趣点和机关组合，后续再把它接到 `MapManager` 的生成规则。
...
| `interest_points[].hazard_ids[]` | 非空时必须存在于 `hazards.csv` |
...
若只改文档，可按文档维护指南缩小为 `python tools/docs_health_check.py`。若导演开始影响地图生成、run snapshot、随机 mutation、replay summary 或 golden fingerprint，追加对应 smoke / replay runner，并在 ADR 和本工作包写明原因。
