...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。当前还可对 `golden_basic_run.replay`、`golden_pause_resume.replay` 与 `golden_full_death.replay` 重跑真实运行时摘要与稳定帧样本，已录制首批 gameplay 输入事件，并能在 runner 内播放输入事件和工具层 runtime 事件；后续再扩全量帧级字段和更多场景。
- 至少 `golden_basic_run`、`golden_pause_resume`、`golden_full_death` 中的一条进入版本库并能稳定通过；当前三条均已入库并通过 runner runtime rerun，更多输入场景和更完整帧字段仍待补。
