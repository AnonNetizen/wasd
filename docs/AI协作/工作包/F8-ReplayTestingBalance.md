...
4. **Replay 重放 runner 首片**：已建立 `python tools/godot_bridge.py --project client replay-runner`，当前先读取 `.replay` envelope、校验 `recording_hash` / data fingerprint，并比较稳定 summary；失败时输出首个字段级 diff。它是 L3 runner 外壳，不声称已完成输入回放 / 帧级复现，后续再把固定 seed 短局和帧级字段逐步接入。
...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。后续再把“摘要对照”收紧为真实输入重放和帧级 diff。
