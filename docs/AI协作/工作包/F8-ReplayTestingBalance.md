...
4. **Replay 重放 runner 首片**：已建立 `python tools/godot_bridge.py --project client replay-runner`，当前先读取 `.replay` envelope、校验 `recording_hash` / data fingerprint，并比较稳定 summary；失败时输出首个字段级 diff。带 `--rerun-runtime-summary` 时可按 replay seed 启动真实 `GameplayRunLoop`、按 tick 播放 `input_events` 并比较 `run_summary`；未传 replay 文件时会生成临时输入播放 smoke replay。它仍是 L3 runner 外壳，不声称已完成帧级复现。
5. **Gameplay 输入录制首片**：已建立 `python tools/godot_bridge.py --project client replay-input-smoke`，当前启动真实 `GameplayRunLoop`，确认移动 / 瞄准状态变化和 `pause` / `ui_back` 离散输入能进入 `Replay.input_events`。它只证明录制路径已接入；输入播放首片由 `replay-runner --rerun-runtime-summary` 覆盖，帧级复现仍未完成。
6. **首批黄金回放**：已入库 `client/tests/replays/golden_basic_run.replay`，由 `python tools/godot_bridge.py --project client capture-golden-replay` 通过真实 `GameplayRunLoop` 生成，当前覆盖固定 seed 180 帧的运行时摘要，并包含玩家是否右移与朝向摘要字段。`golden_pause_resume`、`golden_full_death` 和后续帧级版 `golden_basic_run` 仍待帧级对照能力落地；`golden_relic_synergy` 等遗物运行时 / 协同原语落地后再录，不为空造假。
...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。当前还可对 `golden_basic_run.replay` 重跑真实运行时摘要，已录制首批 gameplay 输入事件，并能在 runner 内播放输入事件；后续再把“摘要对照 + 输入播放”收紧为帧级 diff。
- 至少 `golden_basic_run`、`golden_pause_resume`、`golden_full_death` 中的一条进入版本库并能稳定通过；当前 `golden_basic_run.replay` 已满足运行时摘要 golden，未完成项原因是帧级 diff 尚未落地，更多输入场景 golden 仍待补。
