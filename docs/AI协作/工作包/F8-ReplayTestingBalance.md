...
5. **首批黄金回放**：已入库 `client/tests/replays/golden_basic_run.replay`，由 `python tools/godot_bridge.py --project client capture-golden-replay` 通过真实 `GameplayRunLoop` 生成，当前覆盖固定 seed 180 帧的运行时摘要。`golden_pause_resume`、`golden_full_death` 和后续帧级版 `golden_basic_run` 仍待输入回放 / 对照能力落地；`golden_relic_synergy` 等遗物运行时 / 协同原语落地后再录，不为空造假。
6. **Perf / 平衡采样首片**：已建立 `python tools/godot_bridge.py --project client perf-probe`，输出运行时长、帧时间、敌人 / 子弹 / pickup 池水位、击杀、等级和状态 JSON。F8 只要求可比较的 CSV / JSON，不要求完整 AIPlayer 胜率平台。
...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。当前还可对 `golden_basic_run.replay` 重跑真实运行时摘要；后续再把“摘要对照”收紧为真实输入重放和帧级 diff。
