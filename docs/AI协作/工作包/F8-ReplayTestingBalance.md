...
4. **Replay 重放 runner 首片**：已建立 `python tools/godot_bridge.py --project client replay-runner`，当前先读取 `.replay` envelope、校验 `recording_hash` / data fingerprint，并比较稳定 summary；失败时输出首个字段级 diff。带 `--rerun-runtime-summary` 时可按 replay seed 启动真实 `GameplayRunLoop` 并比较 `run_summary`。它仍是 L3 runner 外壳，不声称已完成输入回放 / 帧级复现。
5. **Gameplay 输入录制首片**：已建立 `python tools/godot_bridge.py --project client replay-input-smoke`，当前启动真实 `GameplayRunLoop`，确认移动 / 瞄准状态变化和 `pause` / `ui_back` 离散输入能进入 `Replay.input_events`。它只证明录制路径已接入，不代表输入播放 / 帧级复现完成。
6. **首批黄金回放**：已入库 `client/tests/replays/golden_basic_run.replay`，由 `python tools/godot_bridge.py --project client capture-golden-replay` 通过真实 `GameplayRunLoop` 生成，当前覆盖固定 seed 180 帧的运行时摘要。`golden_pause_resume`、`golden_full_death` 和后续帧级版 `golden_basic_run` 仍待输入播放 / 对照能力落地；`golden_relic_synergy` 等遗物运行时 / 协同原语落地后再录，不为空造假。
7. **Perf / 平衡采样首片**：已建立 `python tools/godot_bridge.py --project client perf-probe`，输出运行时长、帧时间、敌人 / 子弹 / pickup 池水位、击杀、等级和状态 JSON。F8 只要求可比较的 CSV / JSON，不要求完整 AIPlayer 胜率平台。
8. **门禁整合**：把新 L1 / replay / perf 命令接入本地验证说明；是否进入 pre-commit / CI 要分阶段，不把分钟级以上检查塞进秒级 hook。
...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。当前还可对 `golden_basic_run.replay` 重跑真实运行时摘要，并已录制首批 gameplay 输入事件；后续再把“摘要对照 + 输入录制”收紧为真实输入播放和帧级 diff。
- 至少 `golden_basic_run`、`golden_pause_resume`、`golden_full_death` 中的一条进入版本库并能稳定通过；当前 `golden_basic_run.replay` 已满足运行时摘要 golden，未完成项原因是输入播放 / 帧级 diff 尚未落地。
