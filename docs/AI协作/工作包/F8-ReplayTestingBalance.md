...
6. **首批黄金回放**：已入库 `client/tests/replays/golden_basic_run.replay` 与 `client/tests/replays/golden_pause_resume.replay`，由 `python tools/godot_bridge.py --project client capture-golden-replay` 通过真实 `GameplayRunLoop` 生成。`golden_basic_run` 覆盖固定 seed 180 帧的运行时摘要与 30 帧间隔稳定帧样本；`golden_pause_resume` 覆盖 pause 输入打开暂停菜单、`ui_back` 恢复运行、`ui_stack` 清空和帧样本状态变化。`golden_full_death` 和更完整帧级版 `golden_basic_run` 仍待补；`golden_relic_synergy` 等遗物运行时 / 协同原语落地后再录，不为空造假。
...
- `Replay` 能保存 / 读取一条最小 `.replay`，并能在 headless runner 中对照稳定摘要；失败时输出可定位的首个差异。当前还可对 `golden_basic_run.replay` / `golden_pause_resume.replay` 重跑真实运行时摘要与稳定帧样本，已录制首批 gameplay 输入事件，并能在 runner 内播放输入事件；后续再扩全量帧级字段和更多场景。
- 至少 `golden_basic_run`、`golden_pause_resume`、`golden_full_death` 中的一条进入版本库并能稳定通过；当前 `golden_basic_run.replay` 已满足运行时摘要与稳定帧样本 golden，`golden_pause_resume.replay` 已满足暂停 / 恢复输入场景 golden，更多输入场景仍待补。
