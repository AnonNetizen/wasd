...
3. **设置面板正式场景（首片已落地）**：新增 `client/scenes/ui/settings_panel.tscn` 和脚本，从标题菜单与暂停菜单进入；第一版至少覆盖语言、主音量 / 音乐 / 音效、全屏 / 垂直同步、屏幕震动、失焦暂停和数据收集开关。
...
5. **输入重绑定首片（已落地，收尾 polish 已补）**：用 `InputMap` action 和 `Settings` 保存键盘主绑定；已覆盖移动、瞄准、主动道具、暂停、确认 / 返回这些基础 action，`SettingsPanel` 可修改并持久化，`Settings` 只替换 action 的键盘事件，保留手柄轴 / 按钮事件。收尾 polish 已补输入绑定反馈、共用键位提示和一键恢复输入默认；后续再扩展完整手柄细项和更完整的按键捕获体验。
...
- 语言、音量、显示、玩法开关和基础输入绑定至少有第一版可见控件，并通过 `Settings` 读写；输入绑定首片覆盖键盘主绑定、共用键位提示和恢复输入默认，完整手柄重绑定仍待后续切片。
- `Settings` 配置能保存、重启后读取、损坏时回退默认值，并不污染玩家进度存档。
- `zh_CN` / `en` 可在运行时切换，核心常驻 UI 文案刷新，不显示裸 key；`settings-smoke` 覆盖 SettingsPanel、TitleMenu、PauseMenu、GameplayHud、LevelUpPanel、GameOverPanel 与 MetaProgressionPanel 的既有实例刷新。
