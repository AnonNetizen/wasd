...
5. **输入重绑定首片（已落地）**：用 `InputMap` action 和 `Settings` 保存键盘主绑定；已覆盖移动、瞄准、主动道具、暂停、确认 / 返回这些基础 action，`SettingsPanel` 可修改并持久化，`Settings` 只替换 action 的键盘事件，保留手柄轴 / 按钮事件。后续再扩展完整手柄细项和更完整的按键捕获体验。
...
- 语言、音量、显示、玩法开关和基础输入绑定至少有第一版可见控件，并通过 `Settings` 读写；输入绑定首片覆盖键盘主绑定，完整手柄重绑定仍待后续切片。
