...
3. **设置面板正式场景（首片已落地）**：新增 `client/scenes/ui/settings_panel.tscn` 和脚本，从标题菜单与暂停菜单进入；当前玩家可见控件覆盖语言、主音量 / 音乐 / 音效、回放记录、数据收集和基础输入绑定。全屏 / 垂直同步、松开瞄准停火、自动瞄准、屏幕震动和失焦暂停等未接线项暂时隐藏。
