...
- 当前 run schema 为 v8；导航 / 感知仍不保存，恢复后在下一决策 tick 重新感知。远程前摇 / 点射保存阶段、计时、锁向与剩余弹数。
...
- `runtime-smoke` 覆盖直追、流场 waypoint、视线 / 路径感知、1.5 秒记忆、守家回位，以及冲锋 / 远程不穿墙；突击枪手追加锁向不追踪、精确 4 发 / 间隔 / 冷却、暂停冻结、Combat 伤害、Run v8 前摇 / 点射恢复和共享子弹视觉复用；继续覆盖玩家唯一目标、友伤拒绝和中心分离。
