...
- `ModuleWorldManager` 在组图、技术首片和恢复 assignment 后重建导航数据；导航与感知缓存均为派生状态，不进入 Run v15。ADR #166 的敌人出生生命 / 伤害倍率不改变导航、感知或移速。
...
- `runtime-smoke` 覆盖直追、流场 waypoint、视线 / 路径感知、1.5 秒记忆、守家回位，以及冲锋 / 远程不穿墙；突击枪手追加锁向不追踪、精确 4 发 / 间隔 / 冷却、暂停冻结、Combat 伤害、Run v15 前摇 / 点射恢复和共享子弹视觉复用；继续覆盖普通玩家唯一目标、事件防御目标上下文、跨目标组最近命中、友伤拒绝和中心分离。
