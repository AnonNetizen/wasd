...
4. **F9.3 Demo 表现占位规范**：为新增敌人、武器、拾取、机关、UI 提示、SFX / BGM 等建立一致占位策略；俯视资产遵守 GDD §8.2-A，地面范围可用菱形并对齐量化菱形地图格，非地面资产保留自由轮廓但必须有清晰俯视轮廓、方向读法和真实判定形状。若新增音频 id，先登记词表并走 `AudioManager`，不在业务脚本直接播放。
...
## F9.3 俯视资产处理规则（2026-06-23；ADR #124 更新）
...
- 不因为菱形机关和量化背景网格，把角色、敌人、拾取物、子弹、障碍物或特效强制做成菱形；非地面资产靠俯视轮廓、方向读法、功能色和真实判定形状保持可读性。
