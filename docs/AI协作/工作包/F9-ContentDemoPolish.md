...
4. **F9.3 Demo 表现占位规范**：为新增敌人、武器、拾取、机关、UI 提示、SFX / BGM 等建立一致占位策略；固定斜俯视资产遵守 GDD §8.2-A，地面范围读菱形，非地面资产保留自由轮廓但必须有落地点 / 阴影 / 遮挡层级。若新增音频 id，先登记词表并走 `AudioManager`，不在业务脚本直接播放。
...
- 贴地范围（机关危险区、AOE 预警、房间边界、地面符号）优先用菱形或与菱形网格对齐的轮廓，并让视觉边界和真实判定一致。
...
- 不因为菱形机关和背景网格，把角色、敌人、拾取物、子弹、障碍物或特效强制做成菱形；非地面资产靠落地点、阴影、遮挡和排序统一斜俯视读法。
