...
- **可玩 runtime**：标题进入游戏、默认角色、鼠标瞄准 + 键盘 / 手柄兜底瞄准、自动开火、玩家受击无敌、相机居中、世界网格参照、失败结算、重开 / 回标题、暂停保存退出与继续游戏都已接入正式场景和 smoke。
...
- **机关 / hazard**：`hazards.csv` 有尖刺机关样例，`spawn_waves.csv` 也有可选字段，但 HazardSystem runtime 未落地；不要把机关放进第一批 Demo 内容。
...
- **新敌人变体**：`enemy_bulwark`，复用 `enemy_chaser` 对象池和追击 runtime；定位为慢速高血量压迫型敌人，数值为 `max_hp=28`、`move_speed=72.0`、`contact_damage=2`、`exp_reward=6`，占位色 `#b07d52`。
- **新刷怪波次**：`wave_standard_mid_bulwarks`，在标准生存模式 55 秒后开始，每 4.5 秒尝试刷 1 个 `enemy_bulwark`，同时存活上限为 3，避免首片直接推高实体密度。
- **新升级候选**：`growth_move_speed_small`（移动速度 +18）与 `growth_max_hp_small`（最大生命 +1），均为 `kind=stat_modifier` 且 `min_level=3`，避免改变现有 `golden_level_up_choice` 的二级候选集合。
...
- 核心 smoke：`headless-boot`、`runtime-smoke`、`f9-demo-smoke`、`settings-smoke`、`meta-smoke`、`save-smoke`、`l1-smoke`、`replay-smoke` 均通过；坏 JSON / 非法配置日志仍是既有恢复探针。
...
结论：首批 F9 Demo 内容和无音频视觉反馈已穿过当前自动回归 / 性能守门。下一步进入 F9.5：按手动 Demo checklist 做一次完整人工复核，并决定是开启下一批小内容，还是等待音频资源后补 `AudioManager` 注册 / smoke。

### F9 Demo 手动 checklist 首版

每个 F9 可玩切片后至少人工跑一遍：

- 标题冷启动后进入游戏，移动、鼠标瞄准、兜底瞄准、自动射击和经验拾取正常。
