...
4. **F9.3 Demo 表现占位规范**：为新增敌人、武器、拾取、机关、UI 提示、SFX / BGM 等建立一致占位策略；俯视资产遵守 GDD §8.2-A，地面范围默认使用矩形 / 方形俯视格并对齐量化矩形地图格，非地面资产保留自由轮廓但必须有清晰俯视轮廓、方向读法和真实判定形状。若新增音频 id，先登记词表并走 `AudioManager`，不在业务脚本直接播放。
5. **F9.4 回归守门**：每个内容切片至少跑数据 / locale 校验、相关 smoke、一条或四条 golden replay runtime rerun。有意改变核心运行时语义时才重录 golden，并说明行为差异；`perf-probe` 不属于默认守门，只在用户明确要求性能测试时运行。
...
5. 验证最小集：`sync_contracts --check`、`validate_data`、`lint_project_rules`、`runtime-smoke` 和四条 golden replay runtime rerun；若 gameplay summary 无意变化，禁止重录 golden。性能测试只在用户明确要求时追加。
...
## F9 Actor 专属继承场景更新（2026-07-23；ADR #155）

- 角色 / 敌人的静态表现不再由单一通用场景或 `enemies.csv.visual_color` 承担：`PlayerBase` / `EnemyBase` 提供共享节点契约，当前 1 个角色与 5 个敌人各自使用真正的 Godot 继承场景。
- `characters.json.scene_path` 与 `enemies.csv.scene_path` 显式绑定内容场景；不同内容 id 可复用同一 TSCN。敌人仍每种独立 `pool_id`，预热数量数据化且合计保持 28。
- F9 新增 / 调整敌人时，数值、AI profile 和池预热改数据，静态颜色、轮廓与未来动画 / 素材改专属 TSCN；禁止在 `Enemy.configure()` 中重新用数据覆盖场景表现。
- 默认验证追加 `actor-scene-smoke`；该 smoke 保护真实继承关系、必需节点、五种场景生成、池复用不串类型和场景表现不被 configure 覆盖。性能 probe 仍只在用户明确要求时运行。
...
## F9.3 首个表现占位切片（2026-06-20；历史，已由 ADR #155 取代）

已先做一个低风险的敌人占位可读性切片：

- `Enemy._draw()` 对所有敌人统一绘制暗色轮廓和眼睛描边，提升 `enemy_chaser` / `enemy_swarm` / `enemy_bulwark` 在移动、叠靠和受击/死亡反馈中的轮廓可读性。
- 当时仍只使用 `enemies.csv.visual_color` 作为填充色；ADR #155 已删除该字段并把静态表现迁入专属继承 TSCN。
...
## F9.3 俯视资产处理规则（2026-06-23；ADR #125 更新）

后续占位替换或 AI 生成正式资源时，不要把“矩形地图格”误读为“所有资产都要做成矩形”：

- 贴地范围（机关危险区、AOE 预警、房间边界、地面符号）优先用矩形 / 方形俯视格或与量化矩形地图格对齐的轮廓；机关和规则型地面 footprint 尺寸用格子的整数倍表达，并让视觉边界和真实判定一致。
- 非贴地实体（角色、敌人、宝箱、石柱、墙、树、机器、拾取物、子弹、特效）可以使用自由轮廓；关键是有清晰 `anchor_point`、`orientation_read`、`sort_layer` 和真实碰撞 / 触发形状。
- AI 写资产 brief 时至少声明 `asset_type`、`footprint_shape`、`anchor_point`、`orientation_read`、`sort_layer`、`collision_or_trigger_shape`；缺字段先补 brief，再生成或替换资源。
- ADR #124 后正式运行时不再用低模 3D / 正交渲染层模拟斜俯视；可用 3D 离线辅助生成 2D 帧，但进入游戏的资源仍按俯视轮廓和 2D 判定落地。
...
- 同一局试玩 3~5 分钟无明显卡顿、对象池泄漏、卡死或无法退出状态；若用户明确要求性能测试，再对比 `perf-probe`。
...
- F8 四条 golden replay、核心 smoke、数据 / 文档校验继续作为 F9 内容扩展的回归护栏；`perf-probe` 只保留为用户明确触发的按需工具。
