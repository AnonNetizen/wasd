...
> ADR #142 / #154 / #164 / #166 / #173 / #188 / #190 的当前阶段工作包。模块世界 schema、确定性目标角落、直接通关语义和 Run v15 必须同步 GDD、代码文档、测试策略、数据手册、知识索引与 AI 记忆。
...
- `GameplayRunLoop` 负责首次遭遇、敌人 / 机关 / 事件、局内金币与 Gear Mod、目标完成和 Run v15 总快照。
...
- `module-world-smoke` 与 `module-world-technical-slice-smoke` 覆盖 49 个唯一槽位、77×77 导航、左下起点、三个确定性目标候选、6–12 次跨越、fallback 覆盖、中心 3×3 / 外圈 40 格技术首片、迷雾、意识核直接完成、Run v15 roundtrip 与 hash mismatch。
- `world-event-smoke`、`save-smoke`、`loading-smoke`、`runtime-smoke`、headless boot/editor。
- Replay v6 四条黄金回放重录与运行时回归。
