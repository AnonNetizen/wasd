...
> 本文档是 ADR #142 的 9×9 模块连续大地图阶段工作包；模块制作与烘焙边界以 ADR #154 为准，当前运行存档版本以 ADR #161 的 Run v5 为准。改世界 / 模块 JSON schema、模块组合、流式激活、迷雾、目标撤离、Run v5 世界字段或验收命令时，必须同步 GDD、ADR、AI 导航、模块文档、测试策略、数据手册、知识库索引与 AI 记忆。
