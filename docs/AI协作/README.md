...
10. **改了词表 / 数据 / 文案 / GDScript**？跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/lint_gdscript_rules.py`、`python tools/lint_project_rules.py` 与非阻塞 `python tools/lint_semantic_rules.py`；改 F4 运行时追加 `python tools/godot_bridge.py --project client runtime-smoke`；改 Settings 持久化 / 回退追加 `python tools/godot_bridge.py --project client settings-smoke`；改 MetaProgression / 局外成长结算追加 `python tools/godot_bridge.py --project client meta-smoke`；改 SaveManager / run 存档 / 续局 schema 追加 `python tools/godot_bridge.py --project client save-smoke`；改 DataLoader、项目规则 lint 或语义 lint schema 时追加对应 `test_*.py` 回归。
