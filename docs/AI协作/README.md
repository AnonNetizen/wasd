...
4. **是不是正式项目阶段任务**？优先读 `工作包/`；当前 F6 用 `工作包/F6-MetaProgression.md`，F4 最小可玩闭环看 `工作包/F4-MinPlayableLoop.md`，历史 F3 数据闭环看 `工作包/F3-DataLoader.md`。
...
10. **改了词表 / 数据 / 文案 / GDScript**？跑 `python tools/sync_contracts.py --check`、`python tools/validate_data.py`、`python tools/lint_gdscript_rules.py`、`python tools/lint_project_rules.py` 与非阻塞 `python tools/lint_semantic_rules.py`；改 F4 运行时追加 `python tools/godot_bridge.py --project client f4-smoke`；改 MetaProgression / 局外成长结算追加 `python tools/godot_bridge.py --project client meta-smoke`；改 SaveManager / run 存档 / 续局 schema 追加 `python tools/godot_bridge.py --project client save-smoke`；改 DataLoader、项目规则 lint 或语义 lint schema 时追加对应 `test_*.py` 回归。
