...
- 在正式 `client/` 内编排标题 → 主／子英雄组合选择 → Loading → 对局，以及继续、暂停保存退出、重开和回标题。对局包含正式 Player / Weapon / Skill / Enemy / Combat、池化实体、金币成长、显式奖励选择、7×7 Gear Mod v6 棋盘、GameplayEffectRuntime、F13 7×7 模块世界、五类世界事件、三站式传送台网络、模式级威胁时间、冻结内容池 / 本地玩法环境与 Run v20 恢复。
...
- ADR #199 / #200 后当前协议为 Meta v4 / Run v20 / Replay v10 / 游戏 v1.19。Run v20 在完整确定性快照上保存传送选择来源站与暂停 underlying state；旧 Run v19 保留拒绝。Replay v10 记录严格 `teleport_choice` 成功 / 取消语义，旧 v9 保留拒绝；选择与淡出入采用非暂停 simulation-active 语义。
...
| `client/scenes/ui/teleport_choice_panel.tscn` / `teleport_destination_button.tscn` / `client/scripts/gameplay/teleport_choice_panel.gd` / `teleport_minimap.gd` | 暂停世界的 visited-only 目的地面板、编号列表与 7×7 不泄露小地图；支持鼠标、键盘、手柄焦点、`UI_BACK` 取消和暂停菜单叠加 |
...
UIManager
    ...
    ├── TeleportChoicePanel (only while GameState.TELEPORT_CHOICE)
...
| 机关触发 | `Hazard` 在 gameplay simulation active（`PLAYING` 或非暂停 `TELEPORT_CHOICE`）下按 `GameClock.delta_scaled()` 消耗冷却；玩家进入矩形范围后构造 `DamageInfo` 并交给 `Combat`，当前 FEA-12 用于验证 PCG / 手工摆点和伤害链路 | `GameState.is_gameplay_simulation_active()`、`Hazard.configure()`、`Combat.apply_damage()` |
...
| 主动暂停 | `pause` action 在 `PLAYING` 中打开 `PauseMenu`，在 `REWARD_CHOICE` / `TELEPORT_CHOICE` 中由对应面板请求把 `PauseMenu` 叠在其上；菜单通过 `UIManager` 请求 `GameState.PAUSED`，玩法时间、敌人、子弹和刷怪冻结，菜单仍响应鼠标、`ui_back` 和再次 `pause` action；暂停菜单可打开 `SettingsPanel`，关闭后仍回到同一个暂停菜单；关闭选择态上方的暂停菜单后必须恢复原 `REWARD_CHOICE` / `TELEPORT_CHOICE` | `UIManager.push()`、`GameState.PAUSED` |
| 传送选择 / 提交 | 首访传送台模块即发现该站；交互时来源预警、活敌或敌人快照均拒绝。候选仅包含同网络已访问站点，不泄露其他站。`TELEPORT_CHOICE` 与淡出入不暂停：RunLoop、敌人、投射物、机关、状态、回复和冷却继续使用 gameplay simulation active 推进，Player / Weapon / Skill / interact 仍只在 `PLAYING` 接受新输入。确认后在全黑帧二次校验玩家存活、来源、目标、placement 与落点，再依次执行 Player `teleport_to()` → ModuleWorld `tick()` / 流送 → Camera `snap_to_target()` → HUD / 交互刷新；提交成功即记录语义选择，再继续淡入，避免其间死亡导致成功事件丢失。任一失败不移动玩家并回到可选态，期间死亡或外部离态会清理面板且不覆盖新状态 | `GameState.is_gameplay_simulation_active()`、`GameState.TELEPORT_CHOICE`、`apply_replay_teleport_choice()` |
...
| 传送选择续局丢失、世界意外冻结或暂停菜单关闭后回错状态 | Run v20 `ui_restore` 是否保存来源站和 underlying `TELEPORT_CHOICE`；恢复时该站是否仍存在、属于当前模块且已访问；`TELEPORT_CHOICE` 是否保持 simulation active；暂停关闭后是否回到原选择态 |
