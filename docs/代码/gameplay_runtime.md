...
- 在正式 `client/` 内编排标题 → 主／子英雄组合选择 → Loading → 对局，以及继续、暂停保存退出、重开和回标题。对局包含主英雄场景 / 属性 / 被动 / 主色 / 技能 1/2、副英雄副色 / 技能 3/4、双涡旋史莱姆表现、无限射击起始武器、四技能共享能量、基础冲刺、七元素、防御层、武器后坐、敌人击退、池化实体、生成时金币奖励、金币成长、显式奖励选择、手动拾取的无等级局内 Gear Mod、敌人显式攻击、F13 7×7 模块世界、五类世界事件、模式级威胁时间、难度标记器、冻结的内容可用池与 Run v17 恢复。
...
- ADR #161 已提供冷静 / 愤怒两名英雄与正式组合选择，固定四技能、共享能量、技能 / 冲刺 HUD、状态汇总、能力 / 防御详细面板；ADR #187 后两名内置英雄基础 `max_shield` 均为 0，普通护盾从局内属性增长获得，超量护盾容量为当前最大生命与最大护盾之和。ADR #188 后正式定位为俯视角射击 Roguelike；ADR #189 / #193 后 Meta 为 v4、Run 为 v17、Replay 为 v7，并由 `ContentUnlockSystem` 管理英雄、Gear Mod、敌人的横向解锁与图鉴查询。每局 Gear Mod 空开局，每次拾取追加一份无等级固定效果实例并立即生效，完成意识核目标立即结束，不存在暂存战利品、撤离阶段或 Mod 升级接口。
...
| 改局内 Gear Mod 单份授予 / 实例叠加 / 应用 | `client/scripts/gameplay/gameplay_run_loop.gd`、`client/scripts/autoload/gear_mod_system.gd`、`client/scripts/gameplay/player.gd`、`client/scripts/gameplay/weapon_system.gd` |
...
| `client/scripts/data/difficulty_progression.gd` | 模式级威胁时间、难度系数、90 秒阶段、敌人出生倍率和 Run v17 快照；由 RunLoop 决定每帧是否推进 |
...
| `client/tools/module_world_smoke.gd` | 覆盖 49 槽 seed assignment/hash、三角意识核、三种不同事件、12 chunk / 三 pin、无缝流式、起点门禁、敌人倍率、未拾取 Mod 与 Run v17 恢复 |
...
| `client/scenes/gameplay/skill_system.tscn` / `client/scripts/gameplay/skill_system.gd` | 预置 `StatusEffectComponent` 的四槽技能系统；负责槽位快照、共享能量、能力四维缩放、通用效果 / 状态 / 修饰器、屏障、combat gate 和 Run v17 恢复 |
...
| `client/scripts/gameplay/enemy.gd` | 数据驱动敌人 AI、事件主目标 / 玩家附带受击、显式攻击 / 连锁、友伤护栏、出生倍率、锁定金币、退场语义和 Run v17 快照 |
...
| `client/scripts/gameplay/gameplay_hud.gd` / `client/scripts/ui/difficulty_marker.gd` | 响应式 HUD：生命、普通 / 超量护盾、能量、四技能、冲刺、当前战区状态、击杀、威胁时间 / 等级 / 阶段 / 分段进度、起点锁定、Gear Mod 获得反馈与能力 / 防御 / 新敌人倍率详细属性；语言切换时用当前状态重画 |
...
| `client/tools/gear_mod_smoke.gd` | Gear Mod smoke，覆盖空开局、固定效果、同 id 重复实例逐份乘算、单份授予、立即生效、新局清空、Run 恢复、无等级 API 与替换式 modifier 幂等性 |
...
| 输入 | `InputService` 从 GUIDE 的 gameplay context 产生 `move` / `aim` `Vector2` 与开火、四技能、冲刺、交互、暂停等按钮 intent。键鼠瞄准取 pointer viewport position 相对玩家实际屏幕位置的方向，不读取相机震屏噪声 offset；R 与手柄 East 不产生 gameplay intent，Replay v7 记录最终 intent、组合选择与冻结内容池 | `InputService`、生成 `Actions` 常量、`Replay` v7 |

| 内容可用池 | 正式新局在任何相关 RNG 消费前由 `ContentUnlockSystem` 生成稳定排序快照；英雄组合、模块 / 世界事件 / 开放战区敌人及 Gear Mod 公共池、掉落表都先与快照求交集。续局读取 Run v17 快照，Replay 播放读取 v7 快照，开发者测试岛使用全部内容。局内达到条件不会改变当前局池 | `build_run_availability_snapshot()`、`configure_content_availability()` |
...
| Gear Mod 掉落 / 交互 | 敌人、缓存与世界事件先锁定 `mod_id`，再按来源位置生成通用 CPU 实体；单件位于来源上方 36 px，双件左右各 28 px。所有拾取物与缓存、祭坛、事件共用最近距离仲裁，一次 `interact` 只取最近一件；提示通过 `mod_definition()` + `modifiers()` 展示名称和固定效果。成功后追加一份实例，重复 id 同样有效；授予失败实体保留 | `GearModPickup`、`GearModSystem.modifiers()`、`GameplayRunLoop._try_interact_nearest()` |
...
| 保存退出 / 继续 | 暂停菜单生成 Run v17 payload，保存战斗、武器冷却 / 分层修正器、可重复的局内 Gear Mod `mod_ids`、活动与非活动模块的未拾取 Mod、经济、7×7 assignment / 目标角落、世界事件、奖励状态、`content_availability` 与未结算 `content_progress_delta`，不含任何弹药字段。恢复已有实体后从局内 Mod 实例统一替换应用一次；旧 Run v16 不转换等级语义，只删除 Run、保留 Meta v4 | `SaveManager.load_envelope()`、`GearModPickup.restore_snapshot()`、`WeaponSystem.restore_snapshot()` |
...
| 自动 smoke / 按需 probe | `content-progression-smoke` 验证默认开放、规则、计数器、Meta/Run、冻结池与隔离；`codex-smoke` 验证标题入口和锁定隐私；`world_event_smoke` 验证五类状态机及 Mod 实体生成；`gear-mod-pickup-smoke` 验证正式 CPU、手动交互、全部奖励源与池复用；`module-world-smoke` 验证 49 槽、三角意识核、三事件 assignment、pin、未拾取 Mod 与 Run v17；`runtime-smoke` 覆盖无限射击、无弹药契约、金币快照、无等级 Mod 实例、手动拾取、显式攻击和防御目标。Replay v7 runner 重建同 seed 并对照摘要。仅当用户明确要求性能测试时才运行 `perf-probe` | 对应 smoke / runner 工具 |
...
| `Enemy.snapshot()` / `restore_snapshot(snapshot_data)` | 无 / run payload | Dictionary / `void` | 保存生命、位置、AI action / FSM、伤害归因、出生生命 / 伤害倍率、完整奖励明细、owned tag 计数和状态效果；正式 Run v17 要求奖励快照 |
...
| `GameplayRunLoop.create_run_snapshot()` | 无 | `Dictionary` | 生成 Run v17 payload；保存对局状态、武器冷却 / 分层修正器、可重复的局内 Gear Mod `mod_ids`、全部未拾取 Mod、难度 profile、7×7 assignment / 目标角落、模块 / 世界事件事务、敌人奖励快照、冻结内容池、未结算内容进度和全部现役 RNG state。只保存 JSON 友好数据 |
...
| `GameplayHud.set_life()` / `set_kills()` / `set_level()` / `set_gold_progress()` / `set_difficulty_snapshot()` / `show_level_up_feedback()` / `show_reward_feedback()` / `show_gear_mod_drop_feedback()` / `set_stats_panel_visible()` / `set_detailed_stats()` | HUD 状态 | `void` | 常驻 HUD 显示等级和“金币余额 · 等级进度”；Gear Mod 获得反馈只显示名称；详细面板拆分余额、累计金币和等级进度并保留 luck |
...
- 英雄组合：默认“冷静主 + 愤怒子”；`characters.json` schema v4 的两个初始 hero id 可复用同一基础场景，每个 palette 必须且只能含一个 `primary`。RunLoop 入树前接收主／子 id，解析器拒绝未知 id、重复组合或快照外角色；续局从 Run v17 恢复。主英雄独占基础属性、被动、场景、主色和技能 1/2，子英雄只贡献副色与技能 3/4；Resolver palette 精确返回 `main_primary` / `sub_primary`。
...
- 敌人 AI profile：从 `enemies.csv.ai_profile_id` 引用 schema v5 `enemy_ai_profiles.json`；`perception` 配置视觉 / 路径 / 记忆，`movement` 只保留通用移动字段，攻击 action 必须携带精确 `attack`，远程攻击额外必填 `windup` / `burst_count` / `shot_interval`。派生导航缓存不进 Run v17；攻击阶段、事件归属 / target mode、剩余时间、锁向、点射、armed、生成序号、出生倍率、奖励明细和状态效果进入快照。
...
- 玩家俯视表现：`Player` 是 `CharacterBody2D`，`player.json` schema v4 的 `body.radius=25` 同时驱动圆形碰撞、`hit_radius()`、`PlayerSlimeVisual` 与有限地图边界内缩；`player_separation_radius` 同为 25。正式双涡旋史莱姆固定 20 个控制点 / 100 个边界点，3 px 主色外轮廓、1 px 主色提亮湿润边和终点 38 px 主色短束；`Ground=(0,25)`、`Overhead=(0,-36)`、`Muzzle=(38,0)`、世界文字 `(0,-58)`。软体只影响表现且暂停冻结，不写 Run v17；移动、碰撞、受击和武器反冲仍维持 2D。`GameplayCameraController` 是 `GameplayRunLoop/ActiveWorld` 中与 `PlayerHost` 并列的唯一对局级 Rig，RunLoop 在角色创建或恢复后重新绑定 GLUED PCam。当前固定 `ignore_rotation=true`、无插件 smoothing / damping / lookahead / dead zone / auto zoom / load tween，并保持 `Vector2.ONE` 等比缩放。`camera_feedback.json` schema v3 的 `aim_look` 驱动帧率无关平滑引导；由于当前固定版本插件的 GLUED 更新路径不消费其导出 `follow_offset`，项目适配层仍动态写该属性作为权威状态，同时在 host 更新后把相同稳定偏移镜像到真实 `Camera2D` 位置，不修改插件源码。鼠标瞄准使用光标相对玩家实际屏幕位置的方向；`Camera2D.offset` 只承载震屏，开关震屏不清理引导偏移。
...
- 奖励选择：`reward_choice_pools.json` 定义池和候选；当前只解释 `kind=stat_modifier`。调用方显式提供 pool / trigger / 2–5 候选数，控制器按当前等级过滤、稳定 id 排序后使用 `RNG.ui_choice` 加权无放回抽取；`luck` 无影响。Run v17 保存 trigger、pool 和原候选，续局恢复时不再消耗 RNG。
...
- run 续局快照：`RUN_SNAPSHOT_SCHEMA_VERSION` 与 `SaveManager` run envelope 均为 v17。武器保存冷却、普通 / 临时 / Gear Mod 分层修正器、可重复的局内 Mod `mod_ids`、冻结内容池、未结算内容进度与 7×7 assignment / 目标角落，不含弹量、换弹、掉落计数或场上弹匣；旧 Run v16 不兼容，只删除 Run、保留 Meta v4。恢复顺序是 assignment / 内容池 → 实体基础状态 → 普通 / 临时层 → 从排序后的 Mod 实例替换 Gear Mod 层一次。RNG 大整数 state 继续以字符串保存并在 hash 前 JSON 归一化。
- 局内 Gear Mod 权威：`GameplayRunLoop` 保存可重复的 `mod_ids`，新局为空；每次授予只追加一份，无 rank、count 参数、升级或溢出分支。`GearModSystem` 仅解析定义、公共池、掉落和固定 modifiers，不读写 Meta。Player 与 WeaponSystem 使用 `set_gear_modifiers()` 替换专属层，同一实例数组重复重建不得在旧结果上再次叠加；同 id 的多份乘法 modifier 合法逐份相乘。
...
| 死亡后新局仍保留 Gear Mod | `GameplayRunLoop._on_player_died()` 是否先捕获结果页构筑再清理局内 `mod_ids`；`restart_run()` / 新实例是否从空数组开始；Meta payload 是否没有 `gear_mods` |
...
| 本局 Gear Mod 属性无效 | `_run_gear_mod_ids` 是否已追加；`GearModSystem.modifiers()` 是否输出目标 stat；统一单份授予入口是否在更新状态后调用替换式重建 |
...
| 继续游戏后状态不对 | Run v17 是否包含 7×7 assignment / 目标角落、武器冷却 / 分层修正器、可重复的局内 Gear Mod `mod_ids`、活动 / 非活动模块未拾取 Mod、难度 profile / 系数、世界事件、pin、波次计划、敌人归属 / target mode、既有攻击、奖励、冻结内容池与未结算进度字段；是否按依赖顺序恢复且未重复提交 |
...
- 涉及玩家普通 / 超量护盾初始值、容量、获得或恢复钳制时追加 `l1-smoke`、`runtime-smoke`、`save-smoke` 与四条 Replay v7 黄金回放；逐字段审计 data fingerprint 与摘要，未进入基线字段的变化不得用 wall-time 抖动制造无意义重录。
...
- 涉及局内 Gear Mod `mod_ids`、单份授予、固定效果、重复实例、掉落或 modifier snapshot 时追加 `python tools/godot_bridge.py --project client gear-mod-smoke`；如果改了死亡接入、敌人击杀归因或结果面板，同时跑 `runtime-smoke`、`save-smoke`。
...
- 涉及模块世界、事件 placement、chunk / pin、迷雾、地图 hash 或 Run v17 恢复时，追加 `world-event-smoke`、`module-world-smoke` 与 `save-smoke`，并跑 contracts、data 和 schema 检查。
- 涉及默认解锁、规则组合、内容池过滤、Meta / Run / Replay 内容字段、图鉴锁定隐私或结算新解锁时，追加 `content-progression-smoke`、`codex-smoke`、`runtime-smoke`、`save-smoke` 与四条 Replay v7 黄金回放；不得运行性能 probe 代替这些确定性门禁。
...
游戏版本为 v1.16；`SaveManager` 的 `run` envelope 与 gameplay payload 均为 v17，保存战斗 / 经济 / 模块 / 世界事件事务、可重复的局内 Gear Mod `mod_ids`、难度 profile、敌人奖励、冻结内容池、未结算内容进度及武器冷却 / 分层修正器，不保存弹药语义。旧 Run v16 及更早版本不转换等级数据，正式启动显示不兼容提示、只删除该 Run 并要求新开，Meta v4 保持不变且不保存任何 Gear Mod 实例。Replay v7 明确拒绝 v6。死亡与意识核完成原子提交进度后删除 Run，结果页按 Mod id 聚合显示数量，数量不代表等级。不得保存对象池内部状态或节点引用。
