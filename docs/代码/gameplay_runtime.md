...
- 在正式 `client/` 内编排标题 → 主／子英雄组合选择 → Loading → 对局，以及继续、暂停保存退出、重开和回标题。对局包含主英雄场景 / 属性 / 被动 / 主色 / 技能 1/2、副英雄副色 / 技能 3/4、双涡旋史莱姆表现、无限射击起始武器、四技能共享能量、基础冲刺、七元素、防御层、武器后坐、敌人击退、池化实体、生成时金币奖励、金币成长、显式奖励选择、手动拾取的局内 Gear Mod、敌人显式攻击、F13 7×7 模块世界、五类世界事件、模式级威胁时间、难度标记器、冻结的内容可用池与 Run v16 恢复。
...
| `client/scripts/data/difficulty_progression.gd` | 模式级威胁时间、难度系数、90 秒阶段、敌人出生倍率和 Run v16 快照；由 RunLoop 决定每帧是否推进 |
...
| `client/tools/module_world_smoke.gd` | 覆盖 49 槽 seed assignment/hash、三角意识核、三种不同事件、12 chunk / 三 pin、无缝流式、起点门禁、敌人倍率、未拾取 Mod 与 Run v16 恢复 |
...
| `client/scenes/gameplay/skill_system.tscn` / `client/scripts/gameplay/skill_system.gd` | 预置 `StatusEffectComponent` 的四槽技能系统；负责槽位快照、共享能量、能力四维缩放、通用效果 / 状态 / 修饰器、屏障、combat gate 和 Run v16 恢复 |
...
| `client/scripts/gameplay/enemy.gd` | 数据驱动敌人 AI、事件主目标 / 玩家附带受击、显式攻击 / 连锁、友伤护栏、出生倍率、锁定金币、退场语义和 Run v16 快照 |
...
| 内容可用池 | 正式新局在任何相关 RNG 消费前由 `ContentUnlockSystem` 生成稳定排序快照；英雄组合、模块 / 世界事件 / 开放战区敌人及 Gear Mod 公共池、掉落表都先与快照求交集。续局读取 Run v16 快照，Replay 播放读取 v6 快照，开发者测试岛使用全部内容。局内达到条件不会改变当前局池 | `build_run_availability_snapshot()`、`configure_content_availability()` |
...
| Gear Mod 掉落 / 交互 | 敌人、缓存与世界事件先锁定 `mod_id`，再按来源位置生成通用 CPU 实体；单件位于来源上方 36 px，双件左右各 28 px。所有拾取物与缓存、祭坛、事件共用最近距离仲裁，一次 `interact` 只取最近一件；提示展示名称、拾取后阶级和完整该阶效果，满阶预览 75 金币。授予失败实体保留 | `GearModPickup`、`GearModSystem.next_grant_preview()`、`GameplayRunLoop._try_interact_nearest()` |
...
| 保存退出 / 继续 | 暂停菜单生成 Run v16 payload，保存战斗、武器冷却 / 分层修正器、局内 Gear Mod ranks、活动与非活动模块的未拾取 Mod、经济、7×7 assignment / 目标角落、世界事件、奖励状态、`content_availability` 与未结算 `content_progress_delta`，不含任何弹药字段。恢复已有实体后从局内 Mod 状态统一替换应用一次；Run v15 无损补空拾取数组，旧 Run v14 及更早版本仍明确不兼容并只删除 run、保留 Meta v4 | `SaveManager.load_envelope()`、`GearModPickup.restore_snapshot()`、`WeaponSystem.restore_snapshot()` |
...
| 自动 smoke / 按需 probe | `content-progression-smoke` 验证默认开放、规则、计数器、Meta/Run、冻结池与隔离；`codex-smoke` 验证标题入口和锁定隐私；`world_event_smoke` 验证五类状态机及 Mod 实体生成；`gear-mod-pickup-smoke` 验证正式 CPU、手动交互、全部奖励源与池复用；`module-world-smoke` 验证 49 槽、三角意识核、三事件 assignment、pin、未拾取 Mod 与 Run v16；`runtime-smoke` 覆盖无限射击、无弹药契约、金币快照、手动拾取、显式攻击和防御目标。Replay v6 runner 重建同 seed 并对照摘要。仅当用户明确要求性能测试时才运行 `perf-probe` | 对应 smoke / runner 工具 |
...
| `Enemy.snapshot()` / `restore_snapshot(snapshot_data)` | 无 / run payload | Dictionary / `void` | 保存生命、位置、AI action / FSM、伤害归因、出生生命 / 伤害倍率、完整奖励明细、owned tag 计数和状态效果；正式 Run v16 要求奖励快照 |
...
| `GameplayRunLoop.create_run_snapshot()` | 无 | `Dictionary` | 生成 Run v16 payload；保存对局状态、武器冷却 / 分层修正器、局内 Gear Mod ranks、全部未拾取 Mod、难度 profile、7×7 assignment / 目标角落、模块 / 世界事件事务、敌人奖励快照、冻结内容池、未结算内容进度和全部现役 RNG state。只保存 JSON 友好数据 |
...
- 英雄组合：默认“冷静主 + 愤怒子”；`characters.json` schema v4 的两个初始 hero id 可复用同一基础场景，每个 palette 必须且只能含一个 `primary`。RunLoop 入树前接收主／子 id，解析器拒绝未知 id、重复组合或快照外角色；续局从 Run v16 恢复。主英雄独占基础属性、被动、场景、主色和技能 1/2，子英雄只贡献副色与技能 3/4；Resolver palette 精确返回 `main_primary` / `sub_primary`。
...
- 敌人 AI profile：从 `enemies.csv.ai_profile_id` 引用 schema v5 `enemy_ai_profiles.json`；`perception` 配置视觉 / 路径 / 记忆，`movement` 只保留通用移动字段，攻击 action 必须携带精确 `attack`，远程攻击额外必填 `windup` / `burst_count` / `shot_interval`。派生导航缓存不进 Run v16；攻击阶段、事件归属 / target mode、剩余时间、锁向、点射、armed、生成序号、出生倍率、奖励明细和状态效果进入快照。
...
- 玩家俯视表现：`Player` 是 `CharacterBody2D`，`player.json` schema v4 的 `body.radius=25` 同时驱动圆形碰撞、`hit_radius()`、`PlayerSlimeVisual` 与有限地图边界内缩；`player_separation_radius` 同为 25。正式双涡旋史莱姆固定 20 个控制点 / 100 个边界点，3 px 主色外轮廓、1 px 主色提亮湿润边和终点 38 px 主色短束；`Ground=(0,25)`、`Overhead=(0,-36)`、`Muzzle=(38,0)`、世界文字 `(0,-58)`。软体只影响表现且暂停冻结，不写 Run v16；移动、碰撞、受击和武器反冲仍维持 2D。`GameplayCameraController` 是 `GameplayRunLoop/ActiveWorld` 中与 `PlayerHost` 并列的唯一对局级 Rig，RunLoop 在角色创建或恢复后重新绑定 GLUED PCam。当前固定 `ignore_rotation=true`、无插件 smoothing / damping / lookahead / dead zone / auto zoom / load tween，并保持 `Vector2.ONE` 等比缩放。`camera_feedback.json` schema v3 的 `aim_look` 驱动帧率无关平滑引导；由于当前固定版本插件的 GLUED 更新路径不消费其导出 `follow_offset`，项目适配层仍动态写该属性作为权威状态，同时在 host 更新后把相同稳定偏移镜像到真实 `Camera2D` 位置，不修改插件源码。鼠标瞄准使用光标相对玩家实际屏幕位置的方向；`Camera2D.offset` 只承载震屏，开关震屏不清理引导偏移。
...
- 奖励选择：`reward_choice_pools.json` 定义池和候选；当前只解释 `kind=stat_modifier`。调用方显式提供 pool / trigger / 2–5 候选数，控制器按当前等级过滤、稳定 id 排序后使用 `RNG.ui_choice` 加权无放回抽取；`luck` 无影响。Run v16 保存 trigger、pool 和原候选，续局恢复时不再消耗 RNG。
...
- run 续局快照：`RUN_SNAPSHOT_SCHEMA_VERSION` 与 `SaveManager` run envelope 均为 v15。武器保存冷却、普通 / 临时 / Gear Mod 分层修正器、局内 Mod ranks、冻结内容池、未结算内容进度与 7×7 assignment / 目标角落，不含弹量、换弹、掉落计数或场上弹匣；旧 Run v14 不兼容，只删除 run、保留 Meta v4。恢复顺序是 assignment / 内容池 → 实体基础状态 → 普通 / 临时层 → 从 Mod ranks 替换 Gear Mod 层一次。RNG 大整数 state 继续以字符串保存并在 hash 前 JSON 归一化。
...
| 继续游戏后状态不对 | Run v16 是否包含 7×7 assignment / 目标角落、武器冷却 / 分层修正器、局内 Gear Mod ranks、活动 / 非活动模块未拾取 Mod、难度 profile / 系数、世界事件、pin、波次计划、敌人归属 / target mode、既有攻击、奖励、冻结内容池与未结算进度字段；是否按依赖顺序恢复且未重复提交 |
...
- 涉及模块世界、事件 placement、chunk / pin、迷雾、地图 hash 或 Run v16 恢复时，追加 `world-event-smoke`、`module-world-smoke` 与 `save-smoke`，并跑 contracts、data 和 schema 检查。
