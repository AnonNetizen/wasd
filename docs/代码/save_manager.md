...
- 当前 `meta` 为 v2、`run` 为 v7：Meta 保存上次确认组合且保留现有 Gear Mod payload；Run 在既有世界 / 战斗状态之外保存金币余额、累计获得金币、场上金币球和未完成奖励选择，等级加载时重新推导。旧 Run v6 含已退役 XP / 升级选择状态，不做有损迁移；启动流程提示一次后只删除 run，Meta v2 保留。
...
| `run` | 当前一局续局档案，当前 v7：完整世界、英雄组合、金币、奖励选择、威胁时间和敌人出生倍率 |
...
Run v7 payload 包含：schema version、模式、`main_hero_id` / `sub_hero_id`、组合解析快照、击杀、`gold`（余额与累计获得量）、`reward_choice`（未完成请求及原候选）、`GameClock`、`DifficultyProgression`、`RNG`、模块世界、刷怪、玩家生命 / 护盾 / 超盾 / 护盾门 / 冲刺、当前武器后坐速度 / 剩余时间 / 总持续时间、武器、四槽技能与能量、敌人状态、独立接触计时及出生生命 / 伤害倍率、子弹、机关、金币球、能量球、屏障、兴趣点和 `ui_restore`。等级不重复保存，恢复时由 `gold_earned_total` 推导；未完成奖励选择恢复原候选，不重新抽取。难度快照保存 schema、profile、elapsed 与 enabled；恢复时先还原威胁时间，再重建模块与敌人。模块世界的每槽状态还保存首次进入遭遇的 `telegraphing/spawned`、`remaining_telegraph` 与已经固化的 `enemy_id + world_position` 计划；预警跨过阶段线时，敌人在真正生成时取得当时倍率。VFX 节点本身不保存，恢复 / 重新激活时按剩余时间重建。所有对象池实体只保存 JSON 友好活动快照，恢复时重新 acquire；RNG 的大整数仍以字符串保存。
...
当前 `meta` 为 v2、`run` 为 v7、`replay_index` 为 v1。Meta v1→v2 保留 Gear Mod 并补默认组合；Run 保留旧逐级迁移链，v4→v5、v5→v6 与 v6→v7 都是明确的不兼容重置边界。Replay 文件由 `Replay` 独立管理，当前为 v3。未来每次提升 kind 版本时必须：
