...
- 当前 `meta` 为 v2、`run` 为 v6：Meta 保存上次确认组合且保留现有 Gear Mod payload；Run 在组合、四槽、资源 / 防御、武器后坐冲量、状态、屏障、能量球和接触计时之外，保存模式级威胁时间及每只敌人的出生生命 / 伤害倍率。旧 Run v5 无法还原起点房停留时间和敌人出生倍率，启动流程提示一次后只删除 run，Meta 保留。
...
| `run` | 当前一局续局档案，当前 v6：完整世界、英雄组合、威胁时间和敌人出生倍率 |
...
Run v6 payload 包含：schema version、模式、`main_hero_id` / `sub_hero_id`、组合解析快照、等级 / 经验 / 击杀、`GameClock`、`DifficultyProgression`、`RNG`、模块世界、刷怪、玩家生命 / 护盾 / 超盾 / 护盾门 / 冲刺、当前武器后坐速度 / 剩余时间 / 总持续时间、武器、四槽技能与能量、敌人状态、独立接触计时及出生生命 / 伤害倍率、子弹、机关、经验球、能量球、屏障、兴趣点和 `ui_restore`。难度快照保存 schema、profile、elapsed 与 enabled；恢复时先还原威胁时间，再重建模块与敌人。模块世界的每槽状态还保存首次进入遭遇的 `telegraphing/spawned`、`remaining_telegraph` 与已经固化的 `enemy_id + world_position` 计划；预警跨过阶段线时，敌人在真正生成时取得当时倍率。VFX 节点本身不保存，恢复 / 重新激活时按剩余时间重建。所有对象池实体只保存 JSON 友好活动快照，恢复时重新 acquire；RNG 的大整数仍以字符串保存。
...
当前 `meta` 为 v2、`run` 为 v6、`replay_index` 为 v1。Meta v1→v2 保留 Gear Mod 并补默认组合；Run 保留旧逐级迁移链，v4→v5 与 v5→v6 都是明确的不兼容重置边界。Replay 文件由 `Replay` 独立管理，当前为 v3。未来每次提升 kind 版本时必须：
