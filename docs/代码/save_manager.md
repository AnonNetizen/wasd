...
- F5 存档可靠性切片已把 `run` kind 提升到 version 2，并注册 `run` v1 -> v2 迁移；F13 手工房间制首片再把 `run` 提升到 version 3，注册 `run` v2 -> v3 迁移给开放战区旧档回填空 `room` 块。`save-smoke` 覆盖 run roundtrip、`.bak` 回退、双坏档隔离、迁移链（含 v2 -> v3 room 回填断言）和高精度浮点 JSON 归一化 hash。
...
`run` kind version 3（F13 手工房间制首片）为 v2 旧 envelope 回填空 `room = {}` 块。v2 是 F13 之前的开放战区存档，没有房间 carrier 概念；回填空 room 块后，run payload 仍按「无房间 carrier」（开放战区）路径加载，不会被误判为房间 carrier 续局。room 块内部字段（当前房间序列 id、当前房间 id / 门 / 清房状态等）由 `GameplayRunLoop` / `RoomManager` 负责生产与解释，`SaveManager` 不解释，见 `docs/代码/room_manager.md`。迁移后会重算 `data_hash`，因此旧 v2 档不会因结构变更触发损坏隔离。
...
当前 `meta` 和 `replay_index` schema version 为 `1`；`run` schema version 为 `3`，`_ready` 已注册 `run` v1 -> v2 迁移（补齐缺失的结构字段）与 `run` v2 -> v3 迁移 `_migrate_run_v2_to_v3`（给开放战区旧档回填空 `room = {}` 块，使其按「无房间 carrier」加载，迁移后重算 `data_hash`、旧档不触发损坏隔离）。未来每次提升 kind 版本时必须：
