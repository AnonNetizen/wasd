...
`meta` kind version 1 当前由 `MetaProgressionSystem` 写入，payload 字段包括 `schema_version`、`currencies`、`account_xp`、`account_level`、`purchased_upgrades`、`unlocked_ids`、累计 `stats` 和 F11 新增的 `gear_mods` 子 payload。SaveManager 不校验这些业务字段，只校验 envelope 和 hash；旧 meta 归一化、结算公式和购买规则见 `docs/代码/meta_progression_system.md`，装备 Mod 库存 / loadout / 旧 `purchased_upgrades` 补偿记录见 `docs/代码/gear_mod_system.md`。
