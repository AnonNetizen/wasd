...
- 下游调用方：后续 `MetaProgressionSystem`、暂停菜单、主菜单继续游戏、结算流程、回放索引 UI。
...
| 改 `run` 快照 | 玩法快照生产者 | 本文档、测试策略、回放文档 | run roundtrip + L5 存档 checklist |
...
- 当前切片必跑 L0 契约 / 数据 / 文档检查、L2 headless boot，并做至少一次临时 roundtrip smoke。
...
当前 `meta`、`run`、`replay_index` schema version 均为 `1`，尚无迁移函数。未来每次提升 kind 版本时必须：
