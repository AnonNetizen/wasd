...
- 改 `meta` profile、结算、购买或局外成长存档调用方时追加 `python tools/godot_bridge.py --project client meta-smoke`；改死亡结算接入时追加 `python tools/godot_bridge.py --project client f4-smoke`。
