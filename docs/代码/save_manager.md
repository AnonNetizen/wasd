...
- F5 存档可靠性切片已把 `run` kind 提升到 version 2，并注册 `run` v1 -> v2 迁移；`save-smoke` 覆盖 run roundtrip、`.bak` 回退、双坏档隔离、迁移链和高精度浮点 JSON 归一化 hash。
- F11 已由 `GearModSystem` 接管真实 `meta` profile：装备 Mod 资源、库存、loadout、rank 和旧 `purchased_upgrades` 补偿记录写入 `meta.gear_mods`；旧死亡结算货币 / 账号经验 / 永久升级运行时代码已删除。`SaveManager` 仍不解释 profile 字段。
...
| `client/tools/gear_mod_smoke.gd` | F11 `meta.gear_mods` profile headless smoke：资源、库存、loadout、升级、分解、掉落和旧档补偿 |
...
| kind | 用途 |
|------|------|
| `meta` | 局外成长长期档案：当前由装备 Mod 保存资源、库存、rank、loadout 和旧档补偿记录；旧货币 / 账号等级 / 永久升级字段只作为历史 payload 兼容 |
...
F5/F9 的 run payload 当前包含：schema version、模式 / 角色 id、等级、累计经验、击杀数、`GameClock` 快照、`RNG` 快照、有限地图状态、刷怪状态、玩家状态、武器状态、技能状态、活跃敌人、活跃子弹、活跃机关、活跃经验球和 `ui_restore`。`map` / `hazards` 只由玩法运行时解释，用于恢复有限边界、PCG / 人工机关摆放、机关冷却和激活表现；`ui_restore` 当前用于区分普通游玩、暂停菜单和升级选择面板。旧 payload 缺失 `ui_restore` 时由运行时按普通游玩处理。`RNG` seed/state 这类可能超过 JSON 安全整数精度的值必须以字符串保存。
...
`meta` kind version 1 当前由 `GearModSystem` 写入，payload 主要字段为 `gear_mods` 子 payload；旧档可能仍包含 `schema_version`、`currencies`、`account_xp`、`account_level`、`purchased_upgrades`、`unlocked_ids` 和累计 `stats`。SaveManager 不校验这些业务字段，只校验 envelope 和 hash；装备 Mod 库存 / loadout / 旧 `purchased_upgrades` 补偿记录见 `docs/代码/gear_mod_system.md`。
...
- `meta` 接入：`GearModSystem` 负责解释 `meta.gear_mods` 与旧 `purchased_upgrades` 补偿，`SaveManager` 只负责可靠读写；改 profile schema 时同步 `docs/代码/gear_mod_system.md`。
...
| 改 `meta` payload | `GearModSystem`、Gear Mod 数据配置、必要时 legacy `meta_progression.json` 成本表 | 本文档、GearModSystem 文档 | `gear-mod-smoke` + 数据校验 |
...
- 改 `meta.gear_mods` profile、旧 `purchased_upgrades` 补偿或局外装配存档调用方时追加 `python tools/godot_bridge.py --project client gear-mod-smoke`；改死亡面板、标题入口或 run 清理时追加 `python tools/godot_bridge.py --project client runtime-smoke`。
- 后续引入 GUT 后，`SaveManager` 必须覆盖 envelope 字段、hash mismatch、原子写入 / `.bak`、迁移链、坏档隔离、`meta` / `run` roundtrip、slot 校验和删除行为。
- 改存档 schema 必须注册 migration 并补迁移测试；改 `run` 续局字段还要跑 L5 存档 checklist，影响确定性时补黄金回放；改 payload hash / 序列化路径时必须保留高精度浮点 roundtrip 用例。
