...
中央区域显示顶层标量列，可排序、筛选和直接编辑；列标题分隔线支持拖动调宽，最小宽度为 88 px，同一编辑器会话内按数据集、分区和列结构保留布局；`Ctrl+V` 从当前单元格起按 TSV 行列批量写入现有记录。右侧属性树递归处理嵌套对象、数组和不同形态的数组元素；空数组新增项使用对象兜底，非空数组根据首项生成默认结构。
