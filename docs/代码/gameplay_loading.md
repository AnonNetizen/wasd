...
- 不增加最低展示时间；准备完成即可移除。
- 当前 Run v17 envelope 恢复 7×7 assignment / 目标角落、冻结内容池、未结算内容进度、可重复的局内 Gear Mod `mod_ids`、活动 / 非活动模块未拾取 Mod、金币、世界事件、攻击、奖励及武器冷却 / 分层修正状态，不含任何弹药、`pending_loot` 或撤离字段；已有敌人不重抽、不重发。开始 / 重开把可选 difficulty profile id 与 `ContentUnlockSystem` 可用池在入树前贯通到 RunLoop，未指定难度时使用 mode 默认；继续直接恢复保存 profile、目标角落与内容池。旧 Run v16 明确拒绝并只删除 run，Meta v4 保留。当前正式 UI 始终使用标准难度。
