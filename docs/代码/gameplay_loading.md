...
- 不增加最低展示时间；准备完成即可移除。
- 当前 Run v12 envelope 恢复地图、金币、世界事件、攻击、奖励及武器冷却 / 修正状态，不含任何弹药字段；已有敌人不重抽、不重发。开始 / 重开把可选 difficulty profile id 贯通到 RunLoop，未指定时使用 mode 默认；继续直接恢复保存 profile。旧 Run v11 明确拒绝并只删除 run，Meta v2 保留。当前正式 UI 始终使用标准难度。
