...
> 本文档是主／子英雄四技能、能量、能力属性缩放、skills v3 programs、部署物和技能快照的权威运行时契约；统一效果执行见 `gameplay_effect_runtime.md`。
...
| `client/scripts/gameplay/skill_system.gd` | 四槽、能量、缩放、目标、Runtime 激活提交和快照 |
| `client/scripts/gameplay/effects/` | 每局 Runtime、原语 Registry 与受控 Gateway |
| `client/data/characters.json` | 英雄场景、配色、主属性、`passive_id`、两个 `hero_skill_ids` |
| `client/data/skills.json` | schema v3 成本、冷却、目标、`programs[]`、缩放声明和本地化 key |
...
| 输入 | `InputService` 的 `skill_1`～`skill_4` intent 分别调用同名槽位；Replay v9 记录相同归一化 intent |
| 释放 | 先校验 combat gate，再校验槽位、施法者、冷却、ability tag、能量和目标；成功后扣能量、向 Runtime 提交 `skill_activated`，再设置该槽冷却 |
...
| `snapshot()` / `restore_snapshot()` | Run v19 的技能子快照；程序冷却 / 周期 / action state 由 Runtime 单独快照，不保存 Node 引用，combat gate 由当前 RunLoop 重新注入 |
...
- `spawn_barrier` action 经 Gateway 取得 `projectile_barrier` 池对象；无自然持续时间，死亡、重施或世界清理时回池。
...
- hero、passive、skill、slot、resource、targeting、trigger、condition、action、status、stack rule、ability tag、element 和 pool id 全部来自生成常量。
...
| 范围／持续不缩放 | 技能 program action 参数是否显式声明对应 scaling |
...
| 续局丢失技能／屏障 | Run v19 是否保存组合、`skills` 与 GameplayEffectRuntime 快照；恢复是否先解析组合、注册来源再恢复状态 |
...
- 运行时：`lint_gdscript_rules.py`、`l1-smoke`、`effect-runtime-smoke`、`runtime-smoke`、`loading-smoke`、`headless-boot`；L1 覆盖能力缩放后的格式化值，Runtime 覆盖技能等价迁移和 Gateway，屏障覆盖跨圆规则，Loading 覆盖组合卡不泄漏未解析 token。
- 输入／回放：`input-smoke`、`replay-input-smoke`、`replay-smoke` 与四条 Replay v9 黄金回放。
- 存档／世界重建：`save-smoke`、`module-world-smoke`；调试入口变更追加 `debug-test-arena-smoke` 和 release smoke。

## 迁移边界

当前 Meta v4、Run v19、Replay v9、游戏 v1.18。旧 Run v18 与 Replay v8 文件保持原样但拒绝继续 / 播放，不迁移。技能 `default_unlocked=false` 在本阶段为非法配置；局内换子英雄尚未实现，不得从预留的重复槽倍率推导出可用的切换入口。
