...
- `SkillSystem.cast_slot(slot_id)` 管理四槽独立冷却、共享能量、目标筛选、效果解释和 Run v11 快照。冷却键永远是槽位，不是 skill id。
...
| `snapshot()` / `restore_snapshot()` | Run v11 的技能子快照；不保存 Node 引用，combat gate 由当前 RunLoop 重新注入 |
...
| 续局丢失技能／屏障 | Run v11 是否保存组合和 `skills` 快照；恢复是否先解析组合再恢复槽位 |
...
当前 Meta v2、Run v11、Replay v3。旧 Run v10 因缺少弹药 / 换弹 / `RNG.ammo` state 而不迁移玩法快照，提示一次后只删除 run；旧 Replay 明确拒绝。局内换子英雄尚未实现，不得从预留的重复槽倍率推导出可用的切换入口。
