...
- `SkillSystem.cast_slot(slot_id)` 管理四槽独立冷却、共享能量、目标筛选、效果解释和 Run v15 快照。冷却键永远是槽位，不是 skill id；技能与被动随所属英雄整包解锁，不建立独立资格。
...
| `snapshot()` / `restore_snapshot()` | Run v15 的技能子快照；不保存 Node 引用，combat gate 由当前 RunLoop 重新注入 |
...
| 续局丢失技能／屏障 | Run v15 是否保存组合和 `skills` 快照；恢复是否先解析组合再恢复槽位 |
...
当前 Meta v4、Run v15、Replay v6。旧 Run v14 与旧 Replay v5 明确不兼容；只删除旧 run 并保留 Meta v4。技能 `default_unlocked=false` 在本阶段为非法配置；局内换子英雄尚未实现，不得从预留的重复槽倍率推导出可用的切换入口。
