...
| `Enemy.snapshot()` / `restore_snapshot()` | 无 / 快照 | `Dictionary` / `void` | Run v15 精确保存和恢复最终金额、阶段及全部倍率 |
...
- Run v15 保存敌人奖励快照、冻结内容池和 `RNG.economy` state。已有敌人恢复不重抽；尚未生成的计划从保存的子流 state 继续。
...
- Runtime / Save：五种开局范围、成功生成只消费 economy、失败生成不消费、跨阶段锁定、世界事件实际生成时间、单金币球、非奖励退场、Run v15 roundtrip 和恢复不重抽。
