...
> 本文档是 `PlatformServices` 的代码契约权威；改平台 provider、Steamworks 接入边界、成就 / 富状态 / 联机 API 或测试义务时必须同步 GDD、ADR、AI 导航与测试策略。
...
- 接 Steamworks 时，在 `PlatformServices` 内部或相邻 adapter 中完成 SDK 初始化、user identity、achievements、stats、rich presence、overlay、Lobby / P2P / invite 转接。
...
当前只新增空后端接口，不改变存档、回放或数据 schema。未来真实平台接入不得改变既有单机流程：平台不可用时游戏仍应可启动、可游玩、可本地保存。
