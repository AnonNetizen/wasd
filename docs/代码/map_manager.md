...
- 提供统一的世界坐标 ↔ 菱形格中心吸附口径；玩家出生点、人工机关和 PCG 机关中心都落在格点上。
- 使用 `RNG.world` 生成可复现的初始机关摆放；手工摆点先放置，PCG 会吸附到格心并避开它们。
...
| 开局 | `MapManager` 先放 `manual_hazards`，再按 `pcg.hazards` 用 `RNG.world` 摆放机关，所有机关中心吸附到菱形格点 | `generate_hazard_placements()` |
...
| `normalize_hazard_position(world_position, hazard_id)` | 世界坐标、机关 id | `Vector2` | 吸附到格心，并按 `radius_tiles` 的完整菱形边界 clamp 到 bounds 内 |
...
- `player_start` 和 `manual_hazards[]` 必须落在菱形格中心；PCG 随机候选会运行时吸附到菱形格中心。
- `pcg.hazards[].id` 与 `manual_hazards[].id` 必须存在于 `hazards.csv`。
- `pcg.hazards[].count` 为目标数量；约束太紧时实际 placement 可以少于目标，但不能越界或进入安全圈。
- 机关实际边界由 `hazards.csv.radius_tiles` 与当前 grid cell size 推导，PCG 会确保机关完整留在 bounds 内。
...
| 玩家出生点 / 人工机关校验失败 | 坐标是否是 `grid.cell_width/2`、`grid.cell_height/2` 推导出的菱形格中心 |
...
| 机关看起来不像格子整数倍 | `hazards.csv.radius_tiles` 是否为正整数；`WorldBackground`、`MapManager`、`Hazard.configure()` 是否拿到同一份 `grid_cell_size` |
...
| 续局后机关位置变化 | run payload 是否包含 `map.hazard_placements` 和 `hazards`；是否误重新消耗 `RNG.world`；旧自由坐标存档恢复后吸附到格心属于 ADR #105 兼容行为 |
