...
ADR #128 的手工房间制短刷图首片（详见 `docs/代码/room_manager.md`）把 `MapManager` 当作纯几何 carrier 复用，不引入第二套地图系统：

- 房间 carrier 模式下，`MapManager` 只复用矩形 bounds / grid / `clamp_position()` / `player_start()` / `spawn_position()` 等几何能力；每进入一个房间，`GameplayRunLoop._configure_map_for_room()` 用 `RoomManager` 返回的该房 bounds layout 调 `configure()` 重配一次，让玩家 / 敌人移动边界、网格和刷怪 clamp 切到当前房间尺寸。
- 房间 carrier 模式**不调** `generate_hazard_placements()`：不做 PCG、不解释 WarzoneDirector 兴趣点、不生成 director POI / 目标 / 缓存箱 marker。房间内容（玩家入口、敌人、机关、门）由 `RoomManager` 读房间 `.tscn` 的 `@export` marker 提供，再由 `GameplayRunLoop` 走对象池生成；`MapManager` 不感知门 / 清房 / 切房逻辑。
- 默认 open-warzone（F12 短刷图）仍走完整 `configure()` + `generate_hazard_placements()` 路径，本节只描述 opt-in 的房间 carrier；两条路径共享同一份几何 API，不分叉边界 / clamp 实现。
