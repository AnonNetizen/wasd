...
> 本文档是 ADR #154 / #164 的模块 JSON 制作主源、单向 TSCN 烘焙、审核门禁与发布边界权威；修改 JSON schema、中央主编辑区、图块目录、生成场景结构、审核状态或 CLI 时必须同步 F13 工作包、数据手册、测试策略、AI 导航与 ModuleWorldManager 文档。
...
- 运行层：JSON 继续负责 7×7 assignment、导航、inactive-slot placement 查询、map hash、回放与 Run v18 模块子快照；运行开始时按 module id 预加载本次 assignment 用到的唯一规范场景，12 个预置 `ModuleChunk` 只挂载缓存的 `PackedScene` 实例，再对场景根节点应用 0/90/180/270° 正交旋转、方形枢轴补偿和封边方向反映射。旧 Run v17 及更早版本因 ADR #194 不兼容，不能推断 Gear Mod 棋盘坐标。
...
- placement 坐标和 footprint 必须为整数格、在界内并完全落在 floor；四边 socket 由边缘 floor 自动推导，JSON 不重复存储。旧 `module_place_enemy_spawn` 已删除；schema v4 新增 `module_place_world_event`，payload 严格为 `{world_event_id}`，每模块最多一个，编辑器通过登记事件 id 下拉选择并在画布显示专用图标。
- `module_flat_ground` 是当前正式普通池的唯一模板：11×11 全 floor、四边完全开放、没有 placement，只允许 0° 世界旋转。既有模块继续可在主编辑区和技术首片使用。
- `module_start_corner` 是正式左下出生模板：西 / 南边缘完全封闭，北 / 东各保留一个居中 floor socket，玩家 placement 位于 `(5,5)`；其玩法 hash 已由用户人工确认后批准。
...
`ModuleSceneBaker` 优先复用当前进程的 `DataLoader.validate_project_data()`。Godot
编辑器主进程不挂载运行时 autoload 时，插件会使用当前编辑器自身的 Godot
可执行文件启动一次 headless `DataLoader` 校验；只有该进程退出码为 0 才继续模块
语义校验，真实 DataLoader 错误会回显到校验结果。这个后备通道不会读取另一套
schema，也不会把“编辑器里找不到 DataLoader”误报成项目数据损坏。

## 4. CLI 与生成场景

```powershell
python tools/godot_bridge.py --project client module-bake
python tools/godot_bridge.py --project client module-bake --module module_start_corner
python tools/godot_bridge.py --project client module-bake-check
python tools/godot_bridge.py --project client module-bake-check --module module_start_corner
...
`module-bake-check` 是无写入门禁：检查 JSON v4、图块引用、允许旋转、审核状态、唯一规范生成物、遗留方向文件和场景规范指纹；生成物缺失 / 过期或人工改动 TSCN，即使保留原 metadata 也会失败。
