...
- 运行层：JSON 继续负责 7×7 assignment、导航、inactive-slot placement 查询、map hash、回放与 Run v17 模块子快照；运行开始时按 module id 预加载本次 assignment 用到的唯一规范场景，12 个预置 `ModuleChunk` 只挂载缓存的 `PackedScene` 实例，再对场景根节点应用 0/90/180/270° 正交旋转、方形枢轴补偿和封边方向反映射。旧 Run v16 及更早版本因 ADR #193 不兼容，不迁移旧 Gear Mod 等级语义。
