...
> 本文档是 F10 敌巢战区导演的代码契约权威；改导演数据 schema、阶段解释、刷怪接线、调试摘要或未来兴趣点生成时，必须同步 `docs/AI协作/工作包/F10-WarzoneDirector.md`、`docs/代码/gameplay_runtime.md`、`docs/代码/data_loader.md`、`client/data/README.md`、GDD 与 ADR。
...
- 不直接生成敌人、机关或兴趣点；首片只给 `GameplayRunLoop` 的 spawner 做 wave gating。
- 不播放 UI / 音频，不显示玩家可见文案。
...
| `client/scripts/gameplay/gameplay_run_loop.gd` | 创建导演、把 `debug_summary()` 暴露给运行时摘要、在 `_update_spawner()` 调用 `is_wave_enabled()` |
| `client/scripts/autoload/data_loader.gd` | Godot 侧 schema 校验 |
| `tools/validate_data.py` | Python 侧数据校验 |
| `tools/test_data_loader_schema.py` | 坏样例回归：导演引用不存在的 wave 必须 fail-fast |
| `client/tools/runtime_smoke.gd` | 验证开局 warmup phase 摘要 |
| `client/tools/f9_demo_smoke.gd` | 验证 55 秒后 guarded phase 仍允许 bulwark wave |
...
- `interest_points[].hazard_ids[]` 必须存在于 `hazards.csv`。
...
4. 每帧 `GameplayRunLoop._update_spawner()` 先询问 `is_wave_enabled(wave_key, GameClock.now())`，被当前 phase 禁用的 wave 直接跳过。
5. 通过导演后，原有 wave 时间窗、预算、同时存活上限和对象池生成逻辑继续执行。

## 依赖

- 上游依赖：`DataLoader`、`GameClock`、`spawn_waves.csv`、`warzone_directors.json`。
- 下游调用方：`GameplayRunLoop.debug_summary()`、`runtime-smoke`、`f9-demo-smoke`。
- 禁止依赖：`Player`、`WeaponSystem`、`Combat` 结果、HUD 输入、FPS / 性能指标或任何玩家表现数据。

## 扩展点

- 随机 mutation：必须先决定 RNG stream、保存 / 恢复策略和 replay 影响；首片不做。
- 地图兴趣点生成：优先接 `MapManager` 的数据化生成接口，不按 `poi_id` 写分支。
...
| 新增 encounter / interest point | `warzone_directors.json`、必要时 `map_layouts.json` / `hazards.csv` | `validate_data`；接生成逻辑后追加 runtime smoke |
| 改 schema | `data_loader.gd`、`validate_data.py`、`test_data_loader_schema.py`、本文档、数据手册 | schema test + docs health |
| 让导演影响地图 | `warzone_director.gd`、`map_manager.gd`、`gameplay_run_loop.gd` | runtime-smoke、f9-demo-smoke、save-smoke，评估 golden |
...
F10.1 不提升 run snapshot schema。导演当前由静态数据和 `GameClock.now()` 推导，可从已有 `spawn_states` 和局内时间恢复；后续若加入随机 mutation、阶段内部计数器或玩家可见选择，必须保存 director state 并同步 `GameplayRunLoop.create_run_snapshot()` / `configure_restore_snapshot()`。
