...
> 本文档是 F13 模块世界运行时、坐标、流式状态、F14 静态导航查询与 Run v5 模块子快照边界的权威模块契约。
...
`GameplayRunLoop` 仍负责敌人 / 机关 / 奖励 / 目标 / 撤离 primitive 的实体生成、`Combat`、`PoolManager` 和 Run v5 总快照。`ModuleWorldManager` 不直接生成玩法实体。
