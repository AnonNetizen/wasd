...
- 提供正式数据 schema 校验入口，当前覆盖 `player.json`、`characters.json`、`game_modes.json`、`meta_progression.json`、`growth.csv`、`growth_pools.json` 与 `strings.csv`。
...
- 当前 F3 schema 覆盖：
  ...
  - `characters.json`：角色 id、名称 / 描述 key、默认解锁、tags、capabilities、控制配置和角色基础属性。
  ...
  - `game_modes.json`：模式 id、名称 / 描述 key、默认解锁、participants / teams、角色池、成长池、content tag blocklist 与玩家基础属性轻量覆盖；角色池 id 必须存在于 `characters.json`。
  - `strings.csv`：key 前缀、`zh_CN` / `en` 必填、唯一 key。
- 当前只校验 `characters.json` 与 `game_modes.json` 的数据边界，不实现角色选择 UI、模式选择 UI、匹配、联网、刷怪、成长抽取、输入 profile 切换或模式运行时。
