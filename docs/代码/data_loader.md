...
| `client/data/gear_mods.json` | schema v5 的 7×7 棋盘、严格 effect / map / grid 定义、固定 modifiers / 行为、奖励池、图鉴与解锁边界 |
...
- 当前 F3 schema 覆盖：
  ...
  - `gear_mods.json`：根级 7×7 board / 13 格掩码、拾取配置、有序奖励池和严格 kind 判别联合。effect 必须且只能含 slot + 固定 `{stat,type,value}` modifiers；map 必须且只能含登记的 `map_behavior`；grid 必须且只能含登记的 `grid_behavior`。名称、描述、稀有度与图标只用于展示。
  - `gear_mod_drop_tables.csv`：装备 Mod 掉落来源敌人、Mod id、掉落概率和敌人等级区间；敌人必须存在于 `enemies.csv`，Mod 必须存在于 `gear_mods.json`，概率必须是 `0.0..1.0`。
  - `gear_mods.json` schema v5：校验宽高 / 中心 / 精确掩码、非法坐标、类型字段混用、行为参数、公共池、图鉴和默认解锁；明确拒绝旧等级 / 溢出字段、通用 `behavior`、旧 `reset_on_layer_exit` 及跨 kind 字段。`gear_mod_gameplay_fingerprint_payload()` 纳入 board、kind / behavior、modifier、奖励池、拾取和掉落顺序，排除名称、描述、稀有度和图标。
