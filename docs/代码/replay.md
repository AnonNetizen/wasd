...
- 下游调用方：`InputService` 写入 gameplay intent，LevelUp 等系统继续写关键 decision；黄金 runner 经 `InputService` 播放，不直接伪造 GUIDE 物理事件。
...
当前 `.replay` 文件 envelope 与内存 recording schema 都为 2。加载器接受 v1 的 `pressed` / `strength` 方向 action，在内存中把 `move_*` / `aim_*` 聚合成 `move` / `aim` Vector2，并把其余按钮转换为 bool；不重写源文件。仓库保留一份 v1 fixture，四条正式黄金回放重录为 v2。未来格式变化必须提升版本并提供明确迁移；不能把玩家续局 `run` 存档和回放文件混成同一格式。
