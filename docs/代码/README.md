...
| Replay | `docs/代码/replay.md` | `client/scripts/autoload/replay.gd` | Replay v9：统一效果 / Gear Mod v6 / 掉落与奖励池 / 精确本地 Mod 环境进入指纹；旧 Replay v8 文件保留但拒绝播放 |
| PoolManager | `docs/代码/pool_manager.md` | `client/scripts/autoload/pool_manager.gd` | F2 第五片已建立 |
| SaveManager | `docs/代码/save_manager.md` | `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v19、7×7 assignment、Gear Mod 棋盘 / 效果状态 / 本地 Mod 环境 / 带 ID 地面物、稀疏内容进度与冻结池；旧 Run v18 文件保留但拒绝继续 |
| GearModSystem / GearModPickup | `docs/代码/gear_mod_system.md` | `client/scripts/autoload/gear_mod_system.gd`、`client/scripts/gameplay/gear_mod_pickup.gd`、`client/data/gear_mods.json`、`client/data/gear_mod_drop_tables.csv`、`client/tools/gear_mod_smoke.gd`、`client/tools/gear_mod_pickup_smoke.gd` | schema v6 可组合组件查询；所有 Mod 奖励先生成池化 CPU 实体，玩家确认合法棋盘坐标后注册 modifier / program / board rule；无升级、满阶或溢出接口 |
| ContentUnlockSystem | `docs/代码/content_unlock_system.md` | `client/scripts/autoload/content_unlock_system.gd`、`client/scripts/ui/codex_panel.gd`、`client/data/content_unlock_rules.json` | 默认开放 / 显式锁定、Meta v4、Run v19 冻结池、Replay v9 与标题图鉴；本地 Gear Mod 安装即开放且不写 Meta |
...
| SkillSystem | `docs/代码/skill_system.md` | `client/scripts/data/hero_composition_resolver.gd`、`client/scripts/gameplay/skill_system.gd`、`client/data/characters.json`、`client/data/skills.json` | 主／子英雄四槽、能量、能力缩放与 skills v3 激活程序已建立；效果执行委托统一 Runtime |
...
| ModuleWorldManager | `docs/代码/module_world_manager.md` | `client/scripts/gameplay/module_world_manager.gd`、`client/scripts/gameplay/module_chunk.gd`、`client/data/module_worlds.json`、`client/data/module_templates.json`、`client/data/modules/*.json` | 世界 schema v5 的 7×7 / 三角意识核、模块 schema v4、12 chunk、Run v19 与 Gear Mod v6 当前模块周期 program |
...
| WorldEventSystem | `docs/代码/world_event_system.md` | `client/scripts/gameplay/world_events/`、`client/scenes/gameplay/world_events/`、`client/data/world_events.json` | 五类交互事件、7×7 assignment、带 ID Mod 配置拾取、统一效果事件桥与 Run v19 幂等恢复 |
