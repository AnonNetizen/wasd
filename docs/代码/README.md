...
| Replay | `docs/代码/replay.md` | `client/scripts/autoload/replay.gd` | Replay v7：冻结内容池、规范化 Gear Mod 玩法指纹并忽略本机 Meta；旧 v6 明确拒绝 |
| PoolManager | `docs/代码/pool_manager.md` | `client/scripts/autoload/pool_manager.gd` | F2 第五片已建立 |
| SaveManager | `docs/代码/save_manager.md` | `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v17、7×7 assignment、可重复的局内 Gear Mod `mod_ids`、未拾取 Mod、稀疏内容进度与冻结池；旧 Run v16 及更早版本不兼容 |
| GearModSystem / GearModPickup | `docs/代码/gear_mod_system.md` | `client/scripts/autoload/gear_mod_system.gd`、`client/scripts/gameplay/gear_mod_pickup.gd`、`client/data/gear_mods.json`、`client/data/gear_mod_drop_tables.csv`、`client/tools/gear_mod_smoke.gd`、`client/tools/gear_mod_pickup_smoke.gd` | 无状态固定效果查询与预览服务；所有 Mod 奖励先生成池化 CPU 实体，玩家按 `interact` 拾取成功后由 RunLoop 追加一个无等级实例，重复 id 逐份乘算；无升级、满阶、溢出或数量接口 |
| ContentUnlockSystem | `docs/代码/content_unlock_system.md` | `client/scripts/autoload/content_unlock_system.gd`、`client/scripts/ui/codex_panel.gd`、`client/data/content_unlock_rules.json` | ADR #189 默认开放 / 显式锁定、Meta v4、Run v17 冻结池、Replay v7 与标题图鉴 |
...
| Gameplay Runtime | `docs/代码/gameplay_runtime.md` | `client/scripts/gameplay/*.gd`、`client/scripts/ui/title_menu.gd`、`client/scripts/ui/pause_menu.gd`、`client/scripts/ui/game_over_panel.gd`、`client/scripts/boot/formal_client_boot.gd` | Run v17 Roguelike 对局编排、冻结内容池、7×7 模块世界、无等级 Gear Mod 手动拾取与重复实例、世界事件和结算解锁 |
...
| ModuleWorldManager | `docs/代码/module_world_manager.md` | `client/scripts/gameplay/module_world_manager.gd`、`client/scripts/gameplay/module_chunk.gd`、`client/data/module_worlds.json`、`client/data/module_templates.json`、`client/data/modules/*.json` | 世界 schema v5 的 7×7 / 三角意识核、模块 schema v4、12 chunk 与 Run v17 固定模块快照 |
...
| WorldEventSystem | `docs/代码/world_event_system.md` | `client/scripts/gameplay/world_events/`、`client/scenes/gameplay/world_events/`、`client/data/world_events.json` | ADR #173 / #175 / #188–#193 五类交互事件、7×7 assignment、无等级 Mod 实体奖励与 Run v17 幂等恢复 |
