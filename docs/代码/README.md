...
| SaveManager | `docs/代码/save_manager.md` | `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v16、7×7 assignment、未拾取 Gear Mod、稀疏内容进度、冻结池与 v15 无损迁移 |
| GearModSystem / GearModPickup | `docs/代码/gear_mod_system.md` | `client/scripts/autoload/gear_mod_system.gd`、`client/scripts/gameplay/gear_mod_pickup.gd`、`client/data/gear_mods.json`、`client/data/gear_mod_drop_tables.csv`、`client/tools/gear_mod_smoke.gd`、`client/tools/gear_mod_pickup_smoke.gd` | 无状态规则与预览服务；所有 Mod 奖励先生成池化 CPU 实体，玩家按 `interact` 拾取成功后才由 RunLoop 原子授予、升阶或满阶转金币 |
| ContentUnlockSystem | `docs/代码/content_unlock_system.md` | `client/scripts/autoload/content_unlock_system.gd`、`client/scripts/ui/codex_panel.gd`、`client/data/content_unlock_rules.json` | ADR #189 默认开放 / 显式锁定、Meta v4、Run v16 冻结池、Replay v6 与标题图鉴 |
...
| Gameplay Runtime | `docs/代码/gameplay_runtime.md` | `client/scripts/gameplay/*.gd`、`client/scripts/ui/title_menu.gd`、`client/scripts/ui/pause_menu.gd`、`client/scripts/ui/game_over_panel.gd`、`client/scripts/boot/formal_client_boot.gd` | Run v16 Roguelike 对局编排、冻结内容池、7×7 模块世界、Gear Mod 手动拾取、世界事件与结算解锁 |
...
| ModuleWorldManager | `docs/代码/module_world_manager.md` | `client/scripts/gameplay/module_world_manager.gd`、`client/scripts/gameplay/module_chunk.gd`、`client/data/module_worlds.json`、`client/data/module_templates.json`、`client/data/modules/*.json` | 世界 schema v5 的 7×7 / 三角意识核、模块 schema v4、12 chunk 与 Run v16 固定模块快照 |
...
| WorldEventSystem | `docs/代码/world_event_system.md` | `client/scripts/gameplay/world_events/`、`client/scenes/gameplay/world_events/`、`client/data/world_events.json` | ADR #173 / #175 / #188–#191 五类交互事件、7×7 assignment、Mod 实体奖励与 Run v16 幂等恢复 |
