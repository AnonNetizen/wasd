...
| Replay | `docs/代码/replay.md` | `client/scripts/autoload/replay.gd` | Replay v8：冻结内容池、7×7 Gear Mod 指纹与 placement 语义决策并忽略本机 Meta；旧 v7 明确拒绝 |
| PoolManager | `docs/代码/pool_manager.md` | `client/scripts/autoload/pool_manager.gd` | F2 第五片已建立 |
| SaveManager | `docs/代码/save_manager.md` | `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v18、7×7 assignment、Gear Mod 棋盘 / 地图计划 / 带 ID 地面物、稀疏内容进度与冻结池；旧 Run v17 及更早版本不兼容 |
...
| ContentUnlockSystem | `docs/代码/content_unlock_system.md` | `client/scripts/autoload/content_unlock_system.gd`、`client/scripts/ui/codex_panel.gd`、`client/data/content_unlock_rules.json` | ADR #189 / #194 默认开放 / 显式锁定、Meta v4、Run v18 冻结池、Replay v8 与标题图鉴 |
...
| Gameplay Runtime | `docs/代码/gameplay_runtime.md` | `client/scripts/gameplay/*.gd`、`client/scripts/ui/*.gd`、`client/scripts/boot/formal_client_boot.gd` | Run v18 Roguelike 对局编排、冻结内容池、7×7 模块世界、空间 Gear Mod 棋盘 / 拾取配置 / 刷怪笼、世界事件和结算解锁 |
...
| ModuleWorldManager | `docs/代码/module_world_manager.md` | `client/scripts/gameplay/module_world_manager.gd`、`client/scripts/gameplay/module_chunk.gd`、`client/data/module_worlds.json`、`client/data/module_templates.json`、`client/data/modules/*.json` | 世界 schema v5 的 7×7 / 三角意识核、模块 schema v4、12 chunk、Run v18 与同坐标刷怪笼行为 |
...
| WorldEventSystem | `docs/代码/world_event_system.md` | `client/scripts/gameplay/world_events/`、`client/scenes/gameplay/world_events/`、`client/data/world_events.json` | ADR #173 / #175 / #188–#194 五类交互事件、7×7 assignment、带 ID Mod 配置拾取与 Run v18 幂等恢复 |
