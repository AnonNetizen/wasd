...
| `client/addons/vfx_library/` | editor-only `VFX Library` 主界面、预览、Inspector 与向导 |
...
| 字段 | 说明 |
|------|------|
| `id` / `editor_name` | 稳定主键与编辑器名；效果 id 不要求生成代码常量 |
...
`presentation_profiles.json.profiles[]` 以 `id` 为主键，可通过 `parent_profile_id` 继承。子 profile 以 cue 为粒度覆盖父绑定；绑定可含 `effects[]`、`audio_id`、`camera_feedback_id`、`screen_effect_id` 和预留 `hit_stop_profile_id`。cue、anchor、domain、space、lifecycle、quality 等固定词进入生成契约。
...
Godot 的 `VFX Library` 主界面支持按领域、技术、空间、生命周期与标签筛选；预览玩家、五种敌人、dummy 和 UI 容器；切换挂点、背景、25/50/100% 尺度、normal/reduced、质量、1/8/32/64 实例及 CHARGE/CONTACT/AFTERMATH。Inspector 为 `VfxEffectRef` / `PresentationProfileRef` 提供同一选择器，主界面可 Apply to Selected。

新建向导只允许 OneShot、AttachedLoop、GroundTelegraph、UITransition、ScreenOverlay、GeometryComposite、Particle、Flipbook、Shader、AnimationTreeStateful 模板；会创建资源、登记 catalog、校验并可派生 profile variant。插件在 release preset 中排除。
...
编辑器人工验收使用固定 CHARGE / CONTACT / AFTERMATH、25% 尺度、normal / reduced motion 和 1/8/32/64 实例组合；性能 benchmark 只在用户当次明确要求时运行。
