...
| `snapshot()` | 无 | Dictionary | Run v6 保存 schema/profile/elapsed/enabled |
...
5. 已生成敌人不订阅 progression，也不会在跨级时重算。模块卸载和 Run v6 续局从敌人快照恢复同一倍率。
...
- Run v6 新增顶层 `difficulty`，并在每个敌人快照保存 `spawn_health_multiplier` / `spawn_damage_multiplier`。
