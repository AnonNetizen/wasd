...
> 本文档是模式级局内威胁时间、难度 profile 接口、敌人出生倍率和初始房间战斗门禁的代码契约权威；改公式、暂停条件、模式引用、难度系数、敌人倍率、Run 快照或测试义务时必须检查 ADR #166 / #175，并同步本文档、GDD §7.3、Gameplay Runtime、Save / Replay 文档和数据手册；只有重大取代既有决策时才改 ADR。
...
| `snapshot()` | 无 | Dictionary | Run v19 保存 schema/profile/name/difficulty coefficient/elapsed/enabled |
...
6. 已生成敌人不订阅 progression，也不会在跨级时重算。模块卸载和 Run v19 续局从敌人快照恢复同一生命 / 伤害 / 金币结果；恢复不消费 `RNG.economy`。
...
- 后续敌人组合、刷新预算或遭遇预算应新增独立数据字段并同步对应 GDD / 模块文档；只有改变重大跨系统架构或长期玩法方向时才写 ADR，不得偷用生命系数推导数量。
