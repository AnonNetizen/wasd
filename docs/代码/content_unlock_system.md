...
| `client/scripts/gameplay/gameplay_run_loop.gd` | 冻结池、计数增量、死亡 / 通关提交、Run v19 快照 |
| `client/scripts/ui/codex_panel.gd` | 标题图鉴三分类、隐私显示、语言 / 焦点 / 返回 |
| `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v19 envelope、环境匹配与旧 Run 拒绝 |
| `client/scripts/autoload/replay.gd` | Replay v9 保存并校验冻结池与 mod environment 快照 |
...
4. 保存退出把快照与增量写入 Run v19；续局继续累计。本地 Gear Mod 安装即开放，不写 Meta。
5. 主动放弃或重开只删除 Run，不提交增量。
6. 本局提交不会改变当前快照；新内容从下一局入池。

## 7. Replay 与隔离

Replay v9 的 context 保存同一 `content_availability` 与精确 `mod_environment`。播放时由 harness 在 RunLoop 挂树前注入该快照，忽略本机 Meta，并关闭长期进度提交；v8 和其他未知版本明确拒绝且不改写。
...
| 续局进度归零 | Run v19 是否包含 `content_progress_delta`，旧 Run / 环境不匹配文件是否被保留并拒绝 |
...
- 改存档 / Replay 字段时追加 save、loading、mod-loader-v2、replay、replay-input、四条 Replay v9 黄金回放与 headless editor / boot。
