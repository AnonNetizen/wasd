...
| `client/scripts/gameplay/gameplay_run_loop.gd` | 冻结池、计数增量、死亡 / 通关提交、Run v18 快照 |
| `client/scripts/ui/codex_panel.gd` | 标题图鉴三分类、隐私显示、语言 / 焦点 / 返回 |
| `client/scripts/autoload/save_manager.gd` | Meta v4 / Run v18 envelope、迁移与旧 Run 拒绝 |
| `client/scripts/autoload/replay.gd` | Replay v8 保存并校验冻结池快照 |
...
4. 保存退出把快照与增量写入 Run v18；续局继续累计。
5. 主动放弃或重开只删除 Run，不提交增量。
6. 本局提交不会改变当前快照；新内容从下一局入池。

## 7. Replay 与隔离

Replay v8 的 context 保存同一 `content_availability`。播放时由 harness 在 RunLoop 挂树前注入该快照，忽略本机 Meta，并关闭长期进度提交；v7 和其他未知版本明确拒绝。
...
| 续局进度归零 | Run v18 是否包含 `content_progress_delta`，旧 Run 是否被正确迁移或拒绝 |
...
- 改存档 / Replay 字段时追加 save、loading、replay、replay-input、四条 Replay v8 黄金回放与 headless editor / boot。
