...
> 权威范围：项目输入门面、规范 action 与归一化 intent、GUIDE context 生命周期、重绑定持久化、设备提示、Godot UI 兼容桥及 Replay v6 输入接管。插件内部架构见 [`guide.md`](guide.md)，action / binding id 白名单见 [`../词表与契约.md`](../词表与契约.md) §7，架构决策见 [`../决策记录.md`](../决策记录.md) ADR #151 / #152 / #161 / #186 / #188 / #189。
...
| `client/resources/input/contexts/gameplay.tres` | 移动、瞄准、开火、技能、交互、暂停与数值面板映射；R / 手柄 East 不产生 gameplay intent |
...
| `user://input_bindings.tres` | `GUIDERemappingConfig` 当前绑定权威，`custom_data.schema_version = 3` |
...
- `Actions.FIRE`、`SKILL_1`～`SKILL_4`、`DASH`、`INTERACT`、`SHOW_STATS_PANEL`、`PAUSE`：gameplay 布尔 intent。旧 `use_active_item` 与 `reload` 已删除。
...
| `inject_playback_value(action_id, value, participant_id) -> bool` | Replay v6 注入 bool / Vector2 intent；仅 playback 模式接受 |
...
1. GUIDE tracker 在 GUI `Control` 消费前的 `_input()` 阶段接收物理事件；GUIDE 再于过程帧解析启用 context 的输入。
...
- `GUIDERemappingConfig.custom_data.schema_version` 当前为 `3`；旧 v1 / v2 自定义绑定可能包含已删除的换弹语义，整体隔离并恢复不含换弹的新发布默认，不能部分沿用。未知 action、context、slot 或输入类型不得部分应用。
...
Replay v6 记录最终 intent；四技能、冲刺和组合选择都纳入录制，已删除的 `reload` 不再是合法输入语义：
...
布尔 action 使用 `value_type: "bool"` 与布尔 `value`。播放时 `Replay` 先启用 playback，再调用 `inject_playback_value()`；此时本地 GUIDE 采样不污染业务值。Replay v6 只接受当前规范 action，并同时恢复冻结内容池、忽略本机 Meta；旧 Replay v5 不迁移并明确拒绝。详见 [`replay.md`](replay.md)。
...
| 改玩家移动 / 瞄准 / 技能 / 冲刺语义 | InputService + Player / SkillSystem 适配层 + Replay v6；不要改 GUIDE 设备底层 |
...
- **菜单鼠标可点击，但游戏内瞄准 / 开火无效**：确认 vendored `GUIDEInputTracker` 仍从 `_input()` 转发，且没有退回会被全屏 HUD `Control` 抢先消费的 `_unhandled_input()`；再检查 gameplay context。
...
修改下列任一内容时同步本文：公开 API / 信号、规范 action、binding id、context / 优先级、设备源选择、边沿锁存、binding schema / 原子保存、Settings / Replay 兼容边界、UI 桥、prompt 刷新或 Replay 输入接管。插件内部变化同时更新 [`guide.md`](guide.md) 与 [`../../client/addons/README.md`](../../client/addons/README.md)。
