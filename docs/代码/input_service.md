...
> 权威范围：项目输入门面、规范 action 与归一化 intent、GUIDE context 生命周期、重绑定持久化、设备提示、Godot UI 兼容桥及 Replay v4 输入接管。插件内部架构见 [`guide.md`](guide.md)，action / binding id 白名单见 [`../词表与契约.md`](../词表与契约.md) §7，架构决策见 [`../决策记录.md`](../决策记录.md) ADR #151 / #152 / #161 / #186。
...
| `inject_playback_value(action_id, value, participant_id) -> bool` | Replay v4 注入 bool / Vector2 intent；仅 playback 模式接受 |
...
Replay v4 记录最终 intent；四技能、冲刺和组合选择都纳入录制，已删除的 `reload` 不再是合法输入语义：
...
布尔 action 使用 `value_type: "bool"` 与布尔 `value`。播放时 `Replay` 先启用 playback，再调用 `inject_playback_value()`；此时本地 GUIDE 采样不污染业务值。Replay v4 只接受当前规范 action，旧回放不迁移并明确拒绝。详见 [`replay.md`](replay.md)。
...
| 改玩家移动 / 瞄准 / 技能 / 冲刺语义 | InputService + Player / SkillSystem 适配层 + Replay v4；不要改 GUIDE 设备底层 |
