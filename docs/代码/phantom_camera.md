...
└── ActiveWorld (Node2D)
    ...
    └── GameplayCameraController (Node2D)
        ...
        └── WeaponRecoilShake (PhantomCameraNoiseEmitter2D)
            └── 独立 PhantomCameraNoise2D resource
```
...
- 相机与震屏当前不写入 run snapshot；恢复局内状态后由 RunLoop 将场景预置 Rig 重新绑定到新建 Player。Player 的武器反冲与敌人击退属于两套独立玩法状态并保存在 Run v18；旧 Run v17 及更早版本因 ADR #194 不兼容。非暂停 Gear Mod 面板捕获期间 pointer aim 与玩法 intent 一起归零，不能继续驱动朝向或相机引导。不要保存 Noise 的瞬时 phase，除非未来新增明确回放 / 存档决策。
