...
> 本文档是世界事件运行时、模块摆放、敌人事件上下文与 Run v9 快照的代码契约；改事件规则、奖励、后台固定、敌人目标或保存字段时必须同步 GDD、ADR、Gameplay Runtime、EnemyAI、ModuleWorldManager、SaveManager 与测试策略。
...
- Run v9 保存事件、固定模块、事件波次计划、事件敌人归属与事务进度；不保存 Node 引用。
...
| 激活 | Controller 固定所属模块；RunLoop 用 `RNG.world_event` 和激活时难度固化完整敌种 / 位置计划及隐藏奖励 |
...
持续事件完成奖励在激活时固定：70% 即时金币、30% 普通 Gear Mod；防御 / 生存 / 占点金币分别为 90 / 75 / 60。Mod 进入 `pending_loot`，只有成功撤离才提交 Meta；事件敌人仍保留普通击杀金币与个体掉落。
...
Run v9 的 `world_events` 块保存 Controller 实例状态与固定波次计划；模块快照保存 `pinned_slots`，敌人快照保存 `event_instance_id` 与稳定 `target_mode`。Controller 保存 `wave_cursor`、`reward_committed`、祭坛尝试 / 成功 / 成功 Mod、血坛使用次数、价格、目标生命、计时和占点双进度，因此恢复不得重发已提交波次、重复扣费、重复献祭或重复发奖励。

旧 Run v8 不尝试有损迁移：SaveManager 标记 `legacy_run_incompatible`，正式流程删除旧 run 并保留 Meta v2。Replay 继续使用 v3，但地图 assignment、RNG 子流和行为摘要变化需要重录黄金回放。
...
- 改事件波次必须保持激活时一次性固化、独立 `RNG.world_event` 和 Run v9 roundtrip。
...
- 完整变更还需三档 lint、headless boot/editor、四条 Replay v3 golden、文档健康、pre-commit 和 1920×1080 手动验收；ADR #143 后不自动运行性能 probe。

## 相关文档

- `docs/游戏设计文档.md` §5.3
- `docs/决策记录.md` ADR #173
