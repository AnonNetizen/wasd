...
| F13 模块世界 smoke | `--module-world-smoke` 启动默认模块载体，验证 49 槽 assignment/hash、左下起点、三个确定性目标候选、不同 seed、三种不同世界事件、77×77 坐标、3×3 邻域与最多三个事件 pin（共 12 chunk）、起点威胁暂停 / combat gate、敌人出生倍率、跨模块状态、带 ID 未拾取 Mod、刷怪笼程序、迷雾、意识核直接完成、Run v19 模块 / 事件 / 棋盘 / 效果程序子快照恢复与坏 map hash 拒绝 | `client/tools/module_world_smoke.gd` |
