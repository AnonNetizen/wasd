...
| F13 模块世界 smoke | `--module-world-smoke` 启动默认模块载体，验证 49 槽 assignment/hash、左下起点、三个确定性目标候选、不同 seed、三种不同世界事件、77×77 坐标、3×3 邻域与最多三个事件 pin（共 12 chunk）、起点威胁暂停 / combat gate、敌人出生倍率、跨模块状态、未拾取 Mod、迷雾、意识核直接完成、Run v16 模块 / 事件子快照恢复与坏 map hash 拒绝 | `client/tools/module_world_smoke.gd` |
...
| 存档拒绝 | Run v16 的内容池 / 未结算进度 / Gear Mod ranks / 未拾取 Mod / gold / reward choice / difficulty / enemy reward / 7×7 assignment / 目标角落 / map hash / 世界事件 / 显式攻击状态不一致，或读到旧 Run v14 兼容性标记时，启动层删除该 run、回标题并显示一次不可用提示，Meta v4 不受影响 | `restore_failed` / `SaveManager.delete(..., save_kind_run)` |
...
- 标题菜单的“继续游戏”只在 `SaveManager.has_save(slot_0, run)` 为真时可见；标题页提供图鉴但不提供 Gear Mod 局外配置。开始新局和重开会删除旧 `run` 存档、建立空的局内 Mod 状态、冻结当前可用内容，并通过 `RNG.set_random_run_seed()` 生成新的主 seed。继续游戏先显示加载界面，再读取和校验 Run v16；失败时提示并隐藏继续按钮，成功时恢复同一 7×7 assignment / 目标角落及未拾取 Mod，并按 `ui_restore` 恢复普通游玩、暂停菜单或通用奖励选择面板。
...
新游戏流程为“标题开始 → 英雄组合选择 → Loading → 游戏”；选择页默认读取 Meta v4 上次确认组合并与当前解锁英雄求交，首次为冷静主 + 愤怒子，局外禁用同英雄。重开沿用当前组合和 difficulty profile 并重新冻结可用池 / 重建 7×7 世界，继续游戏直接恢复 Run v16 组合、内容池、未结算进度、武器冷却 / 分层修正、局内 Gear Mod ranks、未拾取 Mod、金币、未完成奖励选择、难度、assignment / 目标角落、世界事件、敌人奖励与显式攻击状态。Run v15 无损迁移，旧 Run v14 显示一次不兼容提示并只删除 run，Meta v4 不受影响。`--module-world-technical-slice` 与 `--open-warzone` 仍只作 opt-in 回归入口；DebugTools 只在 debug/dev_tools 或 smoke 路径启用。
