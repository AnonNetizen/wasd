...
| `client/tools/content_progression_smoke.gd` | `--content-progression-smoke` 下验证解锁规则、Meta / Run、冻结池与长期进度隔离 |
| `client/tools/codex_smoke.gd` | `--codex-smoke` 下验证标题入口、三分类、锁定隐私、语言与焦点返回 |
...
| F11 装备 Mod smoke | `--gear-mod-smoke` 启动时只挂载 `GearModSmoke`，验证固定效果、重复实例乘算、单份授予、应用、清空、恢复和掉落，且不存在等级 / 升级接口 | `client/tools/gear_mod_smoke.gd` |
| F13 模块世界 smoke | `--module-world-smoke` 启动默认模块载体，验证 49 槽 assignment/hash、左下起点、三个确定性目标候选、不同 seed、三种不同世界事件、77×77 坐标、3×3 邻域与最多三个事件 pin（共 12 chunk）、起点威胁暂停 / combat gate、敌人出生倍率、跨模块状态、带 ID 未拾取 Mod、刷怪笼、迷雾、意识核直接完成、Run v18 模块 / 事件 / 棋盘子快照恢复与坏 map hash 拒绝 | `client/tools/module_world_smoke.gd` |
...
| 存档拒绝 | Run v18 的内容池 / 未结算进度 / Gear Mod 棋盘 / 地图行为状态 / 带 ID 未拾取 Mod / gold / reward choice / difficulty / enemy reward / 7×7 assignment / 目标角落 / map hash / 世界事件 / 显式攻击状态不一致，或读到旧 Run v17 兼容性标记时，启动层删除该 run、回标题并显示一次不可用提示，Meta v4 不受影响 | `restore_failed` / `SaveManager.delete(..., save_kind_run)` |
...
- 启动日志输出 `data_schema_ok`、`mods`、`player_stats`、`characters`、`weapons`、`enemies`、`content_unlock_rules`、`hazards`、`map_layouts`、`module_worlds`、`module_templates`、`warzone_directors`、`spawn_waves`、`relics`、`active_items`、`consumables`、`locale_keys`、`level_progression_profiles`、`reward_choice_pools`、`game_modes`、`platform_provider`、`platform_available` 等 smoke 计数 / 状态。
- 启动脚本不硬编码玩家可见文本；标题、加载、HUD、设置、失败页和装备 Mod 面板文案见 `client/locale/strings.csv`。加载界面只显示 `ui_loading`，通用准备失败回标题显示 `ui_loading_failed`。
- 标题菜单的“继续游戏”只在 `SaveManager.has_save(slot_0, run)` 为真时可见；标题页提供图鉴但不提供 Gear Mod 局外配置。开始新局和重开会删除旧 `run` 存档、建立初始 Gear Mod 棋盘、冻结当前可用内容，并通过 `RNG.set_random_run_seed()` 生成新的主 seed。继续游戏先显示加载界面，再读取和校验 Run v18；失败时提示并隐藏继续按钮，成功时恢复同一 7×7 assignment / 目标角落、棋盘 / 地图行为及带 ID 未拾取 Mod，并按 `ui_restore` 恢复普通游玩、暂停菜单或通用奖励选择面板。
...
- 修改 `--content-progression-smoke` / `--codex-smoke` 挂载、标题图鉴或内容池注入时，追加 `python tools/godot_bridge.py --project client content-progression-smoke`、`python tools/godot_bridge.py --project client codex-smoke`、`runtime-smoke` 与 headless boot。
...
新游戏流程为“标题开始 → 英雄组合选择 → Loading → 游戏”；选择页默认读取 Meta v4 上次确认组合并与当前解锁英雄求交，首次为冷静主 + 愤怒子，局外禁用同英雄。重开沿用当前组合和 difficulty profile 并重新冻结可用池 / 重建 7×7 世界，继续游戏直接恢复 Run v18 组合、内容池、未结算进度、武器冷却 / 分层修正、Gear Mod 棋盘与地图计划、带 ID 未拾取 Mod、金币、未完成奖励选择、难度、assignment / 目标角落、世界事件、敌人奖励与显式攻击状态。旧 Run v17 显示一次不兼容提示并只删除 run，Meta v4 不受影响。`--module-world-technical-slice` 与 `--open-warzone` 仍只作 opt-in 回归入口；DebugTools 只在 debug/dev_tools 或 smoke 路径启用。
