...
> 本文档是正式项目内“开发者测试岛”的代码契约；改运行用途、配装配置、训练靶、控制面板、伤害统计、存档隔离或 release 导出边界时，必须同步 ADR #159、GDD §9.20、DebugTools / Gameplay Runtime / FormalClientBoot / GearModSystem 文档、测试策略、AI 导航和 AI 记忆。
...
| 路径 | 作用 |
|------|------|
| `client/scenes/debug/debug_test_arena.tscn` | 继承正式 `gameplay_run_loop.tscn`，预置测试岛控制器、三区地面、边界、出生点和伤害 HUD |
...
| `client/scripts/boot/formal_client_boot.gd` | 标题 / CLI / setup / arena 编排及 Replay / Analytics 临时隔离 |
...
```text
DebugTestArena (inherits GameplayRunLoop)
├── ActiveWorld
│   └── ArenaVisuals
│       ├── CentralZone
│       ├── StationaryZone
...
| 阶段 | 行为 |
|------|------|
| 入口 guard | 仅 `OS.is_debug_build()` 或 `OS.has_feature("dev_tools")` 时显示标题入口并接受 `--debug-test-arena` |
| 标题入口 | 点击“开发者测试岛”先打开 setup；CLI 读取上次配置并直接启动 |
| 配置归一化 | 正 seed、角色、武器、主技能与 Gear Mod/rank 被校验；无效 id 回退到首个合法内容并写诊断 |
| RunLoop 配置 | 只允许在节点入树前调用 `configure_debug_test_arena()`；内部运行用途关闭模块世界、Spawner、导演、兴趣点、撤离、成长与普通结算 |
| 服务隔离 | 入场前保存 Replay / Analytics enabled 状态并临时关闭；清理 arena 或回标题时恢复原状态 |
...
| 离开 | 返回配装会重建测试局；返回标题恢复服务，不保存 run、meta、奖励或临时掉落 |
...
| Signal | 来源 | 用途 |
|--------|------|------|
| `debug_test_arena_setup_requested` | `GameplayRunLoop` | 返回配装并由 FormalClientBoot 重建流程 |
| `quit_to_title_requested` | `GameplayRunLoop` | 复用正式回标题编排 |
| `start_requested(config)` | setup | 保存成功后启动新测试局 |
| `closed_requested` | setup | 关闭配装并返回标题 |

## 存档、回放与埋点边界

- `create_run_snapshot()` 在测试用途返回空字典；测试岛不调用 run 保存 / 删除、正式奖励结算或 meta 写入。
- Gear Mod 掉落与击杀奖励路径在测试用途关闭；临时战斗状态只存在当前节点树。
- 测试岛不会删除进入前已有的正式 run；回标题后“继续游戏”仍按原存档判断。
- Replay / Analytics 只在测试岛生命周期内禁用，退出恢复进入前状态；测试操作不进入正式 replay 或 analytics。
- smoke 必须在隔离用户目录写 meta / run 哨兵并证明前后完全一致。

## Release 边界

- 标题按钮场景节点可存在于正式标题结构中，但 release 配置时必须隐藏且禁用；CLI 请求必须被 guard 拒绝。
- `FormalClientBoot` 只保存调试场景 / 脚本的字符串路径，禁止静态 preload。
...
- 上游：`DataLoader`、`GearModSystem`、`FormalClientBoot`、`GameplayRunLoop`、`UIManager`、`InputService`。
...
| 改启动 / release guard | FormalClientBoot、export preset、project lint | arena smoke + release smoke + project lint |

## 故障排查

| 现象 | 优先检查 |
|------|----------|
| 标题没有入口 | 当前是否 debug/dev_tools；`TitleMenu.configure()` 是否收到 guard 结果 |
| CLI 回到标题 | release guard 是否生效；配置 / 调试资源是否存在 |
...
| 返回标题后 Replay/Analytics 仍关闭 | `_clear_gameplay_runtime()` 是否调用服务恢复 |
| 正式继续游戏消失 | 测试岛路径是否误删 `SaveManager` run；先跑哨兵 smoke |
...
- 改标题 / UI / locale：追加 loading、settings、headless editor/boot、export-tree，并手动检查 1920×1080 的 `zh_CN` / `en`、键鼠与手柄焦点。
