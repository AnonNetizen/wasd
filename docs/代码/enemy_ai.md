...
- 通过 Utility 评分选择动作、FSM 执行蓄力 / 冲锋、Steering 处理移动与无伤害中心分离。
...
| 排查模块墙体或出生合法性 | `client/scripts/gameplay/module_world_manager.gd`、`module_chunk.gd` |
...
| `client/data/enemy_ai_profiles.json` | schema v2 对玩家 AI profile |
...
| `tools/validate_data.py` / `tools/test_data_loader_schema.py` | schema v2 与旧生态字段负例门禁 |
...
| 阶段 | 发生什么 | 关键点 |
|------|----------|--------|
| 配置 | `GameplayRunLoop` 合并敌人基础数据与 profile 后调用 `configure(enemy_data, player)` | 玩家是唯一战斗目标 |
| 感知 | 决策 tick 读取玩家距离、出生点距离、动作 cooldown 和环境状态 | 不扫描其他敌人，不按内容 tag 选目标 |
| 评分 | profile 的 `actions[]` 对合法动作评分 | 行为差异来自数据，不按 enemy id 分支 |
| 执行 | 普通动作直接 steering；冲锋动作经 FSM 处理蓄力、释放、冷却 | 接触与远程攻击都只以玩家为目标 |
...
### `enemy_ai_profiles.json` schema v2

| 字段 | 约束 | 说明 |
|------|------|------|
| `schema_version` | 必须为 `2` | v1 生态字段不得继续加载 |
| `profiles[].id` | 唯一非空字符串 | 由敌人数据引用 |
| `decision_interval` | `> 0` | Utility 重算间隔 |
| `senses.player_weight` | `>= 0` | 玩家候选权重；玩家仍是唯一候选 |
| `movement.*` | 按动作字段校验 | 接近、环绕、冲锋、守家、远程参数 |
| `actions[]` | 非空、id 来自词表 | 可参与评分的对玩家动作 |

schema v2 明确拒绝旧的接触间隔、猎食 / 逃跑目标数组和逃跑距离字段；词表也不再登记旧逃跑 action 与三个生态 tag。后续不得以兼容名义重新接受这些字段。
...
| 名称 | 输入 | 输出 | 约束 |
|------|------|------|------|
| `configure(enemy_data, target)` | 合并数据、玩家节点 | `void` | 对象池取得后的唯一配置入口 |
| `set_movement_bounds(half_extents)` | 地图半尺寸 | `void` | 所有移动路径统一使用 |
| `ai_debug_summary()` | 无 | `Dictionary` | smoke / 调试读取 profile、动作、状态和目标 |
...
- 上游：`DataLoader`、`GameClock`、`GameState`、`PoolManager`、`Combat`、`DamageInfo`、`StatusEffectComponent`、地图边界和生成契约常量。
...
- 新导航或地形感知先抽象环境查询，不得借机引入其他敌人作为 focus target。
...
- 旧生态 profile JSON 属于 schema v1，必须被 DataLoader 和 Python validator 拒绝；不做静默字段忽略。
- 旧快照缺少 AI、状态或来源字段时沿用现有默认恢复规则。

## 相关文档

- `docs/游戏设计文档.md` §5.3
- `docs/决策记录.md` ADR #144
- `docs/词表与契约.md`
