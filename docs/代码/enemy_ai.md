...
| `client/data/enemy_ai_profiles.json` | schema v4 对玩家 AI profile 与攻击参数 |
| `client/data/enemies.csv` | 基础数值、通用内容 tag、对象池和 profile 引用 |
| `client/scripts/contracts/enemy_ai_actions.gd` | 由词表生成的 action 常量 |
| `client/tools/runtime_smoke.gd` | 五种显式攻击、连锁、视线、墙体、击退、暂停与 Run v8 恢复 |
| `tools/validate_data.py` / `tools/test_data_loader_schema.py` | schema v4、精确 attack 字段与旧 contact / movement 攻击字段负例门禁 |
...
| 执行 | 畅通时直追，受阻时读共享流场；爆炸 / 近战 / 冲撞进入显式阶段 FSM，远程按冷却发射 | 爆炸 / 近战要求地形视线；冲撞要求清晰走廊并锁定方向 |
| 提交 | 近战提交时做扇区判定；冲撞释放逐帧线段扫掠；爆炸冻结目标快照后按生成序结算 | 每次冲撞最多命中一次；撞墙结束且不沿墙滑行 |
...
| 保存 | 保存位置、生命、动作 / 阶段 / 剩余时间、锁向、命中位、armed、生成序号、倍率和状态效果 | 不保存节点引用或临时感知缓存；恢复后不得重复提交 |
...
### `enemy_ai_profiles.json` schema v4

| 字段 | 约束 | 说明 |
|------|------|------|
| `schema_version` | 必须为 `4` | v3 的 `movement` 攻击参数、旧接触字段和更早 schema 不得继续加载 |
...
| 远程 attack | `attack_range/keep_distance/cooldown/initial_cooldown/damage/element_id/projectile` | projectile 精确声明池、速度、射程、半径、寿命和枪口偏移 |

schema v4 明确拒绝旧 `movement.charge_*`、`movement.ranged_*`、`contact_interval` 和旧 CSV contact 表头；`sense_radius`、猎食 / 逃跑目标数组与逃跑距离同样继续拒绝。
...
| `ai_debug_summary()` | 无 | `Dictionary` | 额外报告 attack action / 阶段、剩余时间、armed、锁向、倍率后伤害、范围和本次命中 |
| `combat_team_id()` | 无 | `String` | 返回 `team_enemy` |
| `apply_status_effect(effect)` | `StatusEffect` | `bool` | 由统一状态系统调用 |
| `snapshot()` / `restore_snapshot(data)` | 无 / `Dictionary` | `Dictionary` / `void` | Run v8 攻击阶段、锁向、命中、armed、生成序号与倍率恢复；导航派生状态不保存 |
...
- `attack_windup_started(enemy, action_id, context)`：前摇开始或续局重建时触发；context 提供世界位置、旋转、二维缩放和剩余 duration。
- `attack_committed(enemy, action_id, context)`：近战 / 冲撞 / 爆炸提交时触发；爆炸 impact 使用 detached 世界 context。
...
- 远程敌人复用 `ai_action_ranged_attack` 与 `actions[].attack.projectile`；投射物必须池化并只命中玩家队伍。
...
| 调远程压力 | 远程 `attack.cooldown/keep_distance/projectile.*` | 数据手册 | data/schema + runtime smoke |
...
- 改实体状态或 run 快照：追加 L1、runtime 与 save smoke，验证 Run v8 阶段 / 击退 roundtrip 和旧 v7 不兼容。
...
- schema v1–v3 profile、旧 `sense_radius`、旧 movement 攻击字段与旧 contact CSV 表头必须被双端 validator 拒绝，不做静默忽略。
