...
- 敌人普通移动、冲锋、中心分离和续局恢复位置都受 `MapManager.bounds()` 约束，避免 AI 或分离逻辑把敌人中心推出有限地图。
...
| 边界 | 普通移动、冲锋移动、中心分离和快照恢复后统一 clamp 到有限地图 bounds | `Enemy.set_movement_bounds()` |
...
| `set_movement_bounds(bounds)` / `clear_movement_bounds()` | `Rect2` / 无 | `void` | 由 `GameplayRunLoop` 注入有限地图 bounds；池化释放 / reset 清空 |
...
- 上游依赖：`DataLoader`、`GameClock`、`GameState`、`PoolManager`、`Combat`、`DamageInfo`、`StatusEffectComponent`、`MapManager.bounds()`、生成契约 `EnemyAiActions` / `AbilityTags`、`content_tags`。
