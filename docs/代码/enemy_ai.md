...
- 以 `runtime_spawn_serial` 固定连锁爆炸目标结算；敌人金币只在实际生成时由 `RNG.economy` 锁定，并保持模块墙体、全图边界、对象池、Run v12 续局和 Replay v4 可控。
...
| `client/tools/runtime_smoke.gd` | 五种显式攻击、事件防御目标、远程锁向点射、连锁、金币快照、无弹药掉落、视线、墙体、击退、暂停与 Run v12 恢复 |
...
| `snapshot()` / `restore_snapshot(data)` | 无 / `Dictionary` | `Dictionary` / `void` | Run v12 攻击阶段、锁向、点射剩余弹数、事件归属 / target mode、命中、armed、生成序号、出生倍率与完整奖励明细恢复；导航派生状态不保存 |
...
- 改实体状态、金币或 run 快照：追加 L1、runtime 与 save / module-world smoke，验证 Run v12 阶段、奖励明细、事件归属、主目标解析和击退 roundtrip；远程点射覆盖前摇与点射中途的计时 / 剩余弹数一致。
...
- Run v12 恢复当前 profile 已删除的 action 时清空阶段并在下一决策 tick 重选；合法攻击阶段按剩余时间继续，不得重复提交。
- `burst_shots_remaining` 继续随 Run v12 保存；字段存在但点射阶段、计时、方向或剩余弹数非法时，清空攻击并应用一次当前远程冷却，防止重复发弹。
