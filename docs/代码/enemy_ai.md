...
- 以 `runtime_spawn_serial` 固定连锁爆炸目标结算；敌人金币只在实际生成时由 `RNG.economy` 锁定，并保持模块墙体、全图边界、对象池、Run v11 续局和 Replay v3 可控。
...
| `client/tools/runtime_smoke.gd` | 五种显式攻击、事件防御目标、远程锁向点射、连锁、金币快照、弹药掉落归因、视线、墙体、击退、暂停与 Run v11 恢复 |
...
突击枪手当前固定口径为：射程 600 px、保持距离 280 px、首次延迟 0.65 秒、枪口局部蓄光前摇 0.32 秒、4 发、间隔 0.12 秒、冷却 0.95 秒；不提前显示弹道线，每弹 18 伤害、350 px/s、720 px 射程、12 px 半径、2.1 秒寿命，枪口距离 24 px。起手要求玩家视线；一旦提交便完成整轮，玩家移动或失去视线都不改变 `_locked_direction`，Bullet 地形阻挡继续有效。难度只乘每弹 `damage`。ADR #181 只取代 ADR #172 的投射物速度、半径、寿命和枪口距离口径，蓄光与点射行为不变。
...
| `snapshot()` / `restore_snapshot(data)` | 无 / `Dictionary` | `Dictionary` / `void` | Run v11 攻击阶段、锁向、点射剩余弹数、事件归属 / target mode、命中、armed、生成序号、出生倍率与完整奖励明细恢复；导航派生状态不保存 |
...
- 改实体状态、金币或 run 快照：追加 L1、runtime 与 save / module-world smoke，验证 Run v11 阶段、奖励明细、事件归属、主目标解析和击退 roundtrip；远程点射覆盖前摇与点射中途的计时 / 剩余弹数一致。
- 性能 probe 只在用户明确要求性能测试时运行。

## 迁移 / 兼容

- 当前 Run schema 为 v10；v9→v10 因缺少敌人奖励明细与 `RNG.economy` state，标记 `legacy_run_incompatible`，正式启动删除旧 run 并保留 Meta v2。
- Run v11 恢复当前 profile 已删除的 action 时清空阶段并在下一决策 tick 重选；合法攻击阶段按剩余时间继续，不得重复提交。
- `burst_shots_remaining` 继续随 Run v11 保存；字段存在但点射阶段、计时、方向或剩余弹数非法时，清空攻击并应用一次当前远程冷却，防止重复发弹。
