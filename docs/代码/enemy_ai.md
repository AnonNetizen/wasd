...
- 让共享 `Enemy` 脚本 / `EnemyBase` 节点契约通过数据 profile 表现追击、快速近战、冲锋、守家、环绕和远程攻击；每种敌人的静态外观由专属继承场景保存。
...
└── Visual (scene-authored Polygon2D / Line2D)
```

- `CollisionShape2D` 负责敌人与模块墙体的物理碰撞；模块 JSON 的封锁格会合并为静态碰撞。
- `Enemy.configure()` 仍从数据应用 `hit_radius`、生命、速度、伤害和 AI profile，但不得覆盖专属场景的 `fill_color` 或归一化视觉几何。
...
| `snapshot()` / `restore_snapshot(data)` | 无 / `Dictionary` | `Dictionary` / `void` | Run v7 敌人状态、接触计时与生成时生命 / 伤害倍率恢复；导航派生状态不保存 |
