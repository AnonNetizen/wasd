...
| **加一个角色** | 在 `client/data/characters.json` 加一条：专属 `scene_path`、基础属性 / tags / capabilities / 控制配置 / `starting_loadout`；需要不同静态外观时从 `scenes/gameplay/actors/player_base.tscn` 新建继承场景。多个角色 id 可复用同一 TSCN；角色 id 先登记词表 §12.1，文案用 `character_*` key，起始内容引用必须存在。当前只提供 `GameplayRunLoop.configure_character_id()` 入树前入口，不含角色选择 UI；改完跑 data/schema、`actor-scene-smoke` 与 runtime/save |
