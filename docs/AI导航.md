...
| **启动 / 推进正式项目** | 优先读当前阶段工作包；当前 F7 入口为 `docs/AI协作/工作包/F7-SettingsLocalizationUI.md`，目标是设置持久化、设置面板、运行时语言切换、输入重绑定首片和 UI 栈稳定化；设置持久化首片已落地并由 `settings-smoke` 覆盖；F6 局外成长已完成当前阶段验收，维护入口为 `docs/AI协作/工作包/F6-MetaProgression.md`；F4 历史入口为 `docs/AI协作/工作包/F4-MinPlayableLoop.md`，F3 数据闭环入口为 `docs/AI协作/工作包/F3-DataLoader.md` |
...
| **查 Godot 场景树 / headless 启动** | 跑 `python tools/godot_bridge.py export-tree`、`python tools/godot_bridge.py headless-boot`、gameplay runtime 专用 `python tools/godot_bridge.py --project client runtime-smoke`、F7 设置持久化专用 `python tools/godot_bridge.py --project client settings-smoke`、F6 局外成长专用 `python tools/godot_bridge.py --project client meta-smoke` 或 SaveManager 专用 `python tools/godot_bridge.py --project client save-smoke`；默认项目为正式 `client/` |
