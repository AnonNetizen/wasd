...
| **改突击枪手 / 远程点射** | 先读 ADR #171、GDD §5.3、EnemyAI / Gameplay Runtime / PoolManager 文档。保留 `enemy_spitter` / `enemy_ai_ranged_spitter` / 专属池与场景路径；schema v5 的 `windup/burst_count/shot_interval` 控制锁向点射，起手要求视线、提交后不追踪、墙体仍挡弹，难度只缩放每弹伤害。共享 `bullet_basic` 必须按 `source_team` 完整重置玩家黄弹与敌方红弹 / 拖尾。改权重时同步 mode / module / open-warzone、金币、Mod 掉率和总生成速率；验证 Run v8 剩余弹数恢复与四条 Replay v3 golden，不运行性能 probe |
