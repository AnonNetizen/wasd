...
| **改地图边界 / 菱形格 / PCG / 人工摆点** | 查 `docs/代码/map_manager.md`；地图尺寸、`grid.cell_width/cell_height`、玩家出生点、安全半径、刷怪边距、PCG 机关数量 / 间距和人工固定摆点都改 `client/data/map_layouts.json`；bounds 必须整除格尺寸，玩家出生点 / 人工机关必须在格心，PCG 用 `RNG.world` 并吸附到格心，刷怪位置仍用 `RNG.spawn`；玩家和敌人中心移动都应保持在 bounds 内；改完跑 `validate_data`、`runtime-smoke`，机关相关追加 `f9-demo-smoke` |
