...
| **改 HUD 模块小地图 / 交互地标** | 先读 ADR #202、GDD §7.3、Gameplay Runtime 与 ModuleWorldManager 文档。`GameplayRunLoop` 仅从 visited 槽的现有 placements 组装 marker payload，`ModuleMinimap` 只解释生成的 placement / world-event kind 常量；缓存方形、祭坛三角、持续事件信标圆环、传送台菱形，未访问模块不得泄露。改完跑 `test_module_minimap.gd`、完整 / technical module-world、teleporter、world-event、runtime、headless 与三层 lint；清晰度、重叠和色弱辨识保留 L5 待用户人工验收，不运行性能 probe |
