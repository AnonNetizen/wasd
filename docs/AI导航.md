...
| **改 Run v20 捕获字段或跨域恢复顺序** | 先读 ADR #197 / #199 / #201、`docs/代码/run_snapshot_coordinator.md`、Gameplay Runtime / SaveManager / Loading 文档与测试策略。`RunSnapshotCoordinator` 只拥有 gameplay payload 顶层字段及验证 / 恢复端口顺序；RunLoop 继续持有生命周期、节点、signals、领域 leaf 与激活后 `ui_restore`，`mod_environment` 继续由 SaveManager 注入。`PLAYING` 内传送 overlay 必须保存来源站，暂停叠加必须保存 `underlying_state="teleport_choice"` UI 标记；该标记不是 GameState。必跑 coordinator unit、save/runtime/loading、完整 / technical module-world、world-event/effect-runtime、teleporter、headless boot 与四条 Replay v10 golden；保持 Meta v4 / Run v20 / Replay v10 / game v1.19，不运行性能 probe |
...
| **改敌人 ability tag / 状态效果宿主 / Run v19 状态恢复** | 先读 ADR #197、`docs/代码/enemy_ai.md`、`docs/代码/status_effect_component.md` 与测试策略；纯 `EnemyStatusHostRuntime` 单一持有借用的 scene-authored `StatusEffectComponent` 引用与 owned tag 计数，转发 ability tag / status API、source-order summary 和 current / legacy restore。`Enemy` Actor 保留组件发现 / 缺节点诊断、公开 façade、`DamageInfo.source_team` 提取、configure / pool / restore 提交顺序与精确 23-key 快照组装。必跑 Runtime unit、Actor integration、full GUT、runtime/save、完整 / technical module-world、headless 与四条 Replay v9 golden；锁定缺组件 fallback、计数 / 排序 / 深拷贝、`clear(false)` 复用及历史 grant flag，只重跑不重录，不运行性能 probe |
| **改敌人远程点射 / projectile 材化** | 先读 ADR #197、`docs/代码/enemy_ai.md`、PoolManager / Gameplay Runtime 文档与测试策略；无状态 `EnemyRangedAttackHandler` 只借用 `EnemyActionRuntime` 推进 windup / burst / scheduled shot / cooldown，无状态 `EnemyProjectileMaterializer` 只按 typed request 执行 acquire→capability→position→reparent→configure。Enemy 保留 movement / LOS / range / cooldown 起手门禁、signal/context 与 service ports。pool / configure 失败仍按旧语义 emit committed、消耗本发并推进 interval / cooldown。必跑两组 unit、Actor integration、L1/actor/runtime/save/loading/headless 与四条 Replay v9 golden；不运行性能 probe |
| **改敌人近战 handler / 命中提交顺序** | 先读 ADR #197、`docs/代码/enemy_ai.md`、Combat / Gameplay Runtime 文档与测试策略；无状态 `EnemyMeleeAttackHandler` 只借用 `EnemyActionRuntime` 与 typed ports，提交时按 primary→distinct player 构造目标，并保持 available/alive→零距离→range→arc→terrain LOS、全部 damage→committed signal→cooldown/clear→focus restore 的顺序。Enemy 保留目标、Combat、LOS、signal/context 与 Run v19 23-key façade。必跑 Handler unit、Actor integration、full GUT、L1/actor/runtime/save/loading/headless 与四条 Replay v9 golden；测试只走 debug-build typed seams，不运行性能 probe |
| **改敌人冲锋 handler / 碰撞提交顺序** | 先读 ADR #197、`docs/代码/enemy_ai.md`、Combat / Gameplay Runtime / ModuleWorld 文档与测试策略；无状态 `EnemyChargeAttackHandler` 只借用 `EnemyActionRuntime` 与 typed ports，保持缺 focus / 零方向、windup→committed→release、每帧 face→no-slide move→bounds→primary→distinct player、几何命中位前置、player-only knockback 与 finish→cooldown→focus restore。Enemy 保留 CharacterBody movement、Combat、signal/context 与 Run v19 23-key façade。必跑 Handler unit、Actor integration、full GUT、L1/actor/runtime/save/loading/module-world full+technical/headless 与四条 Replay v9 golden；测试只走 debug-build typed seams，不运行性能 probe |
| **改敌人爆炸 handler / 连锁提交顺序** | 先读 ADR #197、`docs/代码/enemy_ai.md`、Combat / Gameplay Runtime / ModuleWorld 文档与测试策略；无状态 `EnemyExplosionAttackHandler` 只借用 `EnemyActionRuntime` 与 typed ports，保持 fresh arm / armed restore 分离、committed 同步重入、direct live origin、direct 后收集 active enemies、enemy frozen origin、跨类不去重、spawn serial 排序与 self defeat 最后执行。Enemy 保留 SceneTree / Node targets、Combat、life / reward、signal/context 与 Run v19 23-key façade。必跑 Handler unit、Actor integration、full GUT、L1/actor/runtime/save/loading/module-world full+technical/headless 与四条 Replay v9 golden；测试只走 debug-build typed seams，不运行性能 probe |
| **改敌人受伤决策 / 击杀归因** | 先读 ADR #197、`docs/代码/enemy_ai.md`、Combat / Gameplay Runtime / EnemyRewardResolver 文档与测试策略；无状态 `EnemyDamageHandler` 只接收 typed request 与惰性 source / amount ports，保持 armed / dead / friendly-fire 短路、生命计算、chain / hit / defeat follow-up、击杀 / 掉落 / cause 归因及公开四键结果顺序。Enemy 保留 Combat 动态入口、生命写入、连锁 arm、hit feedback、defeat / reward / presentation / pool 与 Run v19 23-key façade。必跑 Handler unit、Actor integration、full GUT、L1/VFX/actor/runtime/save/loading/module-world full+technical/headless 与四条 Replay v9 golden；锁定同步重入与 `defeated → Combat.damage_applied`，不运行性能 probe |
...
| **加 / 改三站式传送台网络** | 先读 ADR #199 / #201、GDD §7.3、ModuleWorldManager / Gameplay Runtime / SaveManager / Replay / GameState / GameClock / InputService 文档与测试策略。世界 schema v6 的受约束限量组通过 `RNG.world` 一次性回溯提交 3 座、两两曼哈顿距离至少 4，无解只允许 checked-in 三站 fallback；稳定 ID 为 `teleporter_<module_x>_<module_y>_<cell_x>_<cell_y>`，站号行优先。面板只列同网络 visited 站且排除来源；来源预警、活敌或敌人快照阻止打开 / 提交，危险目标允许传入。选择与淡出入始终保持 `PLAYING`，由面板 owner 和 pending 事务复用 `InputService` 非暂停 capture，只锁玩家 gameplay intent；暂停 action 对传送 owner 透传并叠加 `PAUSED`。全黑帧按存活与来源二次校验→Player teleport→ModuleWorld tick / streaming→camera snap→HUD / prompt 提交，失败不得部分移动。版本保持 Meta v4 / Run v20 / Replay v10 / game v1.19，旧 Run v19 / Replay v9 保留拒绝且不迁移。必跑 contracts、data/schema、module bake、完整 / technical module-world、teleporter/input/save/loading/runtime/replay/replay-input、完整 GUT、三层 lint、docs health、headless boot 与四条 v10 golden；不运行性能 probe。视觉、双语、真实手柄焦点、淡出淡入和危险传入手感保留 L5 待用户人工验收 |
...
| **改 HUD 模块小地图 / 交互地标** | 先读 ADR #202 / #204、GDD §7.3、Gameplay Runtime 与 ModuleWorldManager 文档。`GameplayRunLoop` 从整张 7×7 assignment 的现有 placements 组装 marker payload，`ModuleMinimap` 只解释生成的 placement / world-event kind 常量；缓存方形、祭坛三角、持续事件信标圆环、传送台菱形从开局可见，未访问格仍保留迷雾底色。传送目的地列表继续 visited-only。改完跑 `test_module_minimap.gd`、完整 / technical module-world、teleporter、world-event、runtime、headless 与三层 lint；清晰度、引导效果、重叠和色弱辨识保留 L5 待用户人工验收，不运行性能 probe |
...
| **改官方 JSON / CSV 物理读取** | 先读 ADR #197、`docs/代码/data_loader.md` 与测试策略；纯 `DataSourceReader` 每次重新打开官方文件并返回 typed result，不缓存、不访问 Mod/autoload、不输出错误。DataLoader 保留公开 `load_json/load_csv`、路径诊断、成功后 overlay 与 `_contracts.json` 禁止覆盖。必跑 Reader unit + DataLoader adapter integration、full GUT、validate/schema、ModLoader/L1/runtime/replay、headless 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
...
| **改玩家配置校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、数据手册与测试策略；纯 `PlayerDataValidator` 接收已加载 root、stat / pool callback 与错误 sink，保持 root 与 base stats 两处硬返回、schema→body→base stats→gold→energy 的旧顺序、raw stat count 与金币池双诊断。DataLoader 保留 locale 后 / camera feedback 前的单次读取、Mod overlay、资源路径与条件 count。必跑 validator/full GUT、contracts、validate/schema、L1/runtime/actor/save/loading、headless 与四条 Replay v9 golden；不得顺手收紧 defense / dash / energy under-validation，不运行性能 probe |
| **改角色目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、Gameplay Runtime / SkillSystem 文档、数据手册与测试策略；纯 `CharacterCatalogValidator` 接收已加载 root、五类跨目录索引、locale / contract / actor-scene / stat callbacks 与错误 sink，保持 schema→character、scene→locale→tags / capabilities→element / passive / hero skills→palette→loadout→skill resources→base stats 的完整 source-order、局部短路与历史 bool 缺口。DataLoader 保留表现引用→schema→引用索引三次独立 Mod-aware 读取、PackedScene 加载、Dictionary-root count 与 Credits 后 / LevelProgression 前的位置。必跑 validator/full GUT、contracts、validate/schema、`mod-loader-smoke`、L1/actor/content/codex/runtime/save/loading/replay、headless 与四条 Replay v9 golden；不得缓存读取、收紧 contract / Array bool 缺口、改 Run v19 / Replay v9 或重录 golden，不运行性能 probe |
| **改机关目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、Hazard 文档与测试策略；纯 `HazardCatalogValidator` 接收已加载 rows、locale / contract callback 与错误 sink，保持 id→locale→tags→pool→damage→element→trigger→radius→duration 的旧顺序，以及未知 tag 只诊断、不改变总 bool 的历史缺口。DataLoader 保留表现引用→schema→引用索引三次独立读取、Gear Mod 掉落后 / active items 前的位置与无条件 row count。必跑 validator/full GUT、contracts、validate/schema、ModLoader/runtime/F9/module-world、headless 与四条 Replay v9 golden；不得缓存读取或顺手收紧旧 bool 缺口，不运行性能 probe |
| **改主动道具目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、ItemSystem 文档与测试策略；纯 `ActiveItemCatalogValidator` 接收已加载 root、locale / tag / effect callbacks 与错误 sink，保持 schema→item shape→id→两段 locale→default→tags→charge→effects 的旧顺序、charge 局部短路及未知 tag 只诊断不改 bool。DataLoader 保留表现引用→schema→引用索引三次独立读取、Hazard 索引后 / Consumable 前的位置与 Dictionary root count。必跑 validator/full GUT、contracts、validate/schema、ModLoader/L1/replay/headless 与四条 Replay v9 golden；不得抽共享泛型或顺手收紧 params / extra-key 兼容面，不运行性能 probe |
| **改消耗品目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、ItemSystem 文档与测试策略；纯 `ConsumableCatalogValidator` 接收已加载 root、locale / tag / effect callbacks 与错误 sink，保持 schema→item shape→id→两段 locale→default→tags→stack→effects 的旧顺序、stack 局部短路及未知 tag 只诊断不改 bool。DataLoader 保留表现引用→schema→引用索引三次独立读取、ActiveItem 索引后 / Skills 前的位置与 Dictionary root count。必跑 validator/full GUT、contracts、validate/schema、ModLoader/L1/replay/headless 与四条 Replay v9 golden；不得缓存读取或顺手收紧 params / extra-key 兼容面，不运行性能 probe |
| **改英雄被动目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、SkillSystem 文档与测试策略；纯 `HeroPassiveCatalogValidator` 接收已加载 root、locale / passive id / effect / element callbacks 与错误 sink，保持 root 硬返回、exact int-like schema 1、id→name→desc→effect→params→element→multiplier 顺序，以及 passives 非 Array、空 / 未登记 id 只诊断不改总 bool 的历史缺口。DataLoader 保留 elements 后校验→第二次 hero passive 引用索引读取→weapons 的位置、两次独立 Mod-aware 读取与 Dictionary root count。必跑 validator/full GUT、contracts、validate/schema、ModLoader/L1/loading/codex/runtime/replay/headless 与四条 Replay v9 golden；不得缓存读取、抽共享泛型或顺手带入 Python 更严格规则，不运行性能 probe |
| **改元素目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、Combat / SkillSystem 文档与测试策略；纯 `ElementCatalogValidator` 接收已加载 root、locale / element id / 登记列表 callbacks 与错误 sink，保持 root 硬返回、exact int-like schema 1、元素 source-order、遍历后才读取当前契约定义、组合左右侧历史 bool 缺口与无序 pair 去重。DataLoader 保留表现引用后 / hero passives 前的单次 Mod-aware 读取与两个 Dictionary-root count。必跑 validator/full GUT、contracts、validate/schema、`mod-loader-smoke`、headless 与四条 Replay v9 golden；不得缓存契约列表、抽共享泛型、收紧旧 bool 缺口或重录 golden，不运行性能 probe |
| **改 Spawn Wave 目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、WarzoneDirector 文档与测试策略；纯 `SpawnWaveCatalogValidator` 接收已加载 rows、enemy / hazard / game mode 索引、game-mode contract callback 与错误 sink，保持 raw row count、CSV String 解析、完整 source-order、mode contract 只诊断不降低总 bool 的历史缺口与条件式跨引用。DataLoader 保留 map layouts 后的校验读取和 wave index builder 的第二次独立 Mod-aware 读取。必跑 validator/full GUT、contracts、validate/schema、`mod-loader-smoke`、runtime/save/module-world、headless 与四条 Replay v9 golden；不得复用第二次读取、收紧旧 bool 缺口或重录 golden，不运行性能 probe |
| **改敌人 AI profile 校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、EnemyAI 文档、F14 工作包与测试策略；纯 `EnemyAiProfileValidator` 接收已加载 root、contract callback 与错误 sink，保持 schema v5、profile / perception / targeting / movement / actions / 四类 attack 的完整 source-order、局部短路、exact-key 与未知非攻击 action 只诊断但不降低总 bool 的历史缺口。DataLoader 保留单次 Mod-aware 读取、路径 / contract adapters、Dictionary-root profile count，以及其后 EnemyRewardModel 的调用位置。必跑 validator/full GUT、contracts、validate/schema、`mod-loader-smoke`、L1/runtime/actor/save、headless 与四条 Replay v9 golden；不得收紧旧 bool 缺口、改 Run v19 / Replay v9 或重录 golden，不运行性能 probe |
| **改武器目录校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、Gameplay Runtime / WeaponSystem 文档、数据手册与测试策略；纯 `WeaponCatalogValidator` 接收已加载 root、locale / audio / stat / contract callbacks 与错误 sink，保持 schema v5、recoil model、weapon exact-key、base stats、projectile 的完整 source-order、局部短路与数值关系。DataLoader 保留表现引用→schema→引用索引三次独立 Mod-aware 读取、共享 `WEAPON_STATS` 分类、Dictionary-root weapon count 与 hero passives 后的位置。必跑 validator/full GUT、contracts、validate/schema、`mod-loader-smoke`、L1/gear-mod/runtime/save/settings、完整 / technical module-world、replay-smoke、headless 与四条 Replay v9 golden；不得缓存读取、收紧 root / recoil / projectile extra-key 或 presentation value 兼容面、改 Run v19 / Replay v9 或重录 golden，不运行性能 probe |
...
| **改等级曲线校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md` 与测试策略；纯 `LevelProgressionValidator` 只接收已加载 root 与错误 sink，按 schema→首级成本→分子→分母→关系的旧顺序收集错误。DataLoader 保留文件路径、角色后 / 奖励池前的调用位置，以及 Dictionary root 才写 `level_progression_profiles=1` 的计数语义。必跑 validator unit、validate/schema、headless boot 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
| **改通用奖励选择池校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、数据手册与测试策略；纯 `RewardChoicePoolValidator` 接收已加载 root、locale / modifier callback 与错误 sink，保持 schema→pools→pool→entries→entry 的 source-order，以及非 Array / 未登记 stat “有诊断但 bool 可能仍成功”的历史兼容缺口。DataLoader 保留等级曲线后 / 难度 profile 前的读取、Mod-aware adapters 与 Dictionary root count。必跑 validator/full GUT、validate/schema、`mod-loader-smoke`、headless 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
| **改难度 profile 校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、`docs/代码/difficulty_progression.md` 与测试策略；纯 `DifficultyProfileValidator` 接收已加载 root、locale callback 与错误 sink，保持 root/profile exact-key、id/name/numeric/stage source-order。DataLoader 保留奖励池后读取、Mod-aware locale adapter、Dictionary root count、第二次读取与 game mode 引用索引。必跑 validator/full GUT、validate/schema、headless 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
| **改敌人金币模型校验 / 接线** | 先读 ADR #197、`docs/代码/data_loader.md` 与测试策略；纯 `EnemyRewardModelValidator` 只接收已加载 root 与错误 sink，保持 required→extra→schema→四个数值字段→上下界关系的旧诊断顺序。DataLoader 保留 EnemyAI profile 后 / `enemies.csv` 与 Mod 隔离前的调用位置，仅 Dictionary root 写 `enemy_reward_models=1`。必跑 validator unit、validate/schema、L1、`mod-loader-smoke`、headless boot 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
| **改相机反馈校验 / 引用接线** | 先读 ADR #197、`docs/代码/data_loader.md`、Phantom Camera / Gameplay Runtime 文档与测试策略；纯 `CameraFeedbackValidator` 只接收已加载 root 与错误 sink，保持 schema→aim 4 字段→player shake 7 字段→weapon shake 7 字段→weapon exponent 的旧诊断与 profile 类型短路。DataLoader 保留 player 后的校验、第二次读取、引用索引与 visual effects 前的调用顺序，仅 Dictionary root 写 `camera_feedback_profiles=2`。必跑 validator/full GUT、validate/schema、VFX/runtime/headless 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
| **改 Credits 校验 / locale 接线** | 先读 ADR #197、`docs/代码/data_loader.md`、locale 手册与测试策略；纯 `CreditsValidator` 只接收已加载 root、locale 校验 port 与错误 sink，保持 schema→sections→section→entries→entry 的 source-order 诊断、局部类型短路、`external_*` 条件字段和 copyright 末尾校验。DataLoader 保留 `credits.json` 读取、Mod-aware `_require_locale_key()` adapter，以及仅 Dictionary root 写 section / entry count。必跑 validator/full GUT、validate/schema、`mod-loader-smoke`、headless boot 与四条 Replay v9 golden；保持 data hash 不变，不运行性能 probe |
...
| **加暂停/切换游戏状态** | `GameState.change_state(PAUSED)` 等；UI 通过 `UIManager.push(modal_pause_menu)` 自动联动暂停；`PauseMenu` 已覆盖继续、保存并退出、重开和回标题，也支持从奖励选择或 `PLAYING` 内传送 overlay 上方叠出。奖励选择恢复 `REWARD_CHOICE`；传送面板关闭暂停后回到 `PLAYING` 与仍存在的 pending overlay。Gear Mod / 传送 overlay 都不新增 GameState，复用 `InputService` 非暂停 capture；只有叠加 `PAUSED` 才冻结。业务不直接读写 `get_tree().paused`（见 GDD 9.12 / 9.14） |
| **改关闭游戏确认 / 保存退出** | 先读 ADR #203、`docs/代码/formal_client_boot.md`、`ui_manager.md`、`gameplay_runtime.md` 与 `save_manager.md`。标题退出和 `NOTIFICATION_WM_CLOSE_REQUEST` 统一进入暂停型 `ApplicationQuitModal`；取消恢复底层状态，确认只在可保存 Run 状态调用 `GameplayRunLoop.save_run_snapshot()`，保存失败不得退出。改完跑 `loading-smoke`、`ui-manager-smoke`、`runtime-smoke`、`save-smoke` 与 headless boot，双语布局和真实窗口关闭交给 L5 人工验收 |
...
- 两条**确定性基础设施**：`RNG`（种子化随机，子流分流）/ `GameClock`（移动、冷却、状态与 Replay tick 的底层时间源；玩家加载 / 暂停 / 奖励选择 / 结算冻结，传送选择继续推进）
...
flowchart LR
  ...
  Teleport --> CamCtl & UI
  GS -. PLAYING overlay、PAUSED 覆盖冻结 .-> Teleport
...
> `ModuleWorldManager` 不是 autoload：由 `GameplayRunLoop` 在 `ActiveWorld` 下创建并驱动，依赖世界 / 注册表 / 模块 JSON 与每模块一份的单向生成规范 TSCN。它保留公开 `Node2D` facade；纯 `ModuleWorldLayout` 管 assignment / 坐标 / terrain / hash，纯 `ModuleWorldState` 管 current / visit / pins / slot payload，`ModuleSlotStateCodec` 保持坐标与 wire，纯 `ModuleChunkStreamingController` 管候选 / 已提交场景 cache、12 chunk 池与 3×3 + pins active mapping。Manager 继续编排 typed RNG、场景、导航和恢复提交；Enemy 只经 Manager 门面查询导航。首次进入计划、预警、对象池生成、击杀归因、目标和局内奖励仍由 `GameplayRunLoop` 负责。
