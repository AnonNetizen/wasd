...
## 1. 项目是什么
固定斜俯视 2.5D 刷宝动作生存游戏（灵感：《星际战甲》与《暗黑》的刷装备 / 刷词条长期追求 + 《吸血鬼幸存者》的开放有限大地图与生存压力 + 《以撒的结合》的道具 / 机关 / 构筑组合）。项目保留局内随机奖励和单局构筑变化，但不再沿用旧类型定位；玩法判定仍在 2D 平面内，当前通过水平居中的 `Camera2D` 纵深压缩与低模玩家视觉靠近暗黑式可读性。
...
> F9 起默认键鼠瞄准已从 4 方向改为鼠标相对玩家 / 视口中心方向；子弹可任意角度发射。当前正式视角为固定斜俯视 2.5D：`CenteredCamera` 保持屏幕水平和玩家居中，只做纵深轴压缩，不滚转整个 2D 画面；鼠标瞄准会把屏幕偏移换算回当前 canvas 投影后的世界方向；`Player` 仍是 `CharacterBody2D` 并按 2D 平面移动，表现层通过 `Player3DVisual` 显示低模 3D 胶囊。敌人占位表现和玩家 2.5D 视觉都只做左 / 右两种朝向。方向键、手柄右摇杆和 D-pad 继续作为无鼠标动作时的兜底输入。
