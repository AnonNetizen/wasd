...
| **改局外成长 / 元进度** | 查 GDD §7.2；配置改 `client/data/meta_progression.json`，字段说明同步 `client/data/README.md`，文案同步 `client/locale/strings.csv`；存档走 `SaveManager` 的 `meta` kind，标题 / 结算页局外升级入口看 `client/scripts/ui/meta_progression_panel.gd`、`client/scripts/ui/f4_game_over_panel.gd` 和 `docs/代码/meta_progression_system.md`，新增 currency / upgrade / unlock id 先登记词表 §13 |
...
| **启动 / 推进正式项目** | 优先读当前阶段工作包；F5 暂停 / 存档 / 续局已通过最终手动 checklist，F6 入口为 `docs/AI协作/工作包/F6-MetaProgression.md`，已落地局外成长奖励、结算、meta 存档闭环、标题局外升级入口和结算页直达完整局外升级面板；F4 历史入口为 `docs/AI协作/工作包/F4-MinPlayableLoop.md`，F3 数据闭环入口为 `docs/AI协作/工作包/F3-DataLoader.md` |
