...
| `.opencode/` | OpenCode 平台配置；含 `opencode.json`、agents、commands、skills、rules；`vendor/ai-resources/` 存外部整包候选但不自动启用；核心规则语义与 `.codebuddy/` / `.codex/` 一致 |
...
| **用项目级 AI skill** | OpenCode 加载 `.opencode/skills/<name>/SKILL.md`；当前覆盖 Godot 实现、场景验证、Godot 测试诊断、文档同步、安全提交、事实 review、AI 资源筛选和 MCP 评估；外部整包候选在 `.opencode/vendor/ai-resources/`，仅供审阅和摘取模式；资源筛选与安装清单见 `docs/AI协作/AI技能资源评估.md` |
