...
| `output/steamworks_lab/` | 长期维护的独立 Godot 4.7 Steam 应用，专属 App ID `4955670`；Windows 集成锁定 GodotSteam 4.20 GDExtension + Steamworks 1.64，插件进忽略的 `addons/godotsteam/`，editor 直接走 `--godot` / `GODOT_PATH`，templates 走 Godot 标准用户目录，不再创建 `.toolchain/`；tracked lock / export preset 与 Steam/GodotSteam、Lobby、发布、离线降级边界见其 `README.md`；不等同于正式 `client/PlatformServices` 接入 |
...
| **维护 Steamworks Slime Lab / App ID / Windows 导出** | 先读 `output/steamworks_lab/README.md` 与 ADR #129 / #132 / #133 / #134；App ID 权威配置为 `project.godot` 的 `steam/initialization/app_data/app_id`，`steam_appid.txt` 仅供本地开发。Windows 依赖锁定普通 Godot 4.7 + GodotSteam 4.20 GDExtension + Steamworks 1.64，运行 `py -3 tools/steamworks_lab_toolchain.py setup` / `verify` / `export-release` 管理忽略的插件和 release；editor 直接走 `--godot` / `GODOT_PATH`，templates 走 Godot 标准用户目录，禁止重新创建 `.toolchain/`。正式目录还必须带 GDExtension release DLL、`steam_api64.dll` 与 `THIRD_PARTY_NOTICES.txt`。自动验证包含 `steam_config_smoke.gd`、`steam_runtime_presence_smoke.gd`、headless boot 与本地 ENet，SteamPipe、后台 Depot 和双账号 Steam smoke 仍需后续外部配置；不能把 Lab 直连 SDK 当成正式 `client/PlatformServices` 已接入 |
