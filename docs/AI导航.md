...
| **改敌人显式攻击 / 连锁爆炸** | 先读 ADR #170、GDD §5.3、`docs/代码/enemy_ai.md`、Combat / Gameplay Runtime / SaveManager / VFX 文档。攻击数值改 `enemy_ai_profiles.json.actions[].attack`，不得恢复 CSV contact 字段或把攻击参数放回 `movement`。保持爆猎者入前摇即不可逆、完整 0.65 秒逐代连锁、稳定 `runtime_spawn_serial` 结算、普通爆炸击杀奖励与自爆无奖励边界；Run v8 保存阶段 / armed / 序号 / 玩家击退，Replay v3 wire 不变。按测试策略显式攻击整行义务验证并重录四条黄金回放，不运行性能 probe |
| **改突击枪手 / 远程点射** | 先读 ADR #171 / #172、GDD §5.3、EnemyAI / Gameplay Runtime / PoolManager / VFX 文档。保留 `enemy_spitter` / `enemy_ai_ranged_spitter` / 专属池与场景路径；schema v5 的 `windup/burst_count/shot_interval` 控制锁向点射，起手要求视线、提交后不追踪、墙体仍挡弹，难度只缩放每弹伤害。当前 0.32 秒前摇只在枪口局部红色蓄光，不显示提前弹道线；敌弹为 280 px/s、720 px 射程、2.6 秒寿命。共享 `bullet_basic` 必须按 `source_team` 完整重置玩家黄弹与敌方红弹 / 拖尾。改权重时同步 mode / module / open-warzone、金币、Mod 掉率和总生成速率；验证 Run v8 剩余弹数恢复与四条 Replay v3 golden，不运行性能 probe |
...
| **改局内威胁时间 / 敌人出生强化 / 难度标记器** | 先读 ADR #166 / #170、GDD §7.3、`docs/代码/difficulty_progression.md` 与 `docs/代码/difficulty_marker.md`。曲线只改 `difficulty_profiles.json`，模式引用改 `game_modes.json.difficulty_profile_id`；RunLoop 决定模块起点房暂停、开放战区立即推进和测试岛关闭。保持每 90 秒连续 + 跃升、无上限，只强化新生成敌人的生命和全部显式攻击伤害，不改范围、角度、时序、冲刺、击退、移速、AI、掉落、数量或预算。武器 / 技能门禁分别走 combat gate；Run v8 保存威胁 elapsed、每敌出生倍率和攻击提交状态，Replay v3 摘要使用威胁时间。改完按测试策略整套义务验证，不运行性能 probe |
...
| **改金币等级 / 通用奖励选择** | 查 ADR #169 / #170、GDD §7.1、`docs/代码/gameplay_runtime.md` 与 `client/data/README.md`。金币余额可消费、`gold_earned_total` 只增不减；等级只从累计金币按 `level_progression.json` 的 100 起步、13/10 逐段向上取整曲线推导。只有 `Enemy.defeated` 明确声明 `drops_rewards=true` 才按 `enemies.csv.gold_reward` 生成一个 `gold_orb`；爆猎者实际爆炸退场不计击杀、不掉奖励。奖励由调用方在 `PLAYING` 下用登记 pool / trigger 显式请求 2–5 个候选，进入 `REWARD_CHOICE`；`luck` 无影响，升级不自动请求。Run v8 保存金币、金币球和未完成选择，Replay v3 wire 不变 |
...
| **加存档/读档** | 走 `SaveManager.save/load`；当前 Meta v2 保存上次确认的主 / 子英雄组合并保留 Gear Mod，Run v8 保存组合、金币成长 / 奖励选择、四槽、资源 / 防御、两类位移冲量、状态、屏障、掉落物、威胁时间、每只敌人的出生倍率、显式攻击阶段 / 锁向 / 单次命中 / armed、`runtime_spawn_serial` 和下一序号。旧 Run v7 显示一次不兼容提示后只删除 run、保留 Meta；Replay 独立为 v3。schema 必带标准 envelope 字段，写入用临时文件原子替换、保留备份并隔离坏档；扩展字段时同步 Gameplay Runtime / SaveManager 文档与 smoke |
...
### 5.1 模块清单
**业务模块**：`FormalClientBoot` / `LoadingScreen` / `GameplayRunLoop` / `DeveloperTestArena(debug/dev_tools)` / `Player` / `WeaponSystem` / `Bullet` / `SkillSystem` / `Enemy(EnemyAI)` / `Spawner` / `ModuleWorldManager` / `ModuleNavigationField` / `WarzoneDirector` / `HazardSystem` / `ItemSystem` / `GoldProgression` / `RewardChoiceController` / `GearModSystem` / `ModifierEngine` / `MapManager` / `GameplayCameraController` / `VfxHost` / `GameplayFeedbackController` / `ActorPresentationController` / `PauseMenu` / `Combat` / `StatusEffectComponent`。
...
flowchart LR
  ...
  Difficulty -. Run v8 snapshot .- Save
