...
| `client/scripts/autoload/` | F2 横向 autoload 骨架，已含 `DataLoader` / `RNG` / `GameState` / `GameClock` / `Settings` / `Localization` / `UIManager` |
...
当前 F2 已落地 `DataLoader`、`RNG`、`GameState`、`GameClock`、`Settings`、`Localization`、`UIManager` 的 autoload 骨架；其余 autoload 按 `docs/正式项目工作规划.md` 后续补齐。
