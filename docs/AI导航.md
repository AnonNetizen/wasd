...
| **改敌人寻路 / 感知 / Utility 决策** | 先读 ADR #145 / #146 / #197、`F14-EnemyNavigationAndPerception.md`、`docs/代码/enemy_ai.md`、`docs/代码/module_world_manager.md` 与测试策略；profile 感知参数改 `enemy_ai_profiles.json.perception`，局部活动流场 / 全图静态路径 / 视线改 `module_navigation_field.gd`。`Enemy` Actor 负责节点目标与导航查询并构造 typed `SenseInput`；纯 `EnemyBrain` 拥有 profile / action source order、四级感知与记忆、Utility 分数、决策计时和移动意图。保持 `score > best + 0.001`、ranged cooldown 候选例外、决策提交后才刷新 waypoint；派生状态不进 Run v19。必跑 Brain unit、Actor integration、actor/runtime/module-world/save/headless 与四条 Replay v9 golden，不运行性能 probe |
