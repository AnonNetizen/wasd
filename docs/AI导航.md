...
| **改地图边界 / 菱形格 / PCG / 人工摆点** | 查 `docs/代码/map_manager.md`；地图尺寸、`grid.cell_width/cell_height`、玩家出生点、安全半径、刷怪边距、PCG 机关数量 / 间距和人工固定摆点都改 `client/data/map_layouts.json`；bounds 是菱形外接框，必须整除格尺寸并满足 `bounds.height == bounds.width * grid.cell_height / grid.cell_width`；玩家出生点必须在格心，机关按 `radius_tiles` 奇偶吸附到合法锚点（奇数格心、偶数网格顶点），可见和逻辑地图边界必须是同一个贴格菱形，刷怪位置仍用 `RNG.spawn`；玩家和敌人中心移动都应保持在菱形边界内；改完跑 `validate_data`、`runtime-smoke`，机关相关追加 `f9-demo-smoke` |
