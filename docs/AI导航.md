...
| `output/steamworks_lab/` | 长期维护的独立 Godot 4.7 Steam 应用，专属 App ID `4955670`；Steam/GodotSteam、Lobby、发布与离线降级权威入口为其 `README.md`，不等同于正式 `client/PlatformServices` 接入 |
...
| **维护 Steamworks Slime Lab / App ID** | 先读 `output/steamworks_lab/README.md` 与 ADR #129；App ID 权威配置为 `output/steamworks_lab/project.godot` 的 `steam/app_id`，`steam_appid.txt` 仅供本地开发且必须与之一致，运行时入口为 `scripts/transport_adapter.gd`，自动验证跑 `tests/steam_config_smoke.gd` + headless boot；真实发布另需固定 GodotSteam / Steam Multiplayer Peer 版本、导出 preset、Depot 清单和双账号 Steam 手动 smoke，不能把 Lab 直连 SDK 当成正式 `client/PlatformServices` 已接入 |
