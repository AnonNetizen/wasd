...
| **改地图边界 / 菱形格 / PCG / 人工摆点** | 查 `docs/代码/map_manager.md`；地图尺寸、`grid.cell_width/cell_height`、玩家出生点、安全半径、刷怪边距、PCG 机关数量 / 间距和人工固定摆点都改 `client/data/map_layouts.json`；bounds 必须整除格尺寸，玩家出生点必须在格心，机关按 `radius_tiles` 奇偶吸附到合法锚点（奇数格心、偶数网格顶点），可见地图边界必须是菱形轮廓，刷怪位置仍用 `RNG.spawn`；玩家和敌人中心移动都应保持在 bounds 内；改完跑 `validate_data`、`runtime-smoke`，机关相关追加 `f9-demo-smoke` |
...
> 有限地图可见边界当前由 `MapManager.boundary_points()` 画成菱形；玩家和敌人中心点仍由有限 `bounds` 约束。排查敌人越界时先看 `GameplayRunLoop._apply_enemy_movement_bounds()`、`Enemy.set_movement_bounds()` 与 `runtime-smoke` 的敌人边界断言。
