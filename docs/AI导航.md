...
## 1. 项目是什么
俯视角刷宝动作生存游戏（灵感：《星际战甲》与《暗黑》的刷装备 / 刷词条长期追求 + 《吸血鬼幸存者》的开放有限大地图与生存压力 + 《以撒的结合》的道具 / 机关 / 构筑组合）。项目保留局内随机奖励和单局构筑变化，但不再沿用旧类型定位。
...
> F9 起默认键鼠瞄准已从 4 方向改为鼠标相对玩家 / 视口中心方向；子弹可任意角度发射。玩家表现层已接入 `Player3DVisual` 低模 3D 胶囊，但 `Player` 仍是 `CharacterBody2D` 并按 2D 平面移动；敌人占位表现和玩家 2.5D 视觉都只做左 / 右两种朝向。方向键、手柄右摇杆和 D-pad 继续作为无鼠标动作时的兜底输入。
