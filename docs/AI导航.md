...
| **维护 Steamworks Slime Lab / 本地同屏 / App ID / Windows 导出** | 先读 `output/steamworks_lab/README.md` 与 ADR #129 / #132 / #133 / #134 / #135；本地同屏由 `local_input_router.gd` 分配 P1 键鼠与 P2–P4 手柄，必跑 `local_couch_smoke.gd`、`battle_smoke.gd`、headless boot 和真实手柄 checklist；Steam Lobby 不混入同屏玩家，内部 ENet host/client 只守协议。App ID 权威为 `project.godot` 的 `steam/initialization/app_data/app_id`，`steam_appid.txt` 仅供本地开发。Windows 依赖锁定普通 Godot 4.7 + GodotSteam 4.20 GDExtension + Steamworks 1.64，setup / verify / export-release 直接走 `--godot` / `GODOT_PATH` 和 Godot 标准 templates，禁止重建 `.toolchain/`。SteamPipe、Depot 和双账号 Steam smoke 仍需外部配置；不能把 Lab 直连 SDK 当成正式 `client/PlatformServices` 已接入 |
