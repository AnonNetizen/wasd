...
2. For private implementation, tests, or behavior-preserving refactoring, do not update documentation.
3. Otherwise update the single direct authority; use a module document only for a public contract.
4. Add an ADR only for major, long-lived, cross-system, protocol, red-line, or costly hard-to-reverse decisions.
...
- Do not duplicate task routes, validation matrices, or status across navigation, TODO, current state, and module docs.
