...
- 可调数值进 `client/data/*.json`；玩家可见文本走 `tr("key")`；按键走 InputMap action；约定字符串来自 `docs/词表与契约.md` 且以生成常量引用。
...
- 代码变更必须按 `docs/代码文档规范.md` 判断对应文档；长期模块、公共 API、数据 schema、依赖方向或测试义务变化时同步 `docs/代码/` 模块文档与相关权威文档。
...
- 改完同步文档：新规则进三个平台规则入口；新决策进 ADR；重要对话进 `docs/AI记忆/项目记忆.md` 与会话日志。
