...
- 俯视角显示不得通过旋转 `Camera2D`、`Camera2D.zoom` 非等比缩放、`Camera3D` 正交投影或低模 3D 视觉层来模拟斜俯视；唯一对局级 `GameplayCameraController` 使用 Phantom Camera `GLUED` 跟随当前 Player，开局首次有效瞄准前居中，之后按 `camera_feedback.json.aim_look` 向瞄准方向平滑偏移；保持屏幕水平、等比缩放、无 limit / drag margin，并让稳定引导与 `Camera2D.offset` 震屏噪声分离。`gameplay.reduced_motion` 当前不改变相机。
