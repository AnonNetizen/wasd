...
- Windows / PowerShell 环境必须遵守完整规则正文第 29 节：优先 `rg -F`、全部 `rg` 选项放在 `--` 前、路径走 `-LiteralPath`、原生退出码按工具语义归一化后再进入并行 / fail-fast；不得混用 Bash 转义或嵌套其他 shell。
...
- 俯视角显示不得通过旋转 `Camera2D`、`Camera2D.zoom` 非等比缩放、`Camera3D` 正交投影或低模 3D 视觉层来模拟斜俯视；唯一对局级 `GameplayCameraController` 使用 Phantom Camera `GLUED` 跟随当前 Player，开局首次有效瞄准前居中，之后按 `camera_feedback.json.aim_look` 向瞄准方向平滑偏移；保持屏幕水平、等比缩放、无 limit / drag margin，并让稳定引导与 `Camera2D.offset` 震屏噪声分离。
...
- 未来角色 / Gear Mod / 道具可以突破默认玩法限制，但必须通过词表登记的 capability、content tag、effect primitive 或可复用 strategy 表达；Gear Mod v6 用 `components[]` 任意组合 `modifier` / `program` / `board_rule`，每项有唯一 `component_id`，技能与 Gear Mod 的效果程序统一由 `GameplayEffectRuntime` + Registry + Gateway 执行；禁止按内容 id 写一次性特殊分支，也禁止为某个模式复制一套角色 / Gear Mod / 敌人资源或提前实现网络协议。
- 玩家 mod 只支持 `user://mods/<package_id>/mod.json` manifest v2 本地数据包；未来创意工坊只作为分发层。禁止业务系统直接读取 `user://mods`，禁止执行玩家脚本 / 动态库 / 可执行文件或加载场景 / Shader；本地包只能组合官方 stat、trigger、condition、action、status、enemy、pool、RNG 等核心契约。动态 Gear Mod id 必须以 `mod_<package_id>_` 开头；包独立校验并隔离坏包，活动 Run / Replay 中拒绝重载。
...
- 需求、术语、验收标准、授权边界或上下文含义不清，且无法从项目权威文档或当前对话中可靠确认时，必须先问一个简短澄清问题；不要为了推进任务而自行脑补或替用户做高风险假设。
- 发生上下文总结 / 压缩 / 恢复后，必须先以用户最后一条明确指令重新对齐当前任务；摘要、`Next Steps`、`current_state.json` 或历史待办只作候选参考，不能被当作授权执行；授权边界不清时先问一句。
- 任务说明、计划、工作包、测试策略、checklist 或验收项凡明确标为人工检查 / 人工验收 / 手动验收 / L5 / 真实设备 / 视觉、听觉或手感验收，都只由人工执行；AI 不得用 GUI、浏览器、模拟输入、截图或录像替代，也不得自行宣称通过。AI 继续完成其余自动化验证，只交付前置条件、检查步骤和“待人工验收”状态；普通的“手动运行命令”不因此被视为人工验收。
