...
5. 再跑 `python tools/docs_health_check.py`，收集知识库索引、Markdown 链接、`related_code`、AI 修改说明位置、ADR 矩阵、孤儿文档与同步风险报告。
6. 再优先跑 `tools/health_metrics.py`；若不存在，当前阶段做手动版。
7. 手动版检查 `修改建议.md` 编号唯一、README/GDD 版本一致，并汇总各脚本输出。
8. 输出到 `docs/reports/<YYYY-MM-DD>-health.md`，含未达标项的具体位置。
9. 发现红色项时提示用户立即修，不要自动修；`docs_health_check` report 项只列风险，不当作失败。
