...
`AGENTS.md` 仍是本项目所有 AI agent 的通用开工入口；Claude Code 不安装活跃 `.claude/` 外部工具，也不再通过 vendor reference 读取外部 AI 库。需要可复用流程时，可把 `.opencode/skills/<name>/SKILL.md` 当项目级 prompt 模板读取。

## Claude Code 开工顺序

1. 先读 `AGENTS.md`，按其中“开工 5 步”继续。
2. 强制编码规则按 `AGENTS.md` 第 3 步读取；若外部 AI 库建议与本项目规则冲突，以 `AGENTS.md`、当前平台编码规则、`docs/决策记录.md` 为准。
3. 需要 Godot、headless 验证、测试诊断或试玩复盘流程时，直接读取 `.opencode/skills/` 下对应项目级 skill。
...
| Project Skills（可作 prompt 模板） | `.opencode/skills/` |
