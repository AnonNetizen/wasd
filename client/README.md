...
`res://scenes/boot/main.tscn` 挂载 `res://scripts/boot/formal_client_boot.gd`。启动脚本会先执行正式数据 schema smoke 并输出日志；若校验通过，会显示 F4/F5 阶段最小标题界面；开始新局会挂载 `res://scripts/gameplay/f4_run_loop.gd`，继续游戏会先从 `SaveManager` 读取 `run` payload 再挂载同一 runtime；读取失败或坏档被隔离时会回到标题菜单并显示本局存档重置提示。

F4/F5 runtime 当前仍是阶段性实现，文档见 `docs/代码/f4_min_playable_loop.md`。它不迁移 MVP 临时代码；F5 当前实现 `run` 暂停保存续局、坏档提示、v1 -> v2 迁移和存档可靠性 smoke，不实现完整主菜单、局外成长结算、黄金回放或平衡 sim。
