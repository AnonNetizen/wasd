...
func _ready() -> void:
	...
	print("%s formal client boot scene loaded; contracts=%d rng_streams=%d data_schema_ok=%s player_stats=%d weapons=%d enemies=%d hazards=%d spawn_waves=%d relics=%d credits=%d credit_sections=%d characters=%d locale_keys=%d growth_levels=%d growth_pools=%d game_modes=%d meta_upgrades=%d meta_unlocks=%d settings=%d analytics_events=%d analytics_enabled=%s replay_enabled=%s replay_recording=%s pool_ids=%d active_pools=%d save_kinds=%d save_slots=%d audio_prefixes=%d audio_streams=%d audio_buses_ready=%s locale=%s ui_stack=%d state=%s seed=%d" % [
		BOOT_LOG_PREFIX,
		contract_count,
		stream_count,
		str(data_schema_ok),
