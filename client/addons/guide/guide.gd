...
# Authority: docs/决策记录.md ADR #151, client/addons/README.md
extends Node
...
## The currently active contexts. Key is the context, value is an array [priority, enable_serial].
## The serial is used to break ties when contexts have the same priority - more recently enabled
...
## Monotonic serial used to resolve equal-priority context ties deterministically.
var _context_enable_serial:int = 0
...
## Clears GUIDE's shadow state for all currently held physical inputs.
## Project adapters use this when switching mapping contexts so a held input
## cannot reactivate after the new context becomes active.
func release_pressed_inputs() -> void:
	if _input_state != null:
		_input_state.release_pressed_inputs()
...
func enable_mapping_context(context:GUIDEMappingContext, disable_others:bool = false,  priority:int = 0) -> void:
	...
	_active_contexts[context] = [priority, _next_context_enable_serial()]
	_update_caches()
...
## later in the array having higher precedence when merging inputs (due to later serials).
...
func set_enabled_mapping_contexts(contexts:Array[GUIDEMappingContext]) -> Array[GUIDEMappingContext]:
	...
	# Step 4: Add all new contexts with priority=0 and incrementing serials
	# This ensures contexts later in the array have higher precedence when merging inputs
	for i:int in contexts.size():
		var context:GUIDEMappingContext = contexts[i]
		# If context appears multiple times in the array, the later occurrence wins.
		_active_contexts[context] = [0, _next_context_enable_serial()]
...
func _next_context_enable_serial() -> int:
	_context_enable_serial += 1
	return _context_enable_serial
