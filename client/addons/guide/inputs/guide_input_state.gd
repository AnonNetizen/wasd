...
func _refresh_joy_device_ids(_ignore1:int, _ignore2:bool) -> void:
...
## Clears the pressed shadow state after focus loss or a project mapping-context
## switch. Godot performs the focus-loss part in Input::release_pressed_events()
...
##
## Local fork behavior:
##   - mouse button and joypad state are also cleared so pending input cannot
##     restore a held action after InputService handles the focus-loss notification.
##
## See: https://github.com/godotneers/G.U.I.D.E/issues/189
func release_pressed_inputs() -> void:
	...
	var mouse_buttons_dirty:bool = not _pending_mouse_buttons.is_empty() or not _mouse_buttons.is_empty()
	...
	if joy_axes_dirty:
		joy_axis_state_changed.emit()
## Called when the application loses focus and notifies inputs that maintain
## additional state beyond the physical pressed-state dictionaries.
