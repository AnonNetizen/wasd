...
func abort_detection() -> void:
	...
	match _status:
		...
		DetectionState.INPUT_PRE_CLEAR, DetectionState.DETECTING:
			_deliver(null)
		DetectionState.INPUT_POST_CLEAR:
			_last_detected_input = null
			set_process(true)
...
## Routes a synthetic event through the same path as a viewport input event.
## Intended for project-owned headless smoke tests and input adapters only.
