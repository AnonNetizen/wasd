...
func _key_as_text(physical_key:Key) -> String:
	var display_key:Key = physical_key
	# The headless display server exposes the method but reports it as unsupported.
	# Desktop display servers keep using the localized physical-key label.
	if _is_on_desktop and DisplayServer.get_name() != "headless":
		var localized_key:Key = DisplayServer.keyboard_get_label_from_physical(physical_key)
		if localized_key != KEY_NONE:
			display_key = localized_key

	var result:String = OS.get_keycode_string(display_key)
	if result.is_empty() and display_key != physical_key:
		result = OS.get_keycode_string(physical_key)
	return result
...
func get_text(input:GUIDEInput, options:GUIDEInputFormattingOptions) -> String:
	if input is GUIDEInputKey:
		...
		result.append(_format(_key_as_text(input.key)))
		return "+".join(result)
