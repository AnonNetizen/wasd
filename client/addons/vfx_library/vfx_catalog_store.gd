...
func append_effect(entry: Dictionary) -> Dictionary:
	if effect_root.is_empty():
		return _error("The effects catalog is not loaded; writing is disabled.")
	var effect_id: String = String(entry.get("id", ""))
	if not _is_valid_id(effect_id):
		return _error("Effect ID must match ^[a-z][a-z0-9_]*$.")
	if not effect_by_id(effect_id).is_empty():
		return _error("Effect ID already exists: %s" % effect_id)
...
func _read_catalog(path: String, entry_keys: Array) -> Dictionary:
	if not FileAccess.file_exists(path):
		return _error("Catalog is missing: %s" % path)
	var file: FileAccess = FileAccess.open(path, FileAccess.READ)
	if file == null:
		return _error("Cannot read catalog: %s" % path)
	var parser := JSON.new()
	...
	if not parsed is Dictionary:
		return _error("%s root must be an object or array." % path)
	var root: Dictionary = parsed as Dictionary
	var key: String = _existing_key(root, entry_keys)
	if key.is_empty():
		return _error("%s is missing entry array %s." % [path, "/".join(entry_keys)])
	var entries_value: Variant = root.get(key)
	if not entries_value is Array:
		return _error("%s.%s must be an array." % [path, key])
	return {
		"ok": true,
		"errors": PackedStringArray(),
		"root": root,
...
func _validate_effects(errors: PackedStringArray, warnings: PackedStringArray) -> void:
	...
	for index: int in range(effects.size()):
		...
		var effect_id: String = String(entry.get("id", ""))
		if not _is_valid_id(effect_id):
			errors.append("%s.id is invalid: %s" % [prefix, effect_id])
		elif seen.has(effect_id):
			errors.append("%s.id is duplicated: %s" % [prefix, effect_id])
		else:
			seen[effect_id] = true
		...
		if duration < 0.0:
			errors.append("%s.duration must be non-negative." % prefix)
		...
		if bool(entry.get("high_frequency", false)):
			if String(entry.get("pool_id", "")).is_empty():
				errors.append("%s is high-frequency and requires pool_id." % prefix)
		...
		if not entry.get("tags", []) is Array:
			errors.append("%s.tags must be a string array." % prefix)
...
func _validate_profiles(errors: PackedStringArray, warnings: PackedStringArray) -> void:
	...
	for index: int in range(profiles.size()):
		...
		if not _is_valid_id(profile_id):
			errors.append("presentation_profiles[%d].id is invalid: %s" % [index, profile_id])
		elif profile_ids.has(profile_id):
			errors.append("presentation_profiles[%d].id is duplicated: %s" % [index, profile_id])
		else:
			profile_ids[profile_id] = true
	for profile: Dictionary in profiles:
		var profile_id: String = String(profile.get("id", ""))
		var parent_id: String = String(profile.get("parent", profile.get("parent_id", "")))
		if not parent_id.is_empty() and not profile_ids.has(parent_id):
			errors.append("%s references missing parent profile %s." % [profile_id, parent_id])
		var bindings: Dictionary = _profile_bindings(profile)
		if bindings.is_empty():
			warnings.append("%s has no cue bindings." % profile_id)
		for cue: Variant in bindings.keys():
			...
			if not effect_id.is_empty() and effect_by_id(effect_id).is_empty():
				errors.append("%s.%s references missing effect %s." % [profile_id, cue, effect_id])
...
func _validate_reduced_motion(
	...
	if not value is Dictionary:
		errors.append("%s.reduced_motion must be an object." % prefix)
		return
	...
	if not ["same", "variant", "suppress_optional"].has(mode):
		errors.append("%s.reduced_motion.mode is invalid: %s" % [prefix, mode])
	if mode == "variant":
		var variant_id: String = String(policy.get("effect_id", ""))
		if variant_id.is_empty() or effect_by_id(variant_id).is_empty():
			errors.append("%s reduced-motion variant does not exist: %s" % [prefix, variant_id])
...
func _validate_quality_variants(
	...
	if not value is Dictionary:
		errors.append("%s.quality_variants must be an object." % prefix)
		return
	...
	for quality_value: Variant in variants.keys():
		...
		if not ["low", "medium", "high"].has(quality):
			errors.append("%s.quality_variants has invalid quality %s." % [prefix, quality])
			continue
		var variant_id: String = String(variants.get(quality, ""))
		if variant_id.is_empty() or effect_by_id(variant_id).is_empty():
			errors.append("%s quality variant does not exist: %s" % [prefix, variant_id])
...
func _validate_preview(
	...
	if not value is Dictionary:
		errors.append("%s.preview must be an object." % prefix)
		return
	...
	if not ["dark", "combat", "light"].has(background):
		errors.append("%s.preview.background is invalid: %s" % [prefix, background])
	var checkpoint: String = String(preview.get("checkpoint", ""))
	if not ["charge", "contact", "aftermath"].has(checkpoint):
		errors.append("%s.preview.checkpoint is invalid: %s" % [prefix, checkpoint])
	if float(preview.get("scale", 0.0)) <= 0.0:
		errors.append("%s.preview.scale must be greater than zero." % prefix)
	...
	for profile_id_value: Variant in parent_by_id.keys():
		...
		while parent_by_id.has(cursor):
			if visited.has(cursor):
				errors.append("Presentation profile inheritance contains a cycle: %s" % profile_id)
				break
			visited[cursor] = true
...
func _validate_resource_path(
	...
	if resource_path.is_empty():
		errors.append("%s.resource_path must not be empty." % prefix)
		return
	if not resource_path.begins_with("res://"):
		errors.append("%s.resource_path must be under res://." % prefix)
		return
	for forbidden_prefix: String in FORBIDDEN_RUNTIME_PREFIXES:
		if resource_path.begins_with(forbidden_prefix):
			errors.append("%s references editor-only path %s." % [prefix, resource_path])
	if resource_path.begins_with(NAKED_PRIMITIVE_PREFIX):
		errors.append("%s may not directly catalog a naked geometry primitive." % prefix)
	if not ResourceLoader.exists(resource_path):
		errors.append("%s resource does not exist: %s" % [prefix, resource_path])
...
func _validate_enum(
	...
	if not allowed.has(value):
		errors.append("%s.%s is invalid: %s" % [prefix, field, value])
...
func _write_json(path: String, data: Dictionary) -> Dictionary:
	...
	if file == null:
		return _error("Cannot open for writing: %s" % path)
	file.store_string(JSON.stringify(data, "  ", false, true))
	file.store_string("\n")
	var write_error: Error = file.get_error()
	file.close()
	if write_error != OK:
		return _error("Write failed: %s (%s)" % [path, error_string(write_error)])
