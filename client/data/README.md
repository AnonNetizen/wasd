...
| `interest_points[].gear_mod_rewards[]` | array[object] | 可选，非空；`mod_id` 必须存在于 `gear_mods.json`，`count >= 1` | 领取时通过 `GearModSystem.grant_mod()` 写入库存 |
