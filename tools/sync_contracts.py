...
SECTIONS: tuple[ContractSection, ...] = (
    ...
    ContractSection("meta_unlocks", "### 13.4 解锁项 unlock_id", "meta_unlocks.gd", "MetaUnlocks"),
...
def build_artifacts() -> dict[Path, str]:
    ...
    for section in SECTIONS:
        if section.output_file is None or section.class_name is None:
            continue
        if section.key == "meta_unlocks":
            artifacts[CONTRACTS_DIR / section.output_file] = _render_meta_unlocks(contracts)
        else:
...
def _render_meta_unlocks(contracts: dict[str, list[str]]) -> str:
    lines = [
        "# This file is generated by tools/sync_contracts.py. Do not edit by hand.",
    ...
    for value in contracts["meta_unlock_kinds"]:
        lines.append(f'\t"{value}",')
    lines.extend([
        "]",
        "",
    ...
    for value in contracts["meta_unlocks"]:
        lines.append(f'\t"{value}",')
    lines.extend([
        "]",
        "",
    ...
    for value in contracts["meta_unlock_kinds"]:
        lines.append(f'const KIND_{_constant_name(value)}: String = "{value}"')
    for value in contracts["meta_unlocks"]:
        lines.append(f'const {_constant_name(value)}: String = "{value}"')
    return "\n".join(lines) + "\n"
